...
docker run -d -p $FLOWMANAGER_PORT:$FLOWMANAGER_PORT --name flowmanager -e NODE_ENV=$FLOWMANAGER_CONFIG -e GPII_FLOWMANAGER_LISTEN_PORT=$FLOWMANAGER_PORT $UNIVERSAL_IMAGE
