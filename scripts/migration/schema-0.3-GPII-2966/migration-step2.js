...
// This script performs the second step of the data migration for GPII-2966 deployment. It includes:
// 1. Bump schemaVersion value from 0.2 to 0.3 for all documents that have schemaVersion field.
...
// Usage: node scripts/migration/schema-0.3-GPII-2966/migration-step2.js CouchDB-url [maxDocsInBatchPerRequest]
...
// node scripts/migration/schema-0.3-GPII-2966/migration-step2.js http://localhost:25984 10
...
gpii.migration.GPII2966.initOptions = function (processArgv) {
    ...
    options.maxDocsInBatchPerRequest = Number(processArgv[3]) || gpii.migration.GPII2966.defaultMaxDocsInBatchPerRequest;
    options.numOfUpdated = 0;

    // Set up database specific options
    options.allDocsUrl = options.couchDbUrl + "/_design/views/_view/findDocsBySchemaVersion?key=%22" + gpii.migration.GPII2966.oldSchemaVersion + "%22&limit=" + options.maxDocsInBatchPerRequest;
    options.allDocs = [];
 ...
 */
gpii.migration.GPII2966.migrateStep2 = function () {
...
gpii.migration.GPII2966.migrateStep2();
