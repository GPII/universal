...
#### GET /rawPreferences/:gpiiKey

Where `:gpiiKey` is the GPII key of the desired preferences set. Returns an object containing two keys: 'preferences' key holds the the raw and unmodified preferences set, 'gpiiKey' key holds a string with the GPII key identifying the preferences set set.

##### Example GET query

In the below example a query is made for the preferences set stored under the GPII key `myGpiiKey`. In the example a rawPreferences server is located `http://preferences.gpii.net`

**URL for query: `http://preferences.gpii.net/rawPreferences/myGpiiKey`**
...
Given that the generated GPII key is `123e4567-e89b-12d3-a456-426655440000`, the return payload would like the the following:
...
#### PUT /rawPreferences/:gpiiKey

Used to update an existing preferences set or create a new one with a specific name. Here, `:gpiiKey` is the GPII key of the NP set under which the data part of the PUT request should be saved. If the NP set for that GPII key doesn't already exist, a new one will be created. If one already exist, it will be overwritten by the request. The 'data' portion of the PUT request should contain the a raw preferences set. The preferences set will be saved as is (unmodified). The returned payload is an object containing two keys: 'preferences' key holds the the raw and unmodified preferences set, 'gpiiKey' key holds a string with the GPII key identifying the NP set.

##### Example PUT query

In the below example a query is made for the preferences set stored under the GPII key `myGpiiKey`.
