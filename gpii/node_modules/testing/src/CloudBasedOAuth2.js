...
// On startup of the datastore, overwrite its model so that the stored GPII user token
// and authDecisions agree with the data in the current test fixture
gpii.test.cloudBased.oauth2.populateDataStore = function (dataStore, testCaseHolder) {
    // userToken
    ...
    // authDecisions
    var authDecisionsIdSeq = dataStore.model.authDecisionsIdSeq;
    var authDecisions = fluid.copy(dataStore.model.authDecisions);
    fluid.each(testCaseHolder.options.authDecisions, function (authDecision) {
        var authDecisionCopy = fluid.copy(authDecision);
        authDecisionCopy.id = authDecisionsIdSeq;
        authDecisionsIdSeq = authDecisionsIdSeq + 1;
        authDecisions.push(authDecisionCopy);
    });
    dataStore.applier.change("authDecisionsIdSeq", authDecisionsIdSeq);
    dataStore.applier.change("authDecisions", authDecisions);
};
...
gpii.test.cloudBased.oauth2.sendAccessTokenRequest = function (accessTokenRequest, options, formBody) {
    formBody = gpii.test.cloudBased.oauth2.filter(formBody, options, "accessTokenForm");
    accessTokenRequest.send(gpii.test.stringifyFormBody(formBody));
};
...
gpii.test.cloudBased.oauth2.sendAccessTokenRequestInAuthCode = function (accessTokenRequest, decisionRequest, options) {
    ...
    gpii.test.cloudBased.oauth2.sendAccessTokenRequest(accessTokenRequest, options, formBody);
};
...
gpii.test.cloudBased.oauth2.verifyAccessTokenInAuthCodeResponse = function (body, accessTokenRequest) {
...
gpii.test.cloudBased.oauth2.verifyAccessTokenInClientCredentialsResponse = function (body, accessTokenRequest) {
    var response = gpii.test.verifyJSONResponse(body, accessTokenRequest);
    var token = response.access_token;
    jqUnit.assertValue("Should have received an access token", token);
    console.log("Got access token ", token);
...
gpii.test.cloudBased.oauth2.verifyAuthorizedServicesResponse = function (authorizedServicesRequest, response, clientId) {
    ...
    console.log("Got authorized services payload " + JSON.stringify(payload, null, 2) + ": looking for clientId " + clientId);
    ...
    console.log("Got authorized service record " + JSON.stringify(record, null, 2));
...
gpii.test.cloudBased.oauth2.verifySecuredSettingsResponse = function (body, securedSettingsRequest, expected) {
    ...
    console.log("GOT SECURED SETTINGS " + JSON.stringify(response, null, 2));
...
gpii.test.cloudBased.verifyPayloadMatchMakerOutput = function (body, expectedMatchMakerOutput) {
    ...
    jqUnit.assertDeepEq("Verify expected matchMakerOutput",
                        expectedMatchMakerOutput, payload.matchMakerOutput);
};
...
fluid.defaults("gpii.test.cloudBased.oauth2.testCaseHolder", {
    ...
    }, { // TODO: Correct this strategy once FLUID-5556 is implemented
        record: {
...
// number following implementation of FLUID-5506

gpii.test.cloudBased.oauth2.mainSequence = [{ // 0
        ...
        listener: "gpii.test.cloudBased.oauth2.verifyAccessTokenInAuthCodeResponse",
...
gpii.test.cloudBased.oauth2.filter = function (model, options, filterName) {
    ...
    console.log("Fetched changes " + JSON.stringify(changes, null, 2) +
        " for filter name " + filterName);
...
gpii.test.cloudBased.oauth2.filterModel = function (model, changes) {
    ...
    console.log("Returning filtered model ", JSON.stringify(holder.model, null, 2));
    return holder.model;
};
