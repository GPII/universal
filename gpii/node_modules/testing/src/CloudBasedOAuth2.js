...
gpii.test.cloudBased.oauth2.populateDataStore = function (testCaseHolder, doneEvent) {
    ...
    // TODO: What's the best way get access to and modify the data store?
    var db = testCaseHolder.pouchHarness.pouch.pouch.databaseInstances.gpiiOauth;
    ...
    if (!testCaseHolder.options.isAnonymousToken) {
        ...
        db.get("user-1").then(function (user) {
            return db.put({
                _id: user._id,
                _rev: user._rev,
                type: user.type,
    ...
    // TODO: Update authDecisions for pouchdb
...
gpii.test.cloudBased.oauth2.startPouchHarnessSequence = [
    {
...
gpii.test.cloudBased.oauth2.buildDisruptedFixture = function (testDef, commonRec, disruption, baseDir) {
    ...
    sequence.unshift.apply(sequence, gpii.test.cloudBased.oauth2.startPouchHarnessSequence);
