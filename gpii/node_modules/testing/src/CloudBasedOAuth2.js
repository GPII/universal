...
gpii.test.cloudBased.oauth2.sendAccessTokenRequestInGpiiAppInstallation = function (accessTokenRequest, options) {
    var formBody = {
        grant_type: "password",
        client_id: options.client_id,
        client_secret: options.client_secret,
...
gpii.test.cloudBased.oauth2.verifyDataStoreAuthorization = function (dataStore, expectedAuthorization, verificationDoneEvent) {
    var findWebPrefsConsumerAuthorizationPromise = dataStore.findWebPrefsConsumerAuthorization(expectedAuthorization.gpiiToken,
                                                             expectedAuthorization.clientId,
                                                             expectedAuthorization.redirectUri);
    findWebPrefsConsumerAuthorizationPromise.then(function (foundAuthorization) {
        ...
        jqUnit.assertDeepEq("Authorization contents are as expected",
                            fluid.extend({}, { id: foundAuthorization.id, accessToken: foundAuthorization.accessToken }, expectedAuthorization),
                            foundAuthorization);
