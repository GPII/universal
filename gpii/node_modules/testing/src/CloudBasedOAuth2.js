...
gpii.test.cloudBased.oauth2.populateDataStore = function (dataStore, testCaseHolder) {
    ...
    if (!testCaseHolder.options.isAnonymousToken) {
        ...
        users[0].defaultGpiiToken = testCaseHolder.options.userToken;
        dataStore.applier.change("users", users);

        var gpiiTokens = fluid.copy(dataStore.model.gpiiTokens);
        gpiiTokens.push({
            gpiiToken: testCaseHolder.options.userToken,
            userId: users[0].id
        });
        dataStore.applier.change("gpiiTokens", gpiiTokens);
    }
...
gpii.test.cloudBased.oauth2.verifyDataStoreAuthorization = function (dataStore, expectedAuthDecision) {
    var foundAuthDecision = dataStore.findAuthDecision(expectedAuthDecision.gpiiToken,
    ...
    jqUnit.assertDeepEq("authDecision contents are as expected",
                        $.extend({}, { id: foundAuthDecision.id, accessToken: foundAuthDecision.accessToken }, expectedAuthDecision),
                        foundAuthDecision);
};
...
fluid.defaults("gpii.test.cloudBased.oauth2.testCaseHolder", {
    ...
    distributeOptions: {
        "flowManager.development.dataStore": { // This relies on new FLUID-5824 support to override by namespace
            record: "gpii.oauth2.dataStore.acceptanceData",
            target: "{that gpii.oauth2.dataStore}.options.gradeNames"
        },
        "cloudBased.oauth2.testCaseHolder": {
