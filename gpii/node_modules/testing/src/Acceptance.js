...
gpii.test.acceptance.isCommandProcessSpec = function (processSpec) {
    var command = processSpec.command !== undefined;
    var configName = processSpec.maxTimeouts !== undefined;
    var configPath = processSpec.timeout !== undefined;

    return command && configName && configPath;
};

gpii.test.acceptance.isPromiseProcessSpec = function (processSpec) {
    var type = processSpec.type !== undefined;
    var processToWait = processSpec.processToWait !== undefined;
    var processToClose = processSpec.processToClose !== undefined;

    return type && processToWait && processToClose;
};

gpii.test.acceptance.exec.exec = function (that, processSpec, expected) {
    if (gpii.test.acceptance.isCommandProcessSpec(processSpec)) {
    ...
    } else if (gpii.test.acceptance.isPromiseProcessSpec(processSpec)) {
        processSpec.type(that, processSpec, expected);
    }
};
