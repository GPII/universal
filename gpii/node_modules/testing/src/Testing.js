...
var fluid = fluid || require("infusion"),
    ...
    path = typeof(require) === "undefined" ? null : require("path"), // courtesy to allow use in file-based tests
...
gpii.test.getSettings = function (payload, nameResolver) {
    ...
    fluid.each(payload, function (handlerBlock, handlerID) {
        ...
        ret[handlerID] = gpii.resolveSync(response);
    });
    return ret;
};
...
fluid.defaults("gpii.test.common.testCaseHolder", {
    ...
    components: {
        ...
        // variableResolver: "{that lifecycleManager variableResolver}" without the listener hack above - this requires FLUID-XXXX
        logoutRequest: {
...
// Expand the test fixture using the variableResolver fished out of the real implementation, and place it at 
// this public member for the assertions to use
gpii.test.expandSettings = function (testCaseHolder) {
    testCaseHolder.settingsHandlers = testCaseHolder.variableResolver.resolve(testCaseHolder.options.settingsHandlers);
};
...
gpii.test.buildSingleTestFixture = function (testDef, rootGrades) {
    ...
    testDef.sequence.unshift({
        ...
        args: ["{tests}.settingsHandlers", "{tests}.settingsStore", "{nameResolver}"]
    }, {
        ...
        args: ["{tests}.settingsHandlers", "{nameResolver}"]
    });
