...
// FUNCTIONS FOR STARTING TEST SEQUENCE WHEN LFM IS READY

// Override the original "kettle.test.testDefToServerEnvironment" function provided by the kettle library to boil
// "{flowManager}.events.resetAtStartSuccess" event that is used to ensure the local flow manager is ready before
...
// Add the wait on the readiness of the local flow manager to the test definition.
gpii.test.addStartEventToTestDef = function (testDef) {
...
/**
 * Run test definitions when the local flow manager is ready. This function does not set up the pouchDB to
 * support the cloud based flow manager(CBFM). Use gpii.test.bootstrapServer() to run tests that need pouchDB
 * and CBFM.
 * @param {Array} testDefs - The test definitions.
 */
gpii.test.bootstrapServerNoDb = function (testDefs) {
    kettle.test.bootstrapServer(fluid.transform(testDefs, function (testDef) {
        return gpii.test.addStartEventToTestDef(testDef);
    }));
};

// END OF FUNCTIONS FOR STARTING TEST SEQUENCE WHEN LFM IS READY
