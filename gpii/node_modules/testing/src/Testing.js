...
gpii.test.setInitialState = function (settingsHandlers, nameResolver, onComplete) {
    console.log("KASPER: SETTING INITIAL STATE " + JSON.stringify(settingsHandlers, null, 2)); // TODO KASPER
...
gpii.test.checkConfiguration = function (settingsHandlers, nameResolver, onComplete, customMsg) {
    console.log("KASPER: CHECKING CONFIGURATION " + JSON.stringify(settingsHandlers, null, 2));
    ...
    configPromise.then(function (config) {
        ...
        // console.log(JSON.stringify(noOptions, null, 2))
        // console.log("VS")
...
/** A mixin grade to hoist out the lifecycleManager to a location where it can be easily resolved by
 * various test fixtures. This will become unnecessary after the FLUID-4892 rewrite of Infusion
 */

fluid.defaults("gpii.test.common.lifecycleManagerReceiver", {
    distributeOptions: {
        record: {
            funcName: "gpii.test.common.receiveLifecycleManager",
            args: ["{testCaseHolder}", "{arguments}.0"]
        },
...
gpii.test.common.receiveLifecycleManager = function (testCaseHolder, lifecycleManager) {
    fluid.globalInstantiator.recordKnownComponent(testCaseHolder, lifecycleManager, "lifecycleManager", false);
};
...
gpii.test.initalStateSequence = fluid.freezeRecursive([
    {
        func: "gpii.test.expandSettings",
        args: [ "{tests}", "initialState" ]
    }, {
        func: "gpii.test.setInitialState",
...
gpii.test.buildSingleTestFixture = function (testDef, rootGrades) {
    ...
    if (testDef.initialState) {
        gpii.test.unshift(testDef.sequence, gpii.test.initalStateSequence);
        testDef.expect++;
    }
...
gpii.test.filterSuites = function (files, specs) {
    ...
    var togo = fluid.remove_if(fluid.copy(files), function (file, index) {
