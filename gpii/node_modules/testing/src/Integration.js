...
gpii.test.integration.deviceReporterAware.populate = function (that) {
    fluid.each(that.options.deviceReporters, function (options, key) {
        if (fluid.getGlobalValue(key)) {
            return;
        }
        var expectInstalled = [];
        fluid.each(gpii.tests.deviceReporterAware, function (tests) {
        ...
        var mockedFunction = gpii.test.integration.deviceReporterAware.mockDeviceReporters[options.mockFunc].bind(undefined, expectInstalled);
        fluid.setGlobalValue(key, mockedFunction);
    });
};

fluid.defaults("gpii.test.integration.deviceReporterAware.linux", {
    gradeNames: ["gpii.test.integration.deviceReporterAware", "autoInit"],
    deviceReporters: {
