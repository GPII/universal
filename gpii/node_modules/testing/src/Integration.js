...
fluid.defaults("gpii.test.integration.testCaseHolder.linux", {
    ...
    components: {
        ...
        mockDeviceReporters: {
            type: "gpii.test.integration.mockDeviceReportersRegistry.linux"
        },
        mockPlatformReporter: {
            type: "gpii.test.integration.mockPlatformReporter",
            options: {
...
fluid.defaults("gpii.test.integration.mockDeviceReportersRegistry", {
    gradeNames: ["fluid.eventedComponent", "autoInit"],
    members: {
        deviceReporters: {}
    },
    listeners: {
        onCreate: "gpii.test.integration.mockDeviceReportersRegistry.populate"
    }
});

gpii.test.integration.mockDeviceReporters = {};

gpii.test.integration.mockDeviceReporters.findNameInList = function (expectInstalled, name) {
    var found = expectInstalled.some(function (el) { return el === name});
    return found;
};

gpii.test.integration.mockDeviceReportersRegistry.populate = function (that) {
    fluid.each(that.options.deviceReporters, function (options, key) {
        var expectInstalled = [];
        fluid.each(gpii.tests, function (tests, platform) {
            fluid.each(tests, function (testDefs, test) {
                fluid.each(testDefs, function(testSpecs, item) {
                    if (testSpecs.deviceReporters) {
                        if (testSpecs.deviceReporters.hasOwnProperty(key)) {
                            fluid.each(testSpecs.deviceReporters[key].expectInstalled, function(item) {
        ...
        fluid.defaults(key, options.defaults);
        var mockedFunction = gpii.test.integration.mockDeviceReporters[options.mockFunc].bind(undefined, expectInstalled);
        fluid.setGlobalValue(key, mockedFunction);
    });
};

fluid.defaults("gpii.test.integration.mockDeviceReportersRegistry.linux", {
    gradeNames: ["gpii.test.integration.mockDeviceReportersRegistry", "autoInit"],
    deviceReporters: {
