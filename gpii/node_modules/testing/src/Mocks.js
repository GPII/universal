...
fluid.defaults("gpii.test.integration.mockHolder", {
    ...
    components: {
        ...
        mockActionsHandlers: {
            type: "gpii.test.integration.actionHandlersAware"
        },
        nameResolver: {
            ...
            options: {
                invokers: {
                    resolveName: {
                        ...
                        args: ["{mockSettingsHandlers}", "{mockDeviceReporters}", "{mockActionsHandlers}", "{arguments}.0", "{arguments}.1"]
                    }
                }
            }
        }
...
gpii.test.integration.resolveName = function (settingsHandlerRegistry, deviceReporterAware, actionHandlersAware, name, category) {
    if (category === "settingsHandler") {
        ...
        return actionHandlersAware.resolveName(name);
    } else {
...
gpii.test.integration.deviceReporterAware.mockDeviceReporters.wmiSettingSupported = function (expectInstalled, namespace, query) {
    return expectInstalled.some(function (el) {
...
fluid.defaults("gpii.test.integration.actionHandlersAware", {
    gradeNames: ["fluid.component"],
    rootPath: "gpii.test.integration.actionHandlersAware.mockActionsHandlers",
    members: {
        mockActionsHandlers: {},
        actionsHandlers: {}
    },
    invokers: {
        resolveName: {
            funcName: "gpii.test.integration.actionHandlersAware.resolveName",
            args: ["{that}", "{arguments}.0"]
        }
    },
    listeners: {
        onCreate: "gpii.test.integration.actionHandlersAware.populate"
    }
});


gpii.test.integration.actionHandlersAware.mockActionsHandlers = {};

gpii.test.integration.actionHandlersAware.mockActionsHandlers.rm = function (path) {
    var res;

    try {
        var props = nPath.win32.parse(path);
        if (props.root === "" || props.base === "") {
            res = false;
        } else {
            res = true;
        }
...
gpii.test.integration.actionHandlersAware.mockActionsHandlers.setHighContrastTheme = function () {
    return true;
};

gpii.test.integration.actionHandlersAware.mockActionsHandlers.applyCustomTheme = function () {
    return true;
};

gpii.test.integration.actionHandlersAware.mockActionsHandlers.updateLanguage = function () {
    return true;
};

gpii.test.integration.actionHandlersAware.populate = function (that) {
    fluid.each(that.options.actionsHandlers, function (actionHandler) {
        var mock = that.options.mockActionsHandlers[actionHandler.type];

        if (!mock) {
            var registered = fluid.keys(that.options.mockActionsHandlers).join(", ");
            fluid.fail("Error in actions handler mock configuration - handler " + actionHandler.type +
                " is not registered - registered handlers are " + registered);
        }

        fluid.defaults(that.options.rootPath + "." + actionHandler.type, mock.defaults);

        var mockedFunction = gpii.test.integration.actionHandlersAware.mockActionsHandlers[mock.mockFunc];
        fluid.setGlobalValue(that.options.rootPath + "." + actionHandler.type, mockedFunction);
    });
};

gpii.test.integration.actionHandlersAware.resolveName = function (that, name) {
    var resolvedName = that.options.rootPath + "." + name;
    return resolvedName;
};

fluid.defaults("gpii.test.integration.actionHandlersAware.windows", {
    gradeNames: "gpii.test.integration.actionHandlersAware",
    mockActionsHandlers: {
        "gpii.windows.rm": {
            mockFunc: "rm",
            defaults: {
