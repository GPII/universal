...
gpii.test.cloudBased.oauth2.buildTestFixtureCommon = function (testDef, commonRec, config) {
...
gpii.test.cloudBased.oauth2.buildDisruptedFixture = function (testDef, commonRec, disruption, config) {
    var options = fluid.defaults(disruption.gradeName);
    testDef = gpii.test.cloudBased.oauth2.buildTestFixtureCommon(testDef, commonRec, config);
    if (options.expect) {
...
/*    var topLevel = fluid.transform(disruptions, function (disruption) {
        var levelTwo = kettle.test.bootstrapServer(testDefs, function (testDef) {
            var levelThree = gpii.test.cloudBased.oauth2.buildDisruptedFixture(testDef, commonRec, disruption, config);
            debugger;
            return levelThree; // used in kettle.test.bootstrapServer(), the enclosing function
        });
        var testDefs2 = testDefs;
        var disruptions2 = disruptions;
        debugger;
        return levelTwo;
    });
    var disruptions2 = disruptions;
...
*/
    return fluid.transform(disruptions, function (disruption) {
        return kettle.test.bootstrapServer(testDefs, function (testDef) {
            return gpii.test.cloudBased.oauth2.buildDisruptedFixture(testDef, commonRec, disruption, config);
        });
    });
