...
// On startup of the datastore, populate the data store with to-be-tested GPII GPII key
gpii.test.cloudBased.oauth2.populateDataStore = function (testCaseHolder, doneEvent) {
    ...
    var db = testCaseHolder.pouchHarness.express.expressPouch.databaseInstances.gpii;

    var promisesSequence = [];

    if (testCaseHolder.options.gpiiKey) {
        // add additional GPII key into the database if given
        promisesSequence.push(db.put({
            "_id": testCaseHolder.options.gpiiKey,
            "type": "gpiiKey",
            "schemaVersion": "0.1",
            "prefsSafeId": null,
