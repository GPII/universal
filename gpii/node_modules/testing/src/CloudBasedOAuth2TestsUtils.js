...
// Grade for "disruptions" that are also proper sequence grades.
...
gpii.test.cloudBased.oauth2.runDisruptedTests = function (testDefsToDisrupt, config) {
    fluid.each(testDefsToDisrupt, function (testDefToDisrupt) {
        fluid.each(testDefToDisrupt.disruptions, function (disruption) {
            var disruptedTestDef = gpii.test.cloudBased.oauth2.buildDisruptedFixture(testDefToDisrupt.testDef, {}, disruption, config);

            gpii.test.runTestDefs([disruptedTestDef], function (testDef) {
                var gradedTestDef = gpii.test.testDefToEnvironment(testDef, "gpii.test.couchEnvironment", disruption.sequenceGrade);
