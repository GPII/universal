...
// and authorizations agree with the data in the current test fixture
gpii.test.cloudBased.oauth2.populateDataStore = function (testCaseHolder, doneEvent) {
    ...
    // userToken
    // 1. For a test case with an anonymous token, the db should have a record of the
    // gpiiToken of testCaseHolder.options.userToken. But we want no users
    // in the dataStore to have the gpiiToken of testCaseHolder.options.userToken.
    // 2. For a test case that is not for an anoymous token, set the
    // gpiiToken of the first user in the dataStore to the test
    // case userToken.

    if (testCaseHolder.options.isAnonymousToken) {
        // The record of the gpii token must be in the database for it to be validated
        // even when the token is used anonymously
        db.put({
            _id: "gpiiToken-100",
            type: "gpiiToken",
    ...
    // authorizations
    var postAuthorizationPromises = fluid.transform(testCaseHolder.options.authorizations, function (authorization) {
        return db.post(authorization);
    });
    promisesSequence.push.apply(promisesSequence, postAuthorizationPromises);
...
// The current authorization UI renders the transaction ID as part of a JSON block supplied
...
gpii.test.cloudBased.oauth2.verifyAccessTokenInResponse = function (body, accessTokenRequest) {
    ...
    gpii.test.cloudBased.oauth2.verifyFieldInResponse(response, accessTokenRequest, "oauth2.verifyAccessTokenInResponse", "access_token");
};
...
gpii.test.cloudBased.oauth2.sendAuthorizationRequest = function (authorizationsRequest, authorizationId, withPreferences, payload) {
    if (withPreferences) {
        fluid.set(authorizationsRequest, ["options", "path"], authorizationsRequest.options.path + "/preferences");
    }
    var options = {
        termMap: {
    ...
    authorizationsRequest.send(payload, options);
};
...
gpii.test.cloudBased.oauth2.sendSecuredSettingsRequest = function (securedSettingsRequest, accessTokenRequest) {
    securedSettingsRequest.send(null, {
        headers: {
            Authorization: "Bearer " + accessTokenRequest.access_token
        }
    });
};
...
gpii.test.cloudBased.oauth2.mainSequence = [
    { // 0
        ...
        args: ["{getAuthorizationRequest}", "{getAuthorizedServicesRequest}.authorizedService.authorizationId",
            true]
    }, { // 13
