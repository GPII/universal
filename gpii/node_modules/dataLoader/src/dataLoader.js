 ...
 * The structure of `that.options.databases`:
 * {
 *     dbName1: [data],
 *     dbName2: [data]
 *     ...
 * }
 * The format of [data] is in the structure allowed by CouchDB /_bulk_docs API (http://docs.couchdb.org/en/2.0.0/api/database/bulk-api.html#db-bulk-docs)
 *
...
gpii.dataLoader.load = function (that) {
    ...
    if (dataFileResults.errors.length > 0) {
        ...
        fluid.each(databases, function (dbData, dbName) {
            ...
            // Delete and recreate the database if it already exists; otherwise, create the database
