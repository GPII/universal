/*

    This configuration acts as a base for `gpii.config.cloudBased.development.all.local`.

    It includes configs to instantiate both CouchDB and an express server that can be used to reset the data to its
    initial state.

    Note that PouchDB is only used when GPII runs in the development mode. When GPII runs in the production mode, it is
    expected that an existing CouchDB instance will be available locally.

*/
{
    "type": "gpii.couchConnector.config.base",
    "options": {
        "gradeNames": "gpii.couchConnector",
        "resetServerPort": 8060,
        "databases": {
            "gpii": {
                "data": [
                    "%gpii-universal/testData/dbData/clientCredentials.json",
                    "%gpii-universal/testData/dbData/gpiiAppInstallationClients.json",
                    "%gpii-universal/build/dbData/snapset/gpiiKeys.json",
                    "%gpii-universal/build/dbData/snapset/prefsSafes.json",
                    "%gpii-universal/testData/dbData/views.json"
                ]
            }
        }
    },
    "require": ["gpii-couchdb-test-harness", "couchConnector", "journal"]
}
