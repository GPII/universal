...
(function () {
    ...
    var processData = function (data, dataSource, callback) {
        var responseParser = dataSource.options.responseParser,
            handleError = dataSource.handleError;
    ...
    var urlAll = function (path, method, request, dataSource, callback, model) {
        ...
        var req = http.request(opts, function (res) {
            ...
            res.on("end", gpii.callbackWrapper(request, function () {
                processData(data, dataSource, callback);
            }));
        });
        req.on("error", gpii.callbackWrapper(request, function (error) {
    ...
    var fsAll = function (fileName, method, request, dataSource, callback, model) {
        var args = [fileName];
        if (model) {
            args.push(model);
        }
        args.push("utf8");
        args.push(gpii.callbackWrapper(request, function (error, data) {
            if (error) {
                dataSource.handleError({
                    isError: true,
                    message: error.message
            ...
            processData(data || model, dataSource, callback);
        }));
    ...
    gpii.dataSource.URL.all = function (dataSource, directModel, model, callback) {
        var resolveUrl = dataSource.resolveUrl,
        ...
        if (isFileProtocol) {
            func = fsAll;
            url = url.substring(7);
            method = method === "PUT" ? "write" : "read"
        }
        model = stringifyModel(model);
        func.apply(null, [url, method, request, dataSource, callback, model]);
    };
