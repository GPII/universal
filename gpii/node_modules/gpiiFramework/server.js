...
(function () {
    ...
    gpii.server.preInit = function (that) {
        ...
        that.registerApps = function () {
            fluid.each(that.config.apps, function (app, name) {
                app.createOnEvent = "onRegisterApps";
                that.options.components[name] = app;
            });
            that.events.onRegisterApps.fire();
        };
        that.onConfigHandler = function (config) {
            fluid.set(that.options.components.config, "options", config);
            that.events.createConfig.fire();
        };
    ...
    fluid.defaults("gpii.server.configurations", {
        ...
        components: {
            ...
            // Configuration file
            configFile: {
                type: "gpii.server.config.configFile"
            },
            createRequest: {
    ...
    fluid.defaults("gpii.server.config.configFile", {
        gradeNames: ["autoInit", "gpii.server.config"],
        events: {
            onConfig: {
    ...
    gpii.server.config.configureCreateRequest = function (that) {
    ...
    fluid.demands("gpii.server.config.configure", "gpii.server.config.configFile", {
        funcName: "gpii.server.config.configureCreateRequest",
        args: ["{that}"]
    });
