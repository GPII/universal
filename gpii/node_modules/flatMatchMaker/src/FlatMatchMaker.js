...
(function () {
    ...
    gpii.flatMatchMaker.matchPost = function (flatMatchMaker, requestProxy, body) {
        when(flatMatchMaker.match(body), function (matchedSolutions) {
    ...
    gpii.flatMatchMaker.addInferredCommon = function (preferences, inferred) {
        ...
        fluid.each(inferred, function (context, contextId) {
            ...
            fluid.each(context, function (settings) {
                prefs = $.extend(true, settings, prefs);
            });
            togo.contexts[contextId].preferences = $.extend(true, prefs, togo.contexts[contextId].preferences);
        });
        return togo;
    };
    ...
    gpii.flatMatchMaker.match = function (ontologyHandler, payload, disposeStrategy) {
        ...
        // payload.hierarchicalPrefs = gpii.matchMakerFramework.utils.ontologizePreferences(ontologyHandler, payload.preferences, "flat", "ISO24751");
        ...
        return when(gpii.matchMakerFramework.utils.disposeSolutions(payload, disposeStrategy, transformSpec), function (disposed) {
            var togo = gpii.matchMakerFramework.utils.buildReturnPayload (payload, disposed);
