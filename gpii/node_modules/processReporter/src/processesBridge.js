...
gpii.processes.findProcessesByCommand = function (that, commandName, procArray) {
    if (!procArray) {
        // feds-audit workaround for JAWS
        // getProcessList("jfw") returns nothing
        // procArray = that.getProcessList(commandName);
        procArray = that.getProcessList();
    }
    ...
    return fluid.accumulate(procArray, function (aProcInfo, matchingProcs) {
        var match = aProcInfo.command === commandName ||
            ...
            // feds-audit workaround for JAWS
            // No matter whether we indicate the full name ("jfw.exe") in the solutions registry,
            // it always receives "jfw", which doesn't match with "jfw.exe".
            // There should be a better place to fix this, but can't figure out right now.
            aProcInfo.command.toLowerCase() === (commandNameLower + ".exe");
