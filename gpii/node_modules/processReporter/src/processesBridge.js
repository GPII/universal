...
gpii.processes.findProcessesByCommand = function (that, commandName, procArray) {
    ...
    return fluid.accumulate(procArray, function (aProcInfo, matchingProcs) {
        if (gpii.processes.compareProcessNames(aProcInfo.command, commandName)) {
...
var compare = (os.platform() === "win32")
    ? (new Intl.Collator(undefined, {sensitivity:"base"})).compare
    : Object.is;

/**
 * Compares the names of two processes, returning a boolean indicating whether or not they are equal according to
 * the case-sensitivity rules of the OS.
 *
 * @param processA {String} Name of a process.
 * @param processB {String} Name of another process.
 */
gpii.processes.compareProcessNames = function (processA, processB) {
    return !compare(processA, processB);
};
