...
gpii.rawPreferencesServer.test.get.successfulRequestAssert = function (token) {
    return function (data) {
        ...
        jqUnit.assertDeepEq("Response is correct", data, expected);
    }
};

gpii.rawPreferencesServer.test.get.failingRequestAssert = function (data) {
    data = JSON.parse(data);
    //Error message varies for filesystem vs. couchDB, simply check 'isError' and structure
    jqUnit.assertTrue("Checking that we're getting correct error object for nonexisting token",
        data.isError === true && data.message !== undefined);
};
...
gpii.rawPreferencesServer.test.get.fixtures = [
    {
        ...
        name: "Raw Preferences Server - failing GET test (non-existing user)",
        token: "bogus_np",
        onCompleteSequence: {
            event: "{getRequest}.events.onComplete",
