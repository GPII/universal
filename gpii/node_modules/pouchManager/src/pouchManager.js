 ...
 * 1. A pouchDB express server allows GPII to integrate with CouchDB-like PouchDB. This express server makes use of gpii-pouchdb repo: https://github.com/gpii/gpii-pouchdb/
 * 2. An express server that provides a /reset-pouch API to restore the pouchDB in 1 above with its initial data set.
 * Note that the reset uses a separate express server because it needs to re-instantiate the entire pouchDB express server and the re-instatiation would interrupt the request
 * to send back the response. The reset cannot simply re-instantiate the pouch-express middleware within the pouchDB express server because GPII express server has its own way
 * to wire up children middleware so it doesn't make sense to programmatically control the creation/recreation of a middleware.
 *
