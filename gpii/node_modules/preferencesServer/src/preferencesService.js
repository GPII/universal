...
(function () {
    ...
    fluid.defaults("gpii.preferencesServer.preferencesService", {
        ...
        listeners: {
            ...
            onCloudSafeCredCreate: [{
                ...
                args: ["{arguments}.0", "prefsSafe", "The prefs safe does not exist"]
            }, {
    ...
    gpii.preferencesServer.preferencesService.getPrefsSafe = function (dataStore, prefsSafeId) {
        ...
        dataStore.findById(prefsSafeId).then(
            ...
            function (err) {
                ...
                promiseTogo.reject({
                    isError: true,
                    message: "Error fetching prefsSafe",
                    statusCode: 500
                });
            }
        );
     ...
     * minus the `id`, `timestampCreated`, and `timestampUpdated` properties.
     *
    ...
    gpii.preferencesServer.preferencesService.addPrefsSafe = function (that, dataStore, prefsSafeData) {
        ...
        if (prefsSafeData.type !== "prefsSafe") {
            promiseTogo.reject({
                isError: true,
                message: "Payload needs to have a type of `prefsSafe`."
            });
        }
        else {
    ...
    gpii.preferencesServer.preferencesService.updatePrefsSafe = function (that, dataStore, prefsSafeData) {
        ...
        if (!prefsSafeData.id || prefsSafeData.type !== "prefsSafe") {
            promiseTogo.reject({
                isError: true,
                message: "Payload needs to have an id and type of `prefsSafe`."
            });
        }
        else {
    ...
    gpii.preferencesServer.preferencesService.addGpiiKey = function (that, dataStore, keyData) {
        ...
        if (!keyData.prefsSafeId || !keyData.prefsSetId) {
            promTogo.reject({
                isError: true,
                message: "Unable to add GPII Key to Preferences Safe. `prefsSafeId` and `prefsSetId` are required fields."
            });
        }
        else {
            dataStore.addGpiiKey({
                ...
                function (error) {
                    fluid.error("Error adding key to preferences safe: ", keyData, error);
                    promTogo.reject({
                        isError: true,
