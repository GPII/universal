...
(function () {
    ...
    fluid.defaults("gpii.preferencesServer.preferencesService", {
        ...
        invokers: {
            ...
            updatePreferences: {
                ...
                // preferences, gpiiKey, toCreatePrefsSafe
            }
        },
        events: {
     ...
     * }
     * @param gpiiKey {String} Optional. The new GPII key, an UUID will be generated if it's not provided
     */
    gpii.preferencesServer.preferencesService.createPreferences = function (that, prefsSafeData, gpiiKey) {
        return fluid.promise.fireTransformEvent(that.events.onCreatePreferences, {
            gpiiKey: gpiiKey,
            prefsSafeData: prefsSafeData
        });
    };
     ...
     * @param findPrefsSafeByGpiiKey {Function} The findPrefsSafeByGpiiKey() API provided by preferencesServer.preferencesService
     * @param input {Object} The data passed on from the entry function gpii.preferencesServer.preferencesService.createPreferences(). Its structure:
     *  {
     ...
     */
    gpii.preferencesServer.preferencesService.verifyGpiiKey = function (findPrefsSafeByGpiiKey, input) {
        var promiseTogo = fluid.promise();
        var gpiiKey = input.gpiiKey;

        if (gpiiKey) {
            // verify the uniqueness of the provided GPII key
            var gpiiKeyPromise = findPrefsSafeByGpiiKey(gpiiKey);
     ...
     * @param that {Component} An instance of gpii.preferencesServer.preferencesService
     * @param preferences {Object} The preferences to be updated
     * @param gpiiKey {String} Optional. The new GPII key, an UUID will be generated if it's not provided
     * @return {Promise} A promise object whose resolved value is:
     *  {
     *      gpiiKey: {String},
     ...
     *  }
     */
    gpii.preferencesServer.preferencesService.updatePreferences = function (that, preferences, gpiiKey) {
        var promiseTogo = fluid.promise();
        ...
        gpiiKeyPromise.then(function (data) {
            ...
            if (prefsSafeId) {
                // merge with the existing preferences and update the associated prefs safe
                var prefsSafeData = fluid.extend({}, data.prefsSafe, {preferences: preferences});
                var updatePrefsSafePromise = that.dataStore.updatePrefsSafe(prefsSafeId, prefsSafeData);
                var mapper = function (updatedPrefsSafe) {
                    return {
                        gpiiKey: gpiiKey,
                        preferences: updatedPrefsSafe.preferences
                    };
                };
     ...
     * @param updateGpiiKey {Function} The updateGpiiKey() API provided by gpii.oauth2.dbDataStore
     * @param input {Object} The data passed on from the previous step. Its structure:
     *  {
     *      gpiiKey: {String},
     *      prefsSafeId: {String},
    ...
    gpii.preferencesServer.preferencesService.updateGpiiKey = function (updateGpiiKey, input) {
        ...
        fluid.set(gpiiKeyRec, ["prefsSetName"], gpii.preferencesServer.defaultPrefsSetName);
