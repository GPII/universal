...
(function () {
    ...
    fluid.defaults("gpii.preferencesServer.preferencesService", {
        ...
        invokers: {
            // TODO check type == prefsSafe
            getPrefsSafe: "{dataStore}.findById",
            getPrefsSafeList: {
                func: "{dataStore}.findPrefsSafeList",
                args: ["{arguments}.0"]
            },
            getPrefsSafeByGpiiKey: "{dataStore}.findPrefsSafeByGpiiKey",
            getKeysForPrefsSafe: "{dataStore}.findKeysForPrefsSafe",
    ...
    /**
     * A sequence transform pipeline inspired by `fluid.promise.sequence` and
     * `fluid.promise.fireTransformEvent`.
     *
     * https://issues.fluidproject.org/browse/FLUID-6342
     *
     * @param  {Array} promArray An array of functions that return promises. Each function
     * accepts one argument, which is the result from the last payload.
     * @return {Promise}         Promise resolving with the payload from the last
     * promise in the chain. If any of the promises in the `promArray` reject, then the
     * pipeline will stop, and the returned promise will reject with the error.
     */
    gpii.preferencesServer.preferencesService.sequenceTransform = function (promArray) {
        var finalProm = fluid.promise();

        var currentPromIdx = 0;

        var resolveFunc = function (data) {
            currentPromIdx++;
            if (currentPromIdx === promArray.length) {
                finalProm.resolve(data);
            }
            else {
                promArray[currentPromIdx](data).then(resolveFunc, rejectFunc);
            }
        };

        var rejectFunc = finalProm.reject;

        promArray[currentPromIdx]().then(resolveFunc, rejectFunc);
    ...
    gpii.preferencesServer.preferencesService.cloudSafeCredCreate = function (dbDataStore, userUtils, options) {
        var promArray = [
            // #1 This merely ensures that a prefssafe exists with this id, and if it
            // fails this will be rejected.
            function () {
                return dbDataStore.findById(options.prefsSafeId);
            },

            // #2 Create the gpii-express-user entry
            function () {
                return userUtils.createNewUser({
                    username: options.username,
                    // TODO, if this password string isn't recreated, gpii-express-user errors out
                    // during it's hashing/encryption functions saying that it must be a Buffer
            ...
            function (userRecord) {
                ...
                return gpii.dbOperation.dbDataStore.addRecord(dbDataStore.saveDataSource,
                    "gpiiCloudSafeCredential", "id", cloudCredDoc);
            }
        ];

        return gpii.preferencesServer.preferencesService.sequenceTransform(promArray);
    };
    ...
    gpii.preferencesServer.preferencesService.cloudSafeUnlock = function (dbDataStore, userUtils, options) {
        ...
        var promArray = [
            ...
            // #2 Look up Cloud Credentials
            function (userRecord) {
                return dbDataStore.findSafeByExpressUserLookup(userRecord._id);
            },

            // #3 Fetch and Return the Safe
            function (lookupData) {
                return dbDataStore.findById(lookupData.prefsSafeId);
            }
        ];

        gpii.preferencesServer.preferencesService.sequenceTransform(promArray).then(
            promiseTogo.resolve,
            function (err) {
                fluid.log(err);
                promiseTogo.reject({
