...
gpii.preferencesServer.ready.handler.reportError = function (request, reason) {
    request.events.onError.fire({
        isError: true,
        message: "Problem with the database connection; " + reason,
        statusCode: 503
    });
};
...
gpii.preferencesServer.ready.handler.getPreferences = function (preferencesServer, request) {
    ...
    isLivePromise.then(function (value) {   // onResolve
        if (value.isReady) {
            ...
            gpii.preferencesServer.ready.handler.reportError(request, value.reason);
        }
