 ...
 * 1. If the GPII key is not provided, create an auto generated GPII key and a new prefs safe with the given preferences. Also associates the prefs safe with the GPII key;
 * 1. If the GPII key is provided and not yet exists, create it and also create a new prefs safe with the given preferences to associate with this key;
 * 1. If the GPII key is provided and already exists, reject with an error;
 ...
 * @param gpiiKey {String} Optional. The GPII key to be created
 ...
 * }
 */
gpii.preferencesServer.createPreferences = function (preferencesService, ontologyHandler, preferences, toView, gpiiKey) {
    fluid.log("gpii.preferencesServer.createPreferences called");
    ...
    if (!preferences) {
        ...
        promiseTogo = preferencesService.createPreferences(payload, gpiiKey);
    }
    return promiseTogo;
};
    ...
    if (!gpiiKey) {
        ...
        gpiiKeyPromise.then(function (gpiiKeyRelatedData) {
            ...
            if (gpiiKeyRelatedData) {
                ...
                var updatePromise = preferencesService.updatePreferences(payload, gpiiKey);

                var mapper = function (updateResponse) {
                    return {
                        gpiiKey: updateResponse.gpiiKey,
                        preferences: fluid.get(updateResponse, ["preferences", toView])
                    };
                };
                preferencesPromise = fluid.promise.map(updatePromise, mapper);
            } else {
                // Reject if the GPII key does not exist
                fluid.log("Preferences Server, updatePreferences(), updating preferences, rejected due to the unfound GPII key: " + gpiiKey);
                preferencesPromise.reject({
                    "message": fluid.stringTemplate(gpii.preferencesServer.errors.unfoundGpiiKey, {gpiiKey: gpiiKey})
                });
            }
