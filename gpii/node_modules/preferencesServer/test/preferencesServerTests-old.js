    ...
    components: {
        backingRawPreferencesDataSource: { // not "rawPreferencesDataSource" to avoid runaway like https://issues.fluidproject.org/browse/FLUID-5305
            ...
            options: {
                ...
                path: "%preferencesServer/test/data/%gpiiKey.json",
                termMap: {
                    gpiiKey: "%gpiiKey"
                },
                writable: true
...
gpii.tests.preferencesServer.testServerMock.set = function (model, body, callback, rawPreferencesDataSource) {
    ...
    var gpiiKey = model.gpiiKey;
    if (gpiiKey === "" || gpiiKey === undefined) {
        gpiiKey = uuid.v4();
    }

    gpii.tests.preferencesServer.filesToDelete.push(__dirname + "/data/" + gpiiKey + ".json");

    var promise = fluid.promise();
    rawPreferencesDataSource.set({gpiiKey: gpiiKey}, body, callback).then(function () {
        promise.resolve({ gpiiKey: gpiiKey, preferences: body});
    });
    return promise;
};
...
gpii.tests.preferencesServer.testServerMock.get = function (model, body, callback, rawPreferencesDataSource) {
    ...
    rawPreferencesDataSource.get(model, body, callback).then(function (data) {
        promise.resolve({ gpiiKey: model.gpiiKey, preferences: data});
    }, function (err) {
...
gpii.tests.preferencesServer.put.testRequestResponse = function (prefs, gpiiKey) {
    return function (data) {
        var retrievedData = JSON.parse(data);
        // expect preferences without ontology info - any generated GPII key is fine
        var expected = {
...
gpii.tests.preferencesServer.put.buildTestDef = function (fixture) {
    return {
        ...
        sequence: [{
            ...
            args: [ fixture.gpiiKey, fixture.copyFromGpiiKey ]
        }, {
            ...
            makerArgs: [ fixture.putBody, fixture.gpiiKey ]
        }, {
            func: "gpii.tests.preferencesServer.put.checkRawPreferences",
            args: [ fixture.gpiiKey, fixture.expectedRaw ]
        }]
    };
};

gpii.tests.preferencesServer.put.copyNPSet = function (toGpiiKey, fromGpiiKey) {
    if (!toGpiiKey) {
        return;
    }
    if (fromGpiiKey === undefined) { // if fromGpiiKey is undefined, it means we dont want to copy anything
        return;
    }
    var rawPrefs = fs.readFileSync(__dirname + "/data/" + fromGpiiKey + ".json");
    fs.writeFileSync(__dirname + "/data/" + toGpiiKey + ".json", rawPrefs);
};

gpii.tests.preferencesServer.put.checkRawPreferences = function (gpiiKey, expectedRaw) {
    var rawPrefs;
    // var resolved = fluid.stringTemplate(that.options.ontologySourceList, { root: that.options.root });
    try {
        rawPrefs = fs.readFileSync(__dirname + "/data/" + gpiiKey + ".json");
    } catch (e) {
...
gpii.tests.preferencesServer.put.fixtures = [
    {
        ...
        gpiiKey: "put_test1",
        copyFromGpiiKey: "np_flatOnly_singleContext",
        view: undefined,
        putBody: {
            "contexts": {
                "gpii-default": {
        ...
        gpiiKey: "put_test3",
        copyFromGpiiKey: "np_mixed_singleContext",
        view: "ISO24751",
        putBody: {
            "contexts": {
                "gpii-default": {
