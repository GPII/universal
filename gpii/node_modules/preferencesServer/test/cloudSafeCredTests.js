...
/*
 * Make sure we got a non-error response from the original create call.
 */
gpii.tests.preferencesServer.cloudSafeCred.testCloudSafeCreatePut = function (response /*, expected, receivedStatusCode, expectedStatusCode */) {
    var data = JSON.parse(response);
    jqUnit.assertEquals("Should be ok/true", data.ok, true);
    jqUnit.assertNotUndefined("Should have an id", data.id);
    jqUnit.assertNotUndefined("Should have a revision", data.rev);
};
...
gpii.tests.preferencesServer.cloudSafeCred.testCloudSafeCred = function (response /*, expected, receivedStatusCode, expectedStatusCode */) {
    ...
    jqUnit.assertEquals("There should be a cloud cred", data.keys[0].type, "gpiiCloudSafeCredential");
    jqUnit.assertEquals("There should be a gpii key", data.keys[1].type, "gpiiKey");
};
...
gpii.tests.preferencesServer.cloudSafeCred.put.buildSuccessTestDef = function (fixture) {
    ...
    return {
        ...
        sequence: [{
            ...
            args: ["{arguments}.0", fixture.putCloudCredBodyExpected, "{putCloudCred}.nativeResponse.statusCode", expectedStatusCode]
        }, {
                        ...
                        "gpii-default": {
                            ...
                            "preferences": {
                                "control": {
                                    ...
                                    "mouseEmulation": {
                                        "-provisional-initDelay": 0.120,
                                        "cursorSpeed": 0.850,
                                        "cursorAcceleration": 0.800,
...
gpii.tests.preferencesServer.cloudSafeCred.put.failureFixtures = [
    {
        name: "PUT FAIL: Cannot add a cloudSafe credential to a safe that doesn't exist",
    ...
    {
        name: "PUT FAIL: Cannot add a cloudSafe credential with an existing name",
        expectedStatusCode: 404,
        prefsSafeId: "prefsSafe-1",
        url: "%prefsSafeId",
        putCloudCredBody: {
...
gpii.test.bootstrapServer(gpii.tests.preferencesServer.cloudSafeCred.testDefs);
