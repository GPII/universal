...
gpii.tests.preferencesServer.get.failFixtures = [
    {
        ...
        name: "GET: Not providing GPII key returns an error",
        expectedStatusCode: 500,
        expected: {
            isError: true,
            message: "GPII key \"undefined\" does not exist"
        }
    }
];
...
gpii.tests.preferencesServer.ready.testDefs = gpii.tests.preferencesServer.serverStatus.testDefs;

// TESTING preferencesServer /health endpoint
fluid.registerNamespace("gpii.tests.preferencesServer.health");

gpii.tests.preferencesServer.health.testDefsWithDbConnection = fluid.copy(gpii.tests.preferencesServer.serverStatus.testDefs);

fluid.transform(gpii.tests.preferencesServer.health.testDefsWithDbConnection, function (oneTestDef) {
...
gpii.tests.preferencesServer.health.testDefsWithoutDbConnection = fluid.copy(gpii.tests.preferencesServer.health.testDefsWithDbConnection);

fluid.transform(gpii.tests.preferencesServer.health.testDefsWithoutDbConnection, function (oneTestDef) {
    return fluid.extend(true, oneTestDef, {
...
gpii.test.bootstrapServer(gpii.tests.preferencesServer.ready.testDefs);

// 2. Without a database connection: readiness === true
kettle.test.bootstrapServer(gpii.tests.preferencesServer.ready.testDefs);

// Run tests for /health endpoint:
// 1. With a proper database connection: liveness === true
gpii.test.bootstrapServer(gpii.tests.preferencesServer.health.testDefsWithDbConnection);

// 2. Without a database connection: liveness === false
kettle.test.bootstrapServer(gpii.tests.preferencesServer.health.testDefsWithoutDbConnection);
