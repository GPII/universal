...
gpii.ontologyServer.tests.prefsDir = path.resolve(__dirname, "../../../../testData/preferences/");

gpii.ontologyServer.tests.filesToDelete = [];

gpii.ontologyServer.tests.cleanUpTmpFiles = function () {
    var file;
    while (file = gpii.ontologyServer.tests.filesToDelete.pop()) {
        fs.unlinkSync(file);
    }
};

fluid.defaults("gpii.ontologyServer.tests.testServerMock", {
    gradeNames: ["fluid.littleComponent", "autoInit"],
    invokers: {
        set: {
            funcName: "gpii.ontologyServer.tests.testServerMock.set",
            args: [ "{arguments}.0", "{arguments}.1", "{arguments}.2", "{that}.rawPreferencesDataSource"]
        },
        get: {
            funcName: "gpii.ontologyServer.tests.testServerMock.get",
            args: [ "{arguments}.0", "{arguments}.1", "{arguments}.2", "{that}.rawPreferencesDataSource"]
        }
    },
    components: {
...
gpii.ontologyServer.tests.testServerMock.set = function (model, body, callback, rawPreferencesDataSource) {
...
gpii.ontologyServer.tests.testServerMock.get = function (model, body, callback, rawPreferencesDataSource) {
    rawPreferencesDataSource.get(model, body, callback);
};


gpii.ontologyServer.tests.testGETResponseMaker = function (expected) {
...
gpii.ontologyServer.tests.testPOSTResponseMaker = function (prefs) {
    return function (data) {
        var retrievedData = JSON.parse(data);
...
gpii.ontologyServer.tests.testPUTResponseMaker = function (prefs, token) {
    return function (data) {
        var retrievedData = JSON.parse(data);
        //expect preferences without ontology info - any generated token is fine
        var expected = {
...
gpii.ontologyServer.tests.buildGETTestDef = function (fixture) {
    return {
        name: fixture.name,
        expect: 1,
        config: {
        ...
        sequence: [{
            ...
            listenerMaker: "gpii.ontologyServer.tests.testGETResponseMaker",
...
fluid.each(getFixtures, function (fixture) {
    testDefs.push(gpii.ontologyServer.tests.buildGETTestDef(fixture));
});


gpii.ontologyServer.tests.buildPOSTTestDef = function (fixture) {
    return {
        name: fixture.name,
        expect: 2,
        config: {
        ...
        sequence: [{
            ...
            listenerMaker: "gpii.ontologyServer.tests.testPOSTResponseMaker",
...
fluid.each(postFixtures, function (fixture) {
    testDefs.push(gpii.ontologyServer.tests.buildPOSTTestDef(fixture));
});


gpii.ontologyServer.tests.buildPUTTestDef = function (fixture) {
    return {
        name: fixture.name,
        expect: 3,
        config: {
        ...
        listeners: {
            onDestroy: gpii.ontologyServer.tests.cleanUpTmpFiles
        },
        components: {
            putRequest: {
                type: "kettle.tests.request.http",
        ...
        sequence: [{
            func: "gpii.ontologyServer.tests.copyNPSet",
            ...
            listenerMaker: "gpii.ontologyServer.tests.testPUTResponseMaker",
            makerArgs: [ fixture.putBody, fixture.token ]
        }, {
            func: "gpii.ontologyServer.tests.checkRawPreferences",
            args: [ fixture.token, fixture.expectedRaw ]
        }]
    };
};

gpii.ontologyServer.tests.copyNPSet = function (toToken, fromToken) {
    ...
    gpii.ontologyServer.tests.filesToDelete.push(__dirname+"/data/"+toToken+".json");
};

gpii.ontologyServer.tests.checkRawPreferences = function (token, expectedRaw) {
    //var resolved = fluid.stringTemplate(that.options.ontologySourceList, { root: that.options.root });
...
fluid.each(putFixtures, function (fixture) {
    testDefs.push(gpii.ontologyServer.tests.buildPUTTestDef(fixture));
});
