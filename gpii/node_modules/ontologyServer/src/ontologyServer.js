...
(function () {
    ...
    fluid.defaults("gpii.ontologyServer.development", {
        ...
        invokers: {
            set: {
                ...
                args: ["{ontologySource}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
            }
        }
    });
    ...
    gpii.ontologyServer.doSuppliedTransform = function (preferences, transform) {
        fluid.log("applying transform ", transform);
        var transformed = fluid.model.transformWithRules(preferences,
                gpii.ontologyServer.expandTransformations(transform.transformations));
        return transformed;
    };

    gpii.ontologyServer.development.set = function (ontologySource, directModel, model, callback) {
        ...
        var applyTransformations = function (data) {
            var transformed = fluid.model.transformWithRules(preferences,
                gpii.ontologyServer.expandTransformations(data[source.transformation].transformations));
            callback(transformed);
        };
        // TODO: wrapping should be unnecessary per impl. of promiseCallbackWrapper
        // callback = callbackWrapper.wrap(callback);
        ontologySource.get(null).then(applyTransformations);
    };
