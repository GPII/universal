...
(function () {
    ...
    fluid.defaults("gpii.ontologyServer", {
        ...
        components: {
            ...
            ontologySource: {
                type: "gpii.callbackWrappingPromiseDataSource"
            }
        }
    });
    ...
    gpii.ontologyServer.development.preInit = function (that) {
        that.set = function (directModel, model, callback) {
            var preferences = model.preferences,
                ontologyName = gpii.ontologyServer.getOntologyName(preferences);
            if (ontologyName === "ISO24751-2") {
                callback(preferences);
                return;
            }
            callback = that.callbackWrapper.wrap(callback);
            that.ontologySource.get(null).then(function (data) {
                var transformationPair = ["ISO24751-2", ontologyName].join("/");
                preferences = fluid.model.transformWithRules(preferences,
                    gpii.ontologyServer.expandTransformations(data[transformationPair].transformations));
                callback(preferences);
            });
        };
    };
