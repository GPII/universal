...
(function () {
    ...
    fluid.defaults("kettle.requests.request.handler.ontologyPut", {
        ...
        invokers: {
            handle: {
                ...
                args: [ "{request}", "{ontologyServer}", "{that}.events.onGetRawPreferences"],
                dynamic: true
            },
            onGetRawPreferences: {
                funcName: "gpii.ontology.put.onGetRawPreferences",
                args: [ "{arguments}.0", "{request}", "{ontologyServer}.ontologySource",
                    "{ontologyServer}", "{that}.events.onPutRawPreferences" ]
            },
            onPutRawPreferences: {
    ...
    gpii.ontology.put.request = function (request, ontologyServer, event) {
    ...
    gpii.ontology.put.onGetRawPreferences = function (rawPrefs, request, ontologySource, ontologyServer, event) {
        ...
        gpii.ontologyServer.evaluateOntologyTransforms(transformSpecs, ontologyServer.options.ontologies, ontologySource, function (transforms) {
            /*
            ...
            */
            ...
            fluid.each(transforms, function (data) {
                ...
                var filteredPrefs = gpii.ontologyServer.utils.filterPrefs(togo[data.to], transformed);
                togo[data.to] = filteredPrefs;
            });

            ontologyServer.setRawPreferences(event, togo, token, "PUT");
        });
    };
