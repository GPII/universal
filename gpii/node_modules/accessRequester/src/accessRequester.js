...
fluid.defaults("gpii.accessRequester", {
    ...
    // The API endpoint provided by the authorization server to request access tokens via Resource Owner GPII Token Grant
    url: "/access_token",    // Must be provided by integrators
        ...
        accessTokenDataSource: {
            ...
            options: {
                ...
                dataSourceModel: {
                    ...
                    username: "{accessRequester}.options.gpiiToken",
                    password: "dummy"
                }
            }
        }
    ...
    invokers: {
        getAccessToken: {
            ...
            args: ["{that}.clientCredentialDataSource", "{that}.accessTokenDataSource"]
        }
    }
});

gpii.accessRequester.getAccessToken = function (clientCredentialDataSource, accessTokenDataSource) {
    var promiseTogo = fluid.promise();
    console.log(clientCredentialDataSource);
    var clientCredentialPromise = clientCredentialDataSource.get();
    clientCredentialPromise.then(function (clientCredential) {
        var accessTokenRequestParams = fluid.stringTemplate(accessTokenDataSource.options.dataSourceModel, clientCredential);
        promiseTogo = accessTokenDataSource.set(null, accessTokenRequestParams);
    });
    return promiseTogo;
};
