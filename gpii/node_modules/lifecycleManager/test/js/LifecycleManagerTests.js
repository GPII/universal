...
(function () {
    ...
    gpii.tests.lifecycleManager.mockSettingsHandler = {
        set: function (data) {
            ...
            gpii.invokeLater(function () {
                ...
                togo.resolve(parseHandlerResponseFunctionRequest);
            }, 100);
            return togo;
        }
    };
    ...
    var nameResolver = gpii.lifecycleManager.nameResolver();
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.invokeSettingsHandlers()", function () {
            jqUnit.expect(2);
            var snapshotPromise = gpii.lifecycleManager.invokeSettingsHandlers("org.gnome.desktop.a11y.magnifier", invokeSettingsHandlersRequest, nameResolver);
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.start() and stop()", function () {
            ...
            lifecycleManager.start(gpii.tests.lifecycleManager.userOptions, configurationSpec, function () {
        ...
        jqUnit.test("gpii.lifecycleManager.update() without active session", function () {
            try {
                ...
                lifecycleManager.update(gpii.tests.lifecycleManager.userOptions, configurationSpec);
            } catch (e) {
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.update() with active user session", function () {
            jqUnit.expect(2);

            var lifecycleManager = gpii.lifecycleManager(gpii.tests.lifecycleManager.testOptions);

            fluid.set(lifecycleManager, "activeSessions", gpii.tests.lifecycleManager.sampleActiveSession);
            lifecycleManager.start(gpii.tests.lifecycleManager.userOptions, configurationSpec, function (success) {
                jqUnit.assertTrue("gpii.lifecycleManager.start() succeeds", success);
                
                lifecycleManager.update(gpii.tests.lifecycleManager.userOptions, configurationSpec, function () {
                    var sessionSolutions = lifecycleManager.activeSessions[gpii.tests.lifecycleManager.userOptions.userToken].solutions;
                    jqUnit.assertNotUndefined("Update method succeeds with ongoing session", sessionSolutions);
                    jqUnit.start();
                });
