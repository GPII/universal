...
(function ($) {
    ...
    gpii.tests.lifecycleManager.buildLifecycleInstructions = function (solId) {
        ...
        for (var i = 1; i < arguments.length; ++i) {
            togo[solId] = $.extend(true, {}, togo[solId], arguments[i]);
        }
    ...
    gpii.tests.lifecycleManager.startPayload = $.extend(true, {}, gpii.tests.lifecycleManager.userOptions, {
        lifecycleInstructions: gpii.tests.lifecycleManager.configurationSpec
    });

    gpii.tests.lifecycleManager.sampleSession = {
        123: {
            userToken: 123
        }
    };
    ...
    gpii.tests.lifecycleManager.updateTestDefs = [{
        name: "Updating with the same prefs as already applied",
        session: gpii.tests.lifecycleManager.sampleSession,
        ...
        name: "single pref changed without 'update' directive",
        session: gpii.tests.lifecycleManager.sampleSession,
        startPayload: $.extend(true, {}, gpii.tests.lifecycleManager.userOptions, {
        ...
        startPayload: gpii.tests.lifecycleManager.startPayload,
        updateSpec: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
            gpii.tests.lifecycleManager.buildSettingsHandlersEntry({ "cross-hairs-color": "green", "iamasetting": true }),
            gpii.tests.lifecycleManager.noUpdateLifecycle,
            { "update": [ "settings.myconf" ] }),
        expectedAppliedSolutions: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
            gpii.tests.lifecycleManager.buildSettingsHandlersEntry({ "cross-hairs-clip": true, "cross-hairs-color": "green", "iamasetting": true }),
            gpii.tests.lifecycleManager.noUpdateLifecycle,
            { "update": [ "settings.myconf" ] }),
        expectedOriginalSettings: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
            gpii.tests.lifecycleManager.buildSettingsHandlersEntry({ "cross-hairs-clip": false, "cross-hairs-color": "red" }),
        ...
        fluid.each(gpii.tests.lifecycleManager.updateTestDefs, function (test) {
            jqUnit.asyncTest("gpii.lifecycleManager.update() with active user session: " + test.name, function () {
                ...
                lifecycleManager.start(test.startPayload, function (success) {
                    ...
                    var updatePayload = $.extend(true, {}, test.startPayload, {
                    ...
                    lifecycleManager.update(updatePayload, function () {
                        var appliedSolutions = lifecycleManager.session[test.startPayload.userToken].appliedSolutions;
                        var originalSettings = lifecycleManager.session[test.startPayload.userToken].originalSettings;
                        ...
                        lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                            jqUnit.assertUndefined("no active sessions running after stop", lifecycleManager.session[test.startPayload.userToken]);
                            jqUnit.start();
                        });
                    });
                });
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.start() and stop()", function () {
            ...
            lifecycleManager.start(gpii.tests.lifecycleManager.startPayload, function () {
                ...
                lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                    ...
                    jqUnit.assertDeepEq("stop: Stop message to lifecycle manager, empty session state", {}, lifecycleManager.session);
                    jqUnit.start();
                });
            });
        });
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.update() tests for 'stop' reference", function () {
            // initial payload:
            var startPayload = $.extend(true, {}, gpii.tests.lifecycleManager.userOptions, {
                lifecycleInstructions: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
                    gpii.tests.lifecycleManager.buildSettingsHandlersEntry({ "cross-hairs-clip": true, "cross-hairs-color": "red" }),
                    gpii.tests.lifecycleManager.noUpdateLifecycle,
                    { "update": [ "stop" ] })
            ...
            lifecycleManager.start(startPayload, function (success) {
                ...
                lifecycleManager.update(startPayload, function () {
                    ...
                    jqUnit.assertEquals("Expected pid has been sent to kill handler",
                        8839, gpii.tests.lifecycleManager.staticRepository.killHandler.pid);
                    lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                        jqUnit.assertUndefined("no active sessions running after stop", lifecycleManager.session[startPayload.userToken]);
                        jqUnit.start();
                    });
                });
            });
