...
(function ($) {
                ...
                lifecycleManager.start(test.startPayload, function (success) {
                    ...
                    lifecycleManager.update(updatePayload, function () {
                        ...
                        lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                            jqUnit.assertUndefined(lifecycleManager.activeSessions[test.startPayload.userToken]);
                            jqUnit.start();
                        });
    ...
    gpii.tests.lifecycleManager.buildLoginSemanticsTests = function () {
        jqUnit.asyncTest("Multiple logins without logging out in between", function () {
            jqUnit.expect(9);

            var lifecycleManager = gpii.lifecycleManager(gpii.tests.lifecycleManager.testOptions);
            gpii.tests.lifecycleManager.initBackingMock();

            lifecycleManager.start(gpii.tests.lifecycleManager.startPayload, function (success) {
                jqUnit.assertTrue("gpii.lifecycleManager.start() succeeds", success);
                gpii.tests.lifecycleManager.assertExpectedExec();
                gpii.tests.lifecycleManager.assertExpectedSettingsHandler(" - on start",
                    gpii.tests.lifecycleManager.settingsHandlerExpectedInputNewSettings);

                var newSettings = [{
                    "settings": {
                        "cross-hairs-clip": true,
                        "cross-hairs-color": "blue"
                    },
                    "options": {}
                }];
                var newStartPayload = $.extend(true, {}, gpii.tests.lifecycleManager.startPayload,
                    {
                        userToken: 789,
                        lifecycleInstructions: {
                            "org.gnome.desktop.a11y.magnifier": {
                                "settingsHandlers": newSettings
                            }
                        }
                    });
                lifecycleManager.start(newStartPayload, function (success) {
                    jqUnit.assertTrue("gpii.lifecycleManager.start() succeeds", success);
                    gpii.tests.lifecycleManager.assertExpectedExec();

                    jqUnit.assertDeepEq("Checking input sent to settingshandlers after second login",
                        { "org.gnome.desktop.a11y.magnifier": newSettings },
                        gpii.tests.lifecycleManager.staticRepository.settingsHandler);

                    lifecycleManager.stop({ userToken: 789 }, function () {
                        gpii.tests.lifecycleManager.assertExpectedSettingsHandler(" - on stop",
                            gpii.tests.lifecycleManager.settingsHandlerExpectedInputRestoreSettings);
                        ...
                        jqUnit.assertDeepEq("stop: Stop message to lifecycle manager, empty session state",
                            {}, lifecycleManager.activeSessions);
                        jqUnit.start();
                    });
                });
            });
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        gpii.tests.lifecycleManager.buildLoginSemanticsTests();
