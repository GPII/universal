...
(function () {
     ...
     * @arguments {..Object..} The remaining arguments should be valid lifecycleInstructions that will
    ...
    gpii.tests.lifecycleManager.buildSettingsHandlersEntry = function (settings, setRunning) {
        ...
        if (settings !== undefined) {
            fluid.set(togo, [ "handlers", "myconf", "settings" ], settings);
        }
        if (setRunning !== undefined) {
            fluid.set(togo, [ "handlers", "launcher", "settings" ], { running: setRunning });
        }
        return togo;
    };
    ...
    gpii.tests.lifecycleManager.runAsyncLifecycleTest = function (testDef) {
        ...
        jqUnit.asyncTest(testDef.name, function () {
            ...
            lifecycleManager.start(gpii.tests.lifecycleManager.buildStartPayload(testDef.loginPayload)).then(function (success) {
                session = lifecycleManager.getSession(123); // always just use the default user token
                jqUnit.assertTrue("gpii.lifecycleManager.start() succeeds", success);
                jqUnit.assertDeepEq("Checking appliedSolutions after start", testDef.expectedAfterLogin, session.model.appliedSolutions);
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.test("gpii.lifecycleManager.update() without active session", function () {
            ...
            jqUnit.expect(1);
            ret.then(function () {
                jqUnit.assertTrue("User with token 123 has no active session, so ignoring update request", true);
            });
        });
