...
(function () {
    ...
    gpii.tests.lifecycleManager.trueConfigurableDirective = {
        "isConfigurable": [
            {
                "type": "gpii.tests.lifecycleManager.mockIsConfigurable",
                "toReturn": true
            }
        ]
    };

    gpii.tests.lifecycleManager.falseConfigurableDirective = {
        "isConfigurable": [
            {
                "type": "gpii.tests.lifecycleManager.mockIsConfigurable",
                "toReturn": false
    ...
    gpii.tests.lifecycleManager.makeConfigurableDirective = {
        "makeConfigurable": [
            {
                "type": "gpii.tests.lifecycleManager.mockMakeConfigurable",
                "options": {
                    "filename": "${{environment}.JAWS_DIR}configuration.json"
                }
            }
        ]
    };
    ...
    gpii.tests.lifecycleManager.mockIsConfigurable = function (toReturn) {
        jqUnit.assertTrue("isConfigurable has been called", true);
        return toReturn;
    };

    fluid.defaults("gpii.tests.lifecycleManager.mockIsConfigurable", {
        gradeNames: "fluid.function",
        argumentMap: {
            toReturn: 0
        }
    ...
    gpii.tests.lifecycleManager.mockMakeConfigurable = function (options) {
        gpii.tests.lifecycleManager.staticRepository.configurableStash = {
            options: options
        };
    };

    fluid.defaults("gpii.tests.lifecycleManager.mockMakeConfigurable", {
        gradeNames: "fluid.function",
        argumentMap: {
            options: 0
        }
    ...
    gpii.tests.lifecycleManager.buildUpdateTests = function () {
        fluid.each(gpii.tests.lifecycleManager.updateTestDefs, function (test) {
            jqUnit.asyncTest("gpii.lifecycleManager.update() with active user session: " + test.name, function () {
                ...
                lifecycleManager.start(test.startPayload, function (success) {
                    ...
                    lifecycleManager.update(updatePayload, function () {
                        var appliedSolutions = lifecycleManager.activeSessions[test.startPayload.userToken].appliedSolutions;
                        var originalSettings = lifecycleManager.activeSessions[test.startPayload.userToken].originalSettings;
                        jqUnit.assertDeepEq("Checking appliedSolutions after Update", test.expectedAppliedSolutions, appliedSolutions);
                        jqUnit.assertDeepEq("Checking originalSettings after Update", test.expectedOriginalSettings, originalSettings);
                        lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                            jqUnit.assertUndefined("no active sessions running after stop", lifecycleManager.activeSessions[test.startPayload.userToken]);
                            jqUnit.start();
                        });
                    });
                });
    ...
    gpii.tests.lifecycleManager.configurabilityTestDefs = [{
        name: "configurability test with a configurable solution",
        expect: 5,
        startPayload: fluid.extend(true, {}, gpii.tests.lifecycleManager.userOptions,
            {
                lifecycleInstructions: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
        ...
        stashContent: undefined
    }, {
        name: "configurability test with a non-configurable solution to be made configurable",
        expect: 5,
        startPayload: fluid.extend(true, {}, gpii.tests.lifecycleManager.userOptions,
            {
                lifecycleInstructions: gpii.tests.lifecycleManager.buildLifecycleInstructions("org.gnome.desktop.a11y.magnifier",
        ...
        stashContent: {
            "options": {
                "filename": "e:\\Programs and Things\\Jaws\\configuration.json"
            }
        }
    }];

    // Tests focusing on the isConfigurable and makeConfigurable directives of the solution
    gpii.tests.lifecycleManager.buildConfigurabilityTests = function () {
        fluid.each(gpii.tests.lifecycleManager.configurabilityTestDefs, function (test) {
            jqUnit.asyncTest("gpii.lifecycleManager configurability tests: " + test.name, function () {
                gpii.tests.lifecycleManager.setup();
                jqUnit.expect(test.expect);
                ...
                lifecycleManager.start(test.startPayload, function (success) {
                    ...
                    var expectedFirstAppliedSettings = (test.expectedFirstAppliedSettings !== undefined) ? test.expectedFirstAppliedSettings : gpii.tests.lifecycleManager.settingsHandlerExpectedInputNewSettings;
                    gpii.tests.lifecycleManager.assertExpectedSettingsHandler(" - on start", expectedFirstAppliedSettings);
                    // make configurable shouldn't have been called:
                    jqUnit.assertDeepEq("checking whether makeConfigurable was run", test.stashContent, gpii.tests.lifecycleManager.staticRepository.configurableStash);
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.start() and stop()", function () {
            ...
            lifecycleManager.start(gpii.tests.lifecycleManager.startPayload, function () {
                gpii.tests.lifecycleManager.assertExpectedExec();
        ...
        fluid.each(activeValues, function (active) {
            jqUnit.asyncTest("gpii.lifecycleManager.start() with active: " + (active === undefined ? "undefined" : active), function () {
                ...
                fluid.set(activeFalsePayload, [ "lifecycleInstructions", "org.gnome.desktop.a11y.magnifier", "active" ], active);
                lifecycleManager.start(activeFalsePayload, function () {
                    gpii.tests.lifecycleManager.assertExpectedSettingsHandler(" - on start", gpii.tests.lifecycleManager.settingsHandlerExpectedInputNewSettings);
        ...
        gpii.tests.lifecycleManager.buildConfigurabilityTests();
        ...
        jqUnit.asyncTest("gpii.lifecycleManager.update() tests for 'stop' reference", function () {
            ...
            lifecycleManager.start(startPayload, function (success) {
                jqUnit.assertTrue("gpii.lifecycleManager.start() succeeds", success);
                // check that things have 'started'
                gpii.tests.lifecycleManager.assertExpectedExec();

                lifecycleManager.update(startPayload, function () {
                    // check that things have 'stopped'
                    jqUnit.assertEquals("Expected pid has been sent to kill handler",
                        8839, gpii.tests.lifecycleManager.staticRepository.killHandler.pid);
                    lifecycleManager.stop(gpii.tests.lifecycleManager.userOptions, function () {
                        jqUnit.assertUndefined("no active sessions running after stop", lifecycleManager.activeSessions[startPayload.userToken]);
                        jqUnit.start();
                    });
                });
            });
