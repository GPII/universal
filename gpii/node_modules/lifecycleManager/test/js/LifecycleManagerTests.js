...
(function ($) {
    ...
    // fetcher related tests:
    gpii.tests.lifecycleManager.combineFetchersSpec = {
        functions: [
            function (val) { return val == "fun1" ? "foo1" : undefined },
            "I am not a function",
            function (val) { return val == "fun2" ? "foo2" : undefined }
        ],
        tests: {
            "First match": {
                input: "fun1",
    ...
    gpii.tests.lifecycleManager.combineFetchers = function () {
        var fetchers = gpii.combineFetchers.apply(undefined, gpii.tests.lifecycleManager.combineFetchersSpec.functions);
        fluid.each(gpii.tests.lifecycleManager.combineFetchersSpec.tests, function (spec, desc) {
            var response = fetchers(spec.input);
            jqUnit.assertEquals(desc, spec.expected, response);
        });
    }
    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.test("Testing gpii.combineFetchers", gpii.tests.lifecycleManager.combineFetchers);
