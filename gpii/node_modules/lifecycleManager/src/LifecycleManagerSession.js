    ...
    */
    ...
    gpii.lifecycleManager.computeLocalFetcher = function (session) {
        ...
        return gpii.resolversToFetcher({
            ...
            solution: function (solutionPath) {
                // split into namespace part and variable part, looking for the longest matching solution entry.
                var pathSegments = solutionPath.split(".");
                for (var a = pathSegments.length; a >= 0 ; a--) {
                    var solutionId = pathSegments.slice(0, a).join(".");
                    var dataPath = pathSegments.slice(a).join(".");
                    var solutionEntry = fluid.get(session.model, ["solutionsRegistryEntries", solutionId]);
                    if (solutionEntry) {
                        return fluid.get(solutionEntry, dataPath);
                    }
                }
            }
        });
