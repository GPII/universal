...
(function () {
    ...
    gpii.lifecycleManager.session.setAutoSavePrefs = function (existingAutoSavePrefs, preferences, path) {
        var prefsValue = fluid.get(preferences, path);
        if (prefsValue) {
            fluid.set(existingAutoSavePrefs, path, prefsValue);
        }
        return existingAutoSavePrefs;
    };
    ...
    gpii.lifecycleManager.session.filterPrefsByAutoSave = function (preferences, metadata) {
        ...
        fluid.each(autoSaveScope, function (autoSaveLvl1Value, autoSaveLvl1Path) {
            // handle scoped common terms that are in nested structure:
            // "http://registry.gpii.net/applications/org.gnome.desktop.a11y.magnifier": {
            //     "http://registry.gpii.net/common/magnification"
            // }
            if (fluid.isObjectSimple(autoSaveLvl1Value)) {
                fluid.each(autoSaveLvl1Value, function (isAllowed, autoSaveLvl2Path) {
                    if (isAllowed === true) {
                        prefsForAutoSave = gpii.lifecycleManager.session.setAutoSavePrefs(
                            prefsForAutoSave,
                            preferences,
                            [autoSaveLvl1Path, autoSaveLvl2Path]
                        );
                    }
                });
            }

            if (autoSaveLvl1Value === true) {
                // handle one level common terms such as
