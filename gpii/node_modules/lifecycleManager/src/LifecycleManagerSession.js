...
(function () {
    ...
    gpii.lifecycleManager.session.filterPrefsByAutoSave = function (preferences, metadata) {
        ...
        fluid.each(metadata, function (meta) {
            if (meta.type === "autosave") {
                autosaveScope = fluid.makeArray(meta.scope);
            }
        });
        ...
        fluid.each(preferences, function (value, preference) {
            if (autosaveScope.indexOf(preference) !== -1) {
                prefsForAutoSave = prefsForAutoSave || {};
                fluid.set(prefsForAutoSave, [preference], value);
            }
        });
    ...
    gpii.lifecycleManager.session.savePrefsUpdates = function (that, preferences, transactionSources, preferencesUserUpdated) {
        if (transactionSources.userUpdate) {
            ...
            if (prefsForAutoSave) {
                fluid.set(preferences, ["contexts", that.model.activeContextName, "preferences"], prefsForAutoSave);
                preferencesUserUpdated.fire(that.options.gpiiKey, preferences);
            }
        }
    };
