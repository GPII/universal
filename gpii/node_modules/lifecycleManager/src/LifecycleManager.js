...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        model: {
            currentUserLogon: { // Holds information about the last non system default GPII key login/logout action
    ...
    gpii.lifecycleManager.isBounce = function (currentLogonChange, currentUserLogon, debounceTimeMs, gpiiKey) {
        ...
        if (currentLogonChange.gpiiKey === gpii.defaultGpiiKey && currentLogonChange.type === "login" && !currentLogonChange.inProgress) {
    ...
    gpii.lifecycleManager.performLogin = function (that, gpiiKey) {
        // If the active key is the system default GPII key, log it out before logging in another key
        var activeGpiiKeys = that.getActiveSessionGpiiKeys();
        if (activeGpiiKeys[0] === gpii.defaultGpiiKey) {
            that.addToUserLogonRequestQueue({
                gpiiKey: gpii.defaultGpiiKey,
                logonState: "logout"
            });
        }
