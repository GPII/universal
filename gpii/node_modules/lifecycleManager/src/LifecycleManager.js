...
(function () {
    ...
    */
    gpii.lifecycleManager.executeSettingsAction = function (that, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction) {
        ...
        // console.log("KASPER Record: " + JSON.stringify(solutionRecord, null, 2));
        var tmp = JSON.stringify(fluid.get(solutionRecord, [ "settingsHandlers", settingsHandlerBlockName, "settings"]), null, 2)
        console.log("KASPER: executeSettingsAction: (" + solutionId + ": " + settingsHandlerBlockName + ") with settings: " + tmp);
    ...
    gpii.lifecycleManager.executeActions = function (that, solutionId, solutionRecord, session, actionBlock, rootAction) {
        console.log("KASPER: Execute actions: " + JSON.stringify(actionBlock) + "... rootAction: " + rootAction)
            ...
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    ...
                    if (action === "start" || action === "configure") {
                        ...
                        // TODO KASPER -- OVERVEJ AT GENINDFØRE DET NEDENSTÅENDE
                        ...
                        // TODO KASPER: Har erstattet ovenstående med:
                        return that.executeActions(solutionId, solutionRecord, session, "stop", rootAction);
                    } else {
    ...
    gpii.lifecycleManager.restoreSystem = function (that, session, rootAction) {
        var promises = fluid.transform(session.model.originalSettings, function (changesSolutionRecord, solutionId) {
            ...
            var actions = session.model.runningOnLogin[solutionId] ? [ "update" ] : [ "stop", "restore" ];
    ...
    gpii.lifecycleManager.start = function (that, request, finalPayload, callback) {
        console.log("KASPER: gpii.lifecycleManager.start")
        var userToken = finalPayload.userToken,
            lifecycleInstructions = finalPayload.lifecycleInstructions;
                ...
                if (!fluid.isDestroyed(that)) { // See above comment for GPII-580
                    ...
                    if (isRunning === true) { // if it's already running
                        if (solution.active === false) { // and it shouldn't
                            ...
                            console.log("KASPER: running and active = yes, so using 'update directive'")
                            actions = [ "update" ];
                        }
