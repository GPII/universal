...
(function () {
    ...
    // Transform the response from the handler to a format that we can pass back
    // to it
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    gpii.lifecycleManager.invokeSettingsHandlers = function (solutionId, settingsHandlers) {
        ...
        return fluid.transform(settingsHandlers, function (handlerSpec) {
            ...
            // update the settings section of our snapshot to contain the new
            // format
        });
    };
    ...
    gpii.lifecycleManager.update = function (activeSessions, resolver, options, solutions, callback) {
        ...
        var togo = {};
        fluid.transform(solutions, function (solution) {
            if (solution.dynamic) {
                var solIndex = fluid.find(sessionState.solutions,
                    function (sol, index) {
                        if (sol.id === solution.id) {return index;}
                    });
                var sol;
                if (solIndex < 0) {
                    sol = fluid.copy(solution);
                    try {
                        sol.settingsHandlers = gpii.lifecycleManager.executeActions(
                            solution.id, solution.settingsHandlers,
                            solution.lifecycleManager.start, sessionState);
                        delete togo.lifecycleManager.start;
                    }
                } else {
                    sol = sessionState.solutions[solIndex];
                    try {
            ...
            // var togo = fluid.copy(solution);
            // togo.settingsHandlers = gpii.lifecycleManager.executeActions(
            //     solution.id, solution.settingsHandlers,
            //     solution.lifecycleManager.start, sessionState);
            // delete togo.lifecycleManager.start;
            // var solutionIndex = fluid.find(sessionState.solutions,
            //     function (sol, index) {
    ...
    gpii.lifecycleManager.start = function (activeSessions, resolver, stop, options, solutions, callback) {
        ...
        if (activeSessions[userToken]) {
            // TODO: develop async architecture to prevent rat's nest of
            // callbacks
