...
(function () {
    ...
    gpii.lifecycleManager.applyWithNewPrefs = function (that, session, contextualPrefs, prefsAppliedLocalEvent) {
        var newPrefsSet = {};
        fluid.set(newPrefsSet, ["contexts", session.model.activePrefsSetName, "preferences"], contextualPrefs);
    ...
    gpii.lifecycleManager.applyExistingPrefs = function (that, session, contextualPrefs, prefsAppliedLocalEvent) {
        ...
        session.applier.change(["preferences", "contexts", session.model.activePrefsSetName, "preferences"], contextualPrefs, "ADD", "userUpdate");
    ...
    */
    ...
    gpii.lifecycleManager.processUpdate = function (that, finalPayload) {
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            ...
            var appliedSettingChanges = appliedSolutions[solutionId];
            if (appliedSettingChanges) {
                ...
                if (equal) {
                    // no settings change for this solution
                    settingChangedForThisSolution = false;
                } else {
                    // merge already applied settings with the updates
                    settingChanges = fluid.extend(true, {}, appliedSettingChanges, settingChanges);
                }
            }
    ...
    gpii.lifecycleManager.processStart = function (that, userSession, finalPayload) {
        ...
        var filteredPayload = fluid.filterKeys(finalPayload, ["gpiiKey", "preferences", "activePrefsSetName", "activeConfiguration", "solutionsRegistryEntries", "matchMakerOutput"]);
    ...
    gpii.lifecycleManager.updateActivePrefsSetName = function (that, fullPayload, newPrefsSetName) {
        fluid.log("PREFS SET NAME: updateActivePrefsSetName(), new name is '" + newPrefsSetName + "'"); // for debugging
        var activeSessionPrefsSetName = that.getSession().model.activePrefsSetName;
        if (!newPrefsSetName) {
            if (activeSessionPrefsSetName) {
                newPrefsSetName = activeSessionPrefsSetName;
            } else {
                newPrefsSetName = "gpii-default";
            }
        }
        fullPayload.activePrefsSetName = newPrefsSetName;
        return fullPayload;
    };

    /**
     * On changes to the preferences set:
     * 1) parse the set against the currently logged in user's conditions
    ...
    gpii.lifecycleManager.prefsSetNameChanged = function (that, newPrefsSetName) {
        fluid.log("PREFS SET NAME: prefsSetNameChanged(), new name provided is '" + newPrefsSetName + "'"); // for debugging
        ...
        response.then(function () {
            ...
            fluid.log(fluid.logLevel.FAIL, "lifecycleManager: Failed to apply newly evaluated conditions");
        });
        return response;
    };
