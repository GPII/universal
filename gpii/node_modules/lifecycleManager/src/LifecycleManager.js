...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        retryOptions: { // Options governing how often to recheck whether settings are set (and in future, to check for running processes)
            numRetries: 10, // Make 10 attempts over a period of 10 seconds to discover whether settings are set
            retryInterval: 1000
        },
        components: {
            variableResolver: {
        ...
        invokers: {
            ...
            applySolution: {
                ...
                args: ["{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2", "{arguments}.3"]
                                  // solution, solutionRecord, actions, sessionState
            },
            executeActions: {
    ...
    gpii.lifecycleManager.responseToSnapshot = function (solutionId, handlerResponse) {
        var unValued = gpii.settingsHandlers.transformPayloadSettings(handlerResponse, function (element) {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    gpii.lifecycleManager.invokeSettingsHandlers = function (that, solutionId, settingsHandlers) {
        ...
        var settingsPackage = fluid.transform(settingsHandlers, function (handlerSpec, index) {
            ...
            return {
                setSettings: function () {
                    console.log("||*|*|*||*|* Invoking SET at position ", index, " of ", settingsHandlers.length);
                    var togo = gpii.settingsHandlers.invokeRetryingHandler(resolvedName, settingsHandlerPayload, that.options.retryOptions);
                    console.log("||*|*||*||** Sent SET settings handler payload of ", settingsHandlerPayload[solutionId], " received response ", togo);
                    return togo;
                },
                makeSnapshot: function (handlerResponse) {
                    console.log("****makeSnapshot got handlerResponse ", handlerResponse);
                    ...
                    console.log("!*!*!*!*!*returning settingsSnapshot ", settingsSnapshot);
        ...
        responsePromise.then(function (responses) {
            ...
            console.log("*********** invokeSettingsHandlers resolving with snapshots ", snapshots);
    ...
    // Argument 1 (solution) is the structure which will be modified to form the snapshot record (and stashed in sessionState)
    // Argument 2 (solutionRecord) is not modified and consists of the original transformer return (doesn't really appear necessary)
    ...
    // as a side-effect
    gpii.lifecycleManager.applySolution = function (that, solution, solutionRecord, actions, sessionState) {
        console.log("************** START applySolution for solution ", solutionRecord.id);
        var promise = that.executeActions(solutionRecord.id, solutionRecord.settingsHandlers, actions, sessionState);
        promise.then(function (snapshot) {
            solution.settingsHandlers = snapshot;
            console.log("************ applySolution has assembled snapshotted solution ", snapshot);
    ...
    gpii.lifecycleManager.stop = function (that, requestProxy, options, callback) {
        ...
        var promises = fluid.transform(sessionState.solutions, function (solution) {
            console.log("***** lifecycleManager STOP for solution ", solution.settingsHandlers);
            return that.executeActions(solution.id, solution.settingsHandlers, solution.lifecycleManager.stop, sessionState);
        });
    ...
    gpii.lifecycleManager.update = function (that, requestProxy, options, solutions, callback) {
        ...
        var promises = fluid.transform(solutions, function (solution) {
            ...
            // TODO: probably bug here - it looks like we will trash our original snapshot of the settings taken during "start"
            return that.applySolution(sol, solution, actions, sessionState);
        });
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, options, solutions, callback) {
        ...
        // This "start" action builds up a payload in sessionState.solutions which is exactly isomorphic with the one
        ...
        var tasks = fluid.transform(solutions, function (solution) {
            return function () {
                // build structure for returned values (for later reset)
                var togo = fluid.copy(solution);
                return that.applySolution(togo, solution, solution.lifecycleManager.start, sessionState);
            };
        });
