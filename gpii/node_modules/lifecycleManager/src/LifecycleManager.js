...
(function () {
    ...
    // 1. translates the top level high contrast setting "http://registry.gpii.net/common/highContrast" to 2 settings
    // for windows and UIO+;
    // 2. sync up these 2 values by finding out which one has been changed. The sync-up includes:
    ...
    // This logic should be removed once the proper solution for applying high contrast from QSS is implemented.
    gpii.lifecycleManager.addHighContrastHandling = function (contextualPrefs, sessionModel) {
        var prefsTogo = fluid.copy(contextualPrefs);
        var highContrastTheme = fluid.get(contextualPrefs, ["http://registry.gpii.net/common/highContrastTheme"]);

        if (highContrastTheme) {
            fluid.set(prefsTogo, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrastTheme"], highContrastTheme);
            fluid.set(prefsTogo, ["http://registry.gpii.net/applications/net.gpii.uioPlus", "http://registry.gpii.net/common/highContrastTheme"], highContrastTheme);

            var isEnabled = highContrastTheme === "regular-contrast" ? false : true;
            fluid.set(prefsTogo, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrast/enabled"], isEnabled);
            fluid.set(prefsTogo, ["http://registry.gpii.net/applications/net.gpii.uioPlus", "http://registry.gpii.net/common/highContrast/enabled"], isEnabled);
        }
        prefsTogo = fluid.censorKeys(prefsTogo, ["http://registry.gpii.net/common/highContrastTheme"]);
        return prefsTogo;
    };
    ...
    gpii.lifecycleManager.applyPreferences = function (that, preferences, prefsAppliedLocalEvent) {
        ...
        // TODO: This special handling that translates the top level high contrast setting
        // "http://registry.gpii.net/common/highContrast" to 2 settings for windows and
        // UIO+ can be removed once the proper solution for applying high contrast from QSS
        // is implemented.
        contextualPrefs = gpii.lifecycleManager.addHighContrastHandling(contextualPrefs, session.model);
