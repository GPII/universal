...
(function () {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    /**
     * @param action {String} should be either a reference to a settings block, written as settings.<blockName>
     *     where <blockName> is the name of a block in the settingsHandlers section.
     ...
     *     function.
    */
    gpii.lifecycleManager.executeSettingsAction = function (that, solutionId, solutionRecord, sessionState, settingsReturn, action) {
        ...
        var sequence = fluid.transform(steps, function (action) {
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    return gpii.lifecycleManager.executeSettingsAction(that, solutionId, solutionRecord, sessionState, settingsReturn, action);
     ...
     * The expected outcome is that solution should be restored to it's original state
     *
     * @param solutionId {String} the ID of the solution
     ...
     * @preturn A promise (sequence of promises) as returned by executeActions
     */
