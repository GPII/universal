...
(function () {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
        ...
        var sequence = fluid.transform(steps, function (action) {
            if (typeof(action) === "string") { // settings.*  where * can be configure, restore, update, start or stop
                if (action.indexOf("settings.") === 0) {
                    ...
                    // } else if (action === "stop") {
                    //     promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "stop"));
     ...
     * as a side-effect
     */
    gpii.lifecycleManager.applySolution = function (that, toSnapshot, solutionId, solutionRecord, sessionState, isUpdate) {
        var promises = [];
        if (isUpdate === false) {
            promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "configure"));
            if (solutionRecord.active === true) { //if solution should be launched on configuration
                promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "start"));
            }
        } else {
            promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "update"));
        }
    ...
    gpii.lifecycleManager.removeSolution = function (that, solutionId, solutionRecord, sessionState) {
        var promises = [];
        promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "restore"));
        promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "stop"));
    ...
    gpii.lifecycleManager.stop = function (that, requestProxy, options, callback) {
        ...
        var promises = fluid.transform(sessionState.originalSettings, function (solutionRecord, solutionId) {
            return that.removeSolution(solutionId, solutionRecord, sessionState);
        });
    ...
    gpii.lifecycleManager.update = function (that, requestProxy, finalPayload, callback) {
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            ...
            promises.push(that.applySolution(null, solutionId, sol, sessionState, true));
        });
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, finalPayload, callback) {
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            tasks.push(function () {
                ...
                return that.applySolution(togo, solutionId, solution, sessionState, false);
            });
        });
