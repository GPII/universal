...
(function () {
    ...
    /**
     * Read corresponding setting values of requested preferences.
     * @param {Component} that - An instance of gpii.lifecycleManager.
     * @param {Object} preferences - The preferences to read settings for.
     * @return {Promise} An promise whose resolved value is the an object with setting values read from the system.
     */
    gpii.lifecycleManager.readPreferences = function (that, preferences) {
        ...
        var getSettingsPromise = fluid.promise();
        var solutionSpecs = {};
        ...
        matchMakingPromise.then(function (payload) {
            ...
            var matchedSolutions = fluid.get(payload, ["matchMakerOutput", "inferredConfiguration", activeContextName, "applications"]);
            fluid.each(matchedSolutions, function (trash, solutionId) {
                var oneSolutionSpec = fluid.get(payload, ["solutionsRegistryEntries", solutionId]);
                if (oneSolutionSpec) {
                    fluid.set(solutionSpecs, [solutionId], oneSolutionSpec);
                }
            });

            fluid.log("=== solutionSpecs: ", solutionSpecs);
            var settingsPromise = that.read(solutionSpecs);
            settingsPromise.then(function (result) {
                fluid.log("=== result: ", result);
            });
        });
    };
    ...
    gpii.lifecycleManager.processRead = function (that, solutions) {
        ...
        fluid.each(solutions, function (solution, solutionID) {
            ...
            fluid.each(newSolution.settingsHandlers, function (handlerSpec) {
                ...
                handlerGetPromises.push(safeHandlerGet(that.invokeSettingsHandlerGet, solutionID, handlerSpec));
            });
        });
