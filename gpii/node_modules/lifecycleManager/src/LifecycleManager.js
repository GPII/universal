...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            isConfigurable: {
                funcName: "gpii.lifecycleManager.isConfigurable",
                args: [ "{that}", "{arguments}.0", "{arguments}.1" ]
            }
        }
    });
    ...
    */
    ...
    /* returns whether a solution is configurable or not. This is done by going through each of
     * the blocks in the isConfigurable part of the solution entry. If *any* of them evaluates to
     * true, the solution is considered configurable and true is returned, else false is returned.
    ...
    gpii.lifecycleManager.isConfigurable = function (that, solutionRecord, sessionState) {
        if (solutionRecord.isConfigurable !== undefined) {
            return fluid.find(solutionRecord.isConfigurable, function (configurableCheck) {
                var expanded = sessionState.localResolver(configurableCheck);
                var result = gpii.lifecycleManager.invokeAction(expanded, that.nameResolver);
                if (result === true) {
                    return result;
                }
            }, false);
        } else { // if no isConfigurable block is given, we assume it's configurable
            return true;
        }
    };
    ...
    gpii.lifecycleManager.applySolution = function (that, solutionId, solutionRecord, sessionState, lifecycleBlockKeys, fullSnapshot) {
        ...
        if (that.isConfigurable(solutionRecord, sessionState) === false) {
            if (solutionRecord.makeConfigurable === undefined) {
                fluid.fail("Found solution that is not configurable with no makeConfigurable directive");
            } else {
                promises.push(that.executeActions(solutionId, solutionRecord, sessionState, "makeConfigurable"));
            }
        }
