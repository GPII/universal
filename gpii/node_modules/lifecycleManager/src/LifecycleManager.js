...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            snapshot: {
                ...
                args: ["{arguments}.0", "{arguments}.1", "{arguments}.2","{gpii.lifecycleManager}.variableResolver"]
            }
        }
    });
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    gpii.lifecycleManager.snapshot = function (request, solutions, device, resolver, callback) {
        //TODO:
        ...
        fluid.each(solutions, function (solution) {
            fluid.each(solution.settingsHandlers, function (handlerSpec) {
                ...
                // a more efficient implementation might bulk together payloads
                // destined for the same handler
                handlerSpec.settings = null;
                var settingsHandlerPayload =
                    gpii.lifecycleManager.specToSettingsHandler(solution.id, handlerSpec);
                settingsHandlerPayload = resolver.resolve(settingsHandlerPayload, {}); //sessionState.localResolver(settingsHandlers);

                    var type = handlerSpec.type;
                    type = type.substring(0, type.length-4)+".get";

                //send the payload to the settingsHandler
                var handlerResponse = fluid.invokeGlobalFunction(type,
                    [settingsHandlerPayload]);
                payload[handlerSpec.type] = handlerResponse;
                console.log(JSON.stringify(handlerResponse, null, 4));
                //build structure for returned values (for later reset)
                // var togo = fluid.copy(solution);
                // var expanded = resolver.resolve(settingsHandlers, {}); //sessionState.localResolver(settingsHandlers);
        ...
        request.events.onSuccess.fire(JSON.stringify(payload, null, 4));
    };
}());

//handlerSpec.type: "gpii.settingsHandlers.INISettingsHandler.set"
