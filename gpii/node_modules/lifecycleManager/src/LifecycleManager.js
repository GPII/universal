...
(function () {
    ...
    */
     ...
     *     will be built up as a side-effect within session.currentSettings, session.originalSettings, session.actionResults etc.
    ...
    gpii.lifecycleManager.processStop = function (that, options) {
     ...
     /**
      * Read setting values of one or more solutions.
      * @param {Object} that - An instance of gpii.lifecycleManager.
      * @param {Object} solutions - Solutions with setting to be read from the system.
      * @return {Promise} A promise whose resolved value is the result of setting values.
      */
    gpii.lifecycleManager.processRead = function (that, solutions) {
        ...
        fluid.each(solutions, function (solution, solutionID) {
            ...
            fluid.each(newSolution.settingsHandlers, function (handlerSpec) {
                // If supportedSettings is not specified, don't attempt to get settings
                if (!handlerSpec.supportedSettings) {
                    return;
                }
                handlerSpec.settings = handlerSpec.supportedSettings;

                // TODO: The code below is to work around https://issues.gpii.net/browse/GPII-3677.
                // It transforms the paths in capabilitiesTransforms to the schema information needed for
                // executing get() for the settings handler "gpii.windows.spiSettingsHandler".
                ...
                handlerGetPromises.push(that.invokeSettingsHandlerGet(solutionID, handlerSpec));
            });
        });
    ...
    gpii.lifecycleManager.processUpdate = function (that, finalPayload) {
        ...
        var currentSettings = userSession.model.currentSettings;

        var promises = [];
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            var settingChangedForThisSolution = true;
            var settingChanges = fluid.copy(solution);
            var appliedSettingChanges = currentSettings[solutionId];
            if (appliedSettingChanges) {
                var equal = fluid.model.diff(settingChanges.settingsHandlers, appliedSettingChanges.settingsHandlers);
                if (equal) {
                    // no settings changed for this solution
                    settingChangedForThisSolution = false;
                }
            }
            // Re-apply setting changes for this solution only if there are setting(s) being changes, by comparing
            // with setting changes applied before .
            if (settingChangedForThisSolution) {
                promises.push(that.applySolution(solutionId, settingChanges, userSession, [ "update" ], "update"));
            }
        });
        return fluid.promise.sequence(promises);
    };
    ...
    gpii.lifecycleManager.processStart = function (that, userSession, finalPayload) {
        ...
        fluid.set(finalPayload, ["currentPreferences"], gpii.lifecycleManager.getPreferencesForSet(finalPayload.preferences, finalPayload.activePrefsSetName));
        var filteredPayload = fluid.filterKeys(finalPayload, ["gpiiKey", "preferences", "activePrefsSetName", "activeConfiguration", "solutionsRegistryEntries", "matchMakerOutput", "currentPreferences"]);
    ...
    /**
     * Function to add the current name of the preferences set to the full
     * payload output from the matchmaking process.  The current name is given
     * by one of (in priority sequence):
     * 1. a passed in name (newPrefsSetName)
     * 2. the LifecycleManager's session's model (activePrefsSetName)
     * 3. "gpii-default"
     *
     * @param {Component} that - An instance of LifecycleManager component
     * @param {Object} fullPayload - The full structure output from the matchmaking process. NOTE: This object will be modified
     * @param {String} [newPrefsSetName] - [optional] A preferences set name to be selected.
     * @return {Object} The modified fullPayload, now including activePrefsSetName
     */
    gpii.lifecycleManager.updateActivePrefsSetName = function (that, fullPayload, newPrefsSetName) {
        ...
        fullPayload.activePrefsSetName = (
            newPrefsSetName ? newPrefsSetName :
            activeSessionPrefsSetName ? activeSessionPrefsSetName : "gpii-default"
        );
        return fullPayload;
    };

    /**
     * On changes to the preferences set:
     * 1) compare the set against the currently logged in user's preferences set
     * 2) if the newly calculated active set of preferences is different from the currently active one,
     *    the LifecycleManager's update functionality will be called and the new prefsSet and
     *    configuration will be stored in the active session of the LifecycleManager.
     *
     * @param {Component} that - The lifecycleManager component holding the current session
     * @param {String} [newPrefsSetName] - The name of the preferences set to switch to.  If missing, this
     *    function does nothing.
     * @return {Promise} If the preferences set change causes an actual change in the system (i.e. change of
     *    activePrefsSet and changes to the applied configuration) a promise of an update to the system
     *    configuration is returned. If the call to prefsSetChanged does not result in a different
     *    active preferences set or no user is logged in, undefined is returned.
     **/
    gpii.lifecycleManager.prefsSetNameChanged = function (that, newPrefsSetName) {
        var activeSession = that.getSession();
        ...
        if (!activeGpiiKey || gpii.isReservedGpiiKey(activeGpiiKey)) {
            fluid.log("lifecycleManager: No active GPIIKey, or system reserved GPIIKey: no prefs set change");
            return;
        }
        var fullPayload = fluid.extend(true, {}, activeSession.model);
        var oldPrefsSetName = fullPayload.activePrefsSetName;
        that.updateActivePrefsSetName(fullPayload, newPrefsSetName);
        if (oldPrefsSetName === fullPayload.activePrefsSetName) {
        ...
        // partially applied set of solutions from the new name, confusing, e.g., the PSPChannel
        // This is required because of https://issues.fluidproject.org/browse/FLUID-6208
        activeSession.applier.change("activePrefsSetName", fullPayload.activePrefsSetName);
        fluid.log("lifecycleManager: Preferences set changed from current \"", oldPrefsSetName, "\" to new active \"" + fullPayload.activePrefsSetName, "\" for the GPII key: ", activeGpiiKey);

        // Clean up the previous tracked preferences
        activeSession.applier.change("currentPreferences", null, "DELETE");

        // Delete the old currentSettings as the new structure will be updated directly onto activeSession.model
        // by lifecycleManager.update() called just after.
        delete fullPayload.currentSettings;

        that.addLifecycleInstructionsToPayload(fullPayload);
        var response = that.update(fullPayload);

        response.then(function () {
            fluid.set(fullPayload, ["currentPreferences"], gpii.lifecycleManager.getPreferencesForSet(fullPayload.preferences, fullPayload.activePrefsSetName));
            // LifecycleInstructions is only used in the "finalPayload" as
            // instructions on how to configure the system. Once configured, the
            // applied settings are stored in 'activeConfiguration'
            activeSession.applier.change("", fullPayload); // TODO: this will need to be a "MERGE" in time
            fluid.log("lifecycleManager: Successfully updated configuration triggered by prefsSet changes");
        }, function () {
            fluid.log(fluid.logLevel.FAIL, "lifecycleManager: Failed to apply newly preferences set: ", fullPayload.activePrefsSetName);
        });
        return response;
    };
