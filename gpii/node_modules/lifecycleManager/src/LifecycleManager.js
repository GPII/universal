...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            snapshot: {
                funcName: "gpii.lifecycleManager.snapshot",
                args: ["{requestProxy}", "{gpii.lifecycleManager}.variableResolver", "{arguments}.0"]
                                                                                     //solutions
            }
        }
    });
    ...
    */
    ...
    gpii.lifecycleManager.snapshot = function (request, resolver, solutions) {
        var payload = {};
        fluid.each(solutions, function (solution) {
            fluid.each(solution.settingsHandlers, function (handlerSpec) {
                ...
                if (!handlerSpec.applicationSettings)
                    return;

                handlerSpec.settings = handlerSpec.applicationSettings;
                handlerSpec = resolver.resolve(handlerSpec, {});

                var settingsHandlerPayload =
                    gpii.lifecycleManager.specToSettingsHandler(solution.id, handlerSpec);

                var type = handlerSpec.type;
                type = type.substring(0, type.length-4)+".get";

                //send the payload to the settingsHandler
                var handlerResponse = fluid.invokeGlobalFunction(type, [settingsHandlerPayload]);
                fluid.each(handlerResponse, function (settingsEntries, solutionId) {
        ...
        request.events.onSuccess.fire(payload);
    };
})();
