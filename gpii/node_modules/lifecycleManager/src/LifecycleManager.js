...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        components: {
            ...
            matchMakingRequest: {
                type: "gpii.lifecycleManager.matchMaking.request"
            }
        },
        dynamicComponents: {
            sessions: {
        ...
        listeners: {
            ...
            "onClearQueue.clearQueue": "gpii.lifecycleManager.clearQueue({that}.queue)"
        },
        queueFunctions: {
    ...
    gpii.lifecycleManager.applyPreferences = function (that, preferences, prefsAppliedLocalEvent) {
        ...
        var session = that.getSession(activeGpiiKeys);
        ...
        // The work around for the issue that PSP/QSS only sends in the update on either highContrastTheme or highContrastEnabled.
        // The logic below is to sync up these 2 values by finding out which one has been changed. The sync-up includes:
        // 1. When highContrastTheme is set to a non-regular, highContrast/enabled needs to be set to true;
        // 2. When highContrastTheme is restored to the regular, highContrast/enabled needs to be set to false;
        // 3. When highContrast/enabled is set to false, highContrastTheme must be set to "regular-contrast”.
        // This logic should be removed once PSP/QSS sends in asynchronized values.
        var origHighContrastEnabled = fluid.get(session.model, ["preferences", "contexts", session.model.activeContextName, "preferences", "http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrast/enabled"]);
        var origHighContrastTheme =  fluid.get(session.model, ["preferences", "contexts", session.model.activeContextName, "preferences", "http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrastTheme"]);

        var highContrastEnabled = fluid.get(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrast/enabled"]);
        var highContrastTheme = fluid.get(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrastTheme"]);

        if (highContrastEnabled !== origHighContrastEnabled) {
            if (highContrastEnabled === false) {
                fluid.set(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrastTheme"], "regular-contrast");
            }
            if (highContrastEnabled === true) {
                fluid.set(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrastTheme"], highContrastTheme);
            }
        }
        if (highContrastTheme !== origHighContrastTheme) {
            if (highContrastTheme === "regular-contrast") {
                fluid.set(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrast/enabled"], false);
            } else {
                fluid.set(contextualPrefs, ["http://registry.gpii.net/applications/com.microsoft.windows.highContrast", "http://registry.gpii.net/common/highContrast/enabled"], true);
            }
        }

        var newPrefs = {};
        fluid.set(newPrefs, ["contexts", session.model.activeContextName, "preferences"], contextualPrefs);
        var fullPrefs = fluid.extend({}, session.model.preferences, newPrefs);
        var matchMakingPromise = that.matchMakingRequest.doMatchMaking(activeGpiiKeys[0], fullPrefs);

        matchMakingPromise.then(function (newPayload) {
            var fullPayload = fluid.extend(true, {}, session.model);
            fluid.set(fullPayload, ["preferences"], fullPrefs);
            ...
            session.applier.change(["solutionsRegistryEntries"], newPayload.solutionsRegistryEntries, "ADD", "REMATCHMAKING");
            session.applier.change(["activeConfiguration"], fullPayload.activeConfiguration, "ADD", "REMATCHMAKING");
            session.applier.change(["preferences"], newPrefs, "ADD", "userUpdate");
    ...
    */
    ...
    gpii.lifecycleManager.processQueue = function (that, queue) {
        ...
        promise.then(function (val) {
            ...
            fluid.log(fluid.logLevel.FAIL, "An error occurred in an item of the lifecyclemanager's queue" +
                "(invoker: " + item.invokerName + ", func: " + item.func + "), so clearing queue. Error was ", error);
            that.events.onClearQueue.fire();
            that.isProcessingQueue = false;
