...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            getUserTokens: {
                funcName: "gpii.lifecycleManager.getUserTokens",
                args: [ "{that}" ]
            },
    ...
    */
    ...
    gpii.lifecycleManager.applySolution = function (that, solutionId, solutionRecord, sessionState, lifecycleBlockKeys, fullSnapshot) {
        ...
        promiseSequence.then(function (snapshots) {
            ...
            if (fullSnapshot) {
                ...
                var tmpOrigSettings = $.extend(true, {}, toSnapshot, sessionState.originalSettings[solutionId]);

                // ensure that our augmented snapshot has any 'undefined' settings value copied over as well
                // (since jQuerys extend will treat these as variables that are not set)
                for (shName in toSnapshot.settingsHandlers) {
                    shBlock = toSnapshot.settingsHandlers[shName];
                    for (setting in shBlock.settings) { // will loop over undefined vals
                        tmpOrigSettings.settingsHandlers[shName].settings[setting] = shBlock.settings[setting];
                    }
                }
                ...
                // properly (again, jQuerys extend will treat these as variables that are not set)
                if (sessionState.originalSettings[solutionId]) {
                    for (shName in sessionState.originalSettings[solutionId].settingsHandlers) {
                        shBlock = sessionState.originalSettings[solutionId].settingsHandlers[shName];
                        for (setting in shBlock.settings) { // will loop over undefined vals
                            tmpOrigSettings.settingsHandlers[shName].settings[setting] = shBlock.settings[setting];
                        }
                    }
    ...
    gpii.lifecycleManager.getSession = function (session) {
        return session;
    };

    gpii.lifecycleManager.getUserTokens = function (that) {
        return (that.session && that.session.userTokens) ? that.session.userTokens : [];
    };
    ...
    gpii.lifecycleManager.update = function (that, requestProxy, finalPayload, callback) {
        ...
        var sessionState = that.session;
        if (!sessionState.userTokens) {
            fluid.fail("There currently is not active session to update");
        }
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, finalPayload, callback) {
        ...
        if (that.getUserTokens().length > 0) {
            var userTokens = [];
            // Generally we should, if we hit this point be in a situation of a double login
            // but we double check here - and build the activeUserToken array for later:
            if (finalPayload.preferences instanceof Array) {
                userTokens = that.getUserTokens();
            }
            // Indicate we want to log everyone out
            that.stop({ userToken: null }, fluid.identity);
            // but remember the currently logged in user (we deleted session on logout):
            that.session = {
        ...
        var sessionState = $.extend(true, {
            actionResults: {}
        }, fluid.filterKeys(finalPayload, ["userToken", "preferences", "activeContextName", "activeConfiguration", "solutionsRegistryEntries", "matchMakerOutput", "deviceContext", "webSolutionsRegistryEntries", "selectedMatchMaker", "tryAgain"]));
        ...
        // var tmp =
        sessionState.userTokens = that.getUserTokens().concat(sessionState.userToken);
        var tasks = [];
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            tasks.push(function () {
