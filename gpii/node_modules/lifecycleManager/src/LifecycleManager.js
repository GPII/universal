...
(function () {
    ...
    */
    gpii.lifecycleManager.invokeSettingsHandler = function (that, solutionId, handlerSpec) {
     ...
     * @param rootAction {String} The root action on the LifecycleManager which is being serviced: "start", "stop" or "update"
    ...
    */
    gpii.lifecycleManager.executeSettingsAction = function (that, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction) {
        ...
        return function () {
            ...
            var settingsPromise = that.invokeSettingsHandler(solutionId, expanded);
        ...
        var sequence = fluid.transform(steps, function (action) {
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    ...
                    // if this is related to launching, overwrite entry with the appropriate settings,
                    // that is: { running: true } if we're in a start block, else { running: false }
                    if (actionBlock === "start" || actionBlock === "stop") {
                        var launchSettings = { running: actionBlock === "start" ? true : false };
                        fluid.set(solutionRecord, [ "settingsHandlers", settingsHandlerBlockName, "settings"], launchSettings);
                    }
            ...
            } else { // TODO should be removed when GPII-1235 has been solved (lifecycle actions are settings handlers) // TODO KASPER remove this
                return function () {
    ...
    /** Invoked on "start", "update" and "stop" phases - in addition to forwarding to gpii.lifecycleManager.executeActions,
