...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        members: {
            ...
            isProcessingUserLogonRequestQueue: false // if user logon queue is currently being processed
        },
        model: {
        ...
        invokers: {
            // Handle login, logout and proximityTriggered logon
            performLogin: {
                funcName: "gpii.lifecycleManager.performLogon",
                args: ["{that}", "{arguments}.0", "login"]
                                 // gpiiKey
            },
            performLogout: {
                funcName: "gpii.lifecycleManager.performLogon",
                args: ["{that}", "{arguments}.0", "logout"]
                                 // gpiiKey
            },
            ...
            isBounce: {
                ...
                args: ["{that}", "{that}.options.debounceTimeMs", "{arguments}.0"]
                                                                  // gpiiKey
            },
            errorResponse: {
                funcName: "gpii.lifecycleManager.errorResponse",
                args: ["{that}.userErrors", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
                                            // requestPromise or request, message, statusCode
            },
     ...
     */
    gpii.lifecycleManager.isBounce = function (that, debounceTimeMs, gpiiKey) {
        // if login/logout is in progress for <gpiiKey> we consider it a bounce
        var now = Date.now();
        var currentLogonChange = that.model.logonChange;
        var currentUserLogon = that.model.currentUserLogon;

        if (currentLogonChange.gpiiKey === "noUser" && currentLogonChange.type === "login" && !currentLogonChange.inProgress) {
            return currentUserLogon.gpiiKey === gpiiKey && now - that.model.currentUserLogon.timeStamp < debounceTimeMs;
    ...
    gpii.lifecycleManager.performProximityTriggered = function (that, gpiiKey) {
        ...
        if (activeGpiiKeys.length === 1) {
            ...
            if (previousUser !== gpiiKey) {
                var logoutPreviousUserPromise = that.addToUserLogonRequestQueue({
                ...
                logoutPreviousUserPromise.then(function () {
                    ...
                    that.errorResponse(promiseTogo, "Unable to log previous user(" + previousUser + ") out so canceling login.");
                });
            } else {
                promiseTogo = that.addToUserLogonRequestQueue({
                    gpiiKey: previousUser,
     ...
     * @param {Component} that - An instance of gpii.lifecycleManager.
     * @param {String} gpiiKey - The GPII key of the user for whom the keyin or keyout was performed.
     * @param {String} logonState - login or logout.
     * @return {Promise} The promise whose resolved value contains the logon success message and rejected value contains
     * the fail message.
     */
    gpii.lifecycleManager.performLogon = function (that, gpiiKey, logonState) {
    ...
    /**
     * Report logon error by issuing "KeyInFail" via userErrors component as well as reporting the error via the pass-in
     * parameter `togo` by firing its onError event if `togo` is a component, or, invoke reject() if `togo` is a promise.
     *
     * @param {Component} userErrors - An instance of gpii.userErrors.
     * @param {Promise|Component} togo - When `togo` is a promise, its reject() is invoked with the constructed message.
     * When `togo` is a component, its onError event is fired with the constructed message.
     ...
     */
    gpii.lifecycleManager.errorResponse = function (userErrors, togo, message, statusCode) {
    ...
    */
    ...
    gpii.lifecycleManager.addToActionQueue = function (that, queue, item) {
        ...
        queue.push(newItem);
        if (!that.isProcessingActionQueue) {
    ...
    gpii.lifecycleManager.processActionQueue = function (that, queue) {
        ...
        that.isProcessingActionQueue = true;
        ...
        promise.then(function (val) {
            ...
            if (queue.length > 0) {
                ...
                that.isProcessingActionQueue = false;
            }
        }, function (error) {
