...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            /** Accepts an array of user tokens, of which all but the first will currently be ignored. Returns the session
              * component corresponding to that user, if they have an active session. A typical usage pattern is to call "getActiveSessionTokens"
              * and send its return to "getSession"
              */
            getSession: {
    ...
    */
     ...
     * model snapshot area in the LifecycleManager's session. Tightly bound to executeSettingsAction, executes one-to-one with it with
     * almost identical argument list.
     */
     ...
     * they do not reject, to ensure that we continue to try to restore the system come what may - GPII-2160
     */
    gpii.lifecycleManager.wrapRestorePromise = function (promise, rootAction) {
        return rootAction === "restore" && fluid.isPromise(promise) ? gpii.rejectToLog(promise, " while restoring journal snapshot") : promise;
    };
     ...
     * @param rootAction {String} The root action on the LifecycleManager which is being serviced: "start", "stop", "update" or "restore"
     ...
     */
    gpii.lifecycleManager.executeSettingsAction = function (that, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction) {
        ...
        return function () {
            ...
            settingsPromise.then(function (snapshot) {
                ...
                gpii.lifecycleManager.recordSnapshotInSession(that, snapshot, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction);
            });
     ...
     * This is traditionally called during the "stop" action to unarmour all the settingsHandler blocks by converting from changes back to settings. In a
     * future version of the SettingsHandler API, this will not be necessary.
     * This is called during the "stop" action to convert the snapshotted "originalSettings" model material back to material suitable for being sent
     * to executeSettingsAction, as well as at the corresponding point during the "journal restore" operation
     ...
     */
    // TODO: Duplicate of kettle.upgradeError to avoid dependence on Kettle in this file. This needs to go into a new module once Kettle is factored up.
     ...
     * @param resolveValue {Any} [optional] An optional value to be supplied to `resolve` of the returned promise, when the underlying promise rejects.
     * @return {Promise} The wrapped promise which will resolve whether the supplied promise resolves or rejects.
     */
    // TODO: In theory this is a fairly generic promise algorithm, but in practice there are some further subtleties - for example, a better system would instead replace
    // fluid.promise.sequence with a version that allowed some form of "soft rejection" so that there might be a chance to signal failures to the user.
     ...
     * @param actionBlock {String} The key for the particular block in the solutions registry entry which is being acted on - this may
     * differ from `rootAction` because, e.g. an "update" action may be serviced by stopping and starting the solution, etc.
     * @param rootAction {String} The root action on the LifecycleManager which is being serviced: "start", "stop", "update" or "restore"
     * @return {Promise} A promise that will resolve when all the sequential actions have been performed. Any returned values will be built up as a side-effect
     * within session.appliedSolutions, session.originalSettings, session.actionResults etc.
     * If the `rootAction` is "restore", this promise will never reject, and instead the system will continue to make best efforts to continue executing actions
        ...
        var sequence = fluid.transform(steps, function (action) {
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    ...
                    return gpii.lifecycleManager.executeSettingsAction(that, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction);
                    ...
                    if (action === "start" || action === "configure") {
                        ...
                        var unarmoured = gpii.lifecycleManager.transformSolutionSettings(session.model.originalSettings[solutionId], gpii.settingsHandlers.changesToSettings);
    ...
    /** Invoked on "start", "update", "stop" and "restore" phases - in addition to forwarding to gpii.lifecycleManager.executeActions,
     * it is responsible for saving the settings that are being set (when the fullSnapshot is true) and storing
     * the list of applied solutions to the session state
     *
     ...
     * @param session {Object} the object holding the state of the system. This is updated in place by the settings application process, if `rootAction` is start.
     * @param lifecycleBlockKeys {Array} Array of ordered strings denoting which lifecycle blocks to run (supported values here are "configure", "start" and/or "update")
    ...
    gpii.lifecycleManager.start = function (that, request, finalPayload, callback) {
        ...
        sequence.then(function () {
            callback(true);
        }, function (error) { // TODO: Unclear that this branch has tests
            request.events.onError.fire(error);
        });
    };
