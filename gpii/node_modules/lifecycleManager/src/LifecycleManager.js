...
(function () {
    ...
    */
    ...
    /** Remove all the settings blocks from a solutions registry entry
      */
    gpii.settingsHandlers.removeSettingsBlocks = function (solutionEntry) {
        solutionEntry.settingsHandlers = fluid.transform(solutionEntry.settingsHandlers, function (handler) {
    ...
    gpii.lifecycleManager.recordSnapshotInSession = function (that, snapshot, solutionId, solutionRecord, session, settingsHandlerBlockName, rootAction) {
        var toSnapshot = gpii.settingsHandlers.removeSettingsBlocks(fluid.copy(solutionRecord));
    ...
    */
            ...
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    ...
                    if (actionBlock === "isRunning") {
                        // if we're just checking for the run state, dont actually modify the solutionRecord with settings:
                        var tmpSolutionRecord = fluid.copy(solutionRecord);
                        fluid.set(tmpSolutionRecord, [ "settingsHandlers", settingsHandlerBlockName, "settings"], { running: null });
                        return gpii.lifecycleManager.executeSettingsAction(that, solutionId, tmpSolutionRecord, session, settingsHandlerBlockName, rootAction);
                    } else {
                        ...
                        if (actionBlock === "start" || (actionBlock === "stop" && rootAction !== "stop") || actionBlock === "isRunning") {
    ...
    gpii.lifecycleManager.restoreSystem = function (that, session, rootAction) {
            ...
            tasks.push(function () {
                if (!fluid.isDestroyed(that)) { // See above comment for GPII-580
                    ...
                    if (currentRunState === true) { // if it's already running
                        if (recordedRunState === false) { // and it was not running on start
                            actions = [ "stop", "restore" ]
                        } else { // else update it
                            actions = [ "update" ]
                        }
                    } else { // if it is not running
                        if (recordedRunState === true) { // and it was running when we started
                            actions = [ "restore", "start" ];
        ...
        //     var solutionRecord = gpii.lifecycleManager.transformSolutionSettings(changesSolutionRecord, gpii.settingsHandlers.changesToSettings);
        //     // if a solution was running on login, run the update block to ensure that restored settings are applied
        //     var recordedState = gpii.lifecycleManager.getSolutionRunningStateFromSnapshot(solutionRecord);
        //     var currentState = that.getSolutionRunningState(solutionId, changesSolutionRecord, session);
        //     var actions;
        //     if (recordedState === undefined) { // if we didn't do anything to solution state before, dont do it now
        //         actions = [ "restore" ]
        //     } else if (recordedState === true) { // if originally running and we stopped it, updait was started when we logged in, ensure that it is still started
        //      = recordedState ? [ "update" ] : [ "stop", "restore" ];
        //     }
        //     }
        //     return that.applySolution(solutionId, solutionRecord, session, actions, rootAction);
        // });
    ...
    gpii.lifecycleManager.getSolutionRunningStateFromSnapshot = function (solutionSnapshot) {
        ...
        var isRunningBlock = fluid.makeArray(solutionSnapshot.isRunning)[0];
        if (isRunningBlock && isRunningBlock.indexOf("settings.") === 0) {
            var settingsHandlerBlockName = isRunningBlock.substring("settings.".length);
            var recordedState = fluid.get(solutionSnapshot, ["settingsHandlers", settingsHandlerBlockName, "settings", "running"]);
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            ...
            tasks.push(function () {
                if (!fluid.isDestroyed(that)) { // See above comment for GPII-580
                    ...
                    var actions;
                    if (isRunning === true) { // if it's already running
                        if (solution.active === false) { // and it shouldn't
                            actions = [ "stop", "configure" ];
                        } else { // else call "update" to make changes apply to a running solutions
                            actions = [ "update" ];
                        }
                    } else { // if it is not running
                        if (solution.active === true) { // and we want it to run
                            actions = [ "configure", "start" ];
                        } else { // else just configure it
                            actions = [ "configure" ];
                        }
                    }
