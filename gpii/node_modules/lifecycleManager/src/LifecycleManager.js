...
(function () {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    gpii.lifecycleManager.applySolution = function (solution, solutionId, sessionState) {
        var solutionCpy = fluid.copy(solution);
        solutionCpy.settingsHandlers = gpii.lifecycleManager.executeActions(
            solutionId, solution.settingsHandlers, solution.lifecycleManager.start, sessionState);

        if (solutionCpy.lifecycleManager.start) {
            delete solutionCpy.lifecycleManager.start;
        }
        sessionState.solutions.push(solutionCpy);
    };
    ...
    gpii.lifecycleManager.start = function (activeSessions, resolver, stop, options, solutions, callback) {
        ...
        fluid.each(solutions, function (solution, solutionId) {
            ...
            gpii.lifecycleManager.applySolution(solution, solutionId, sessionState);
        });
