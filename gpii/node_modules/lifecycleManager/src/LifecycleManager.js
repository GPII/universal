...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            removeSolution: {
                funcName: "gpii.lifecycleManager.removeSolution",
                args: ["{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2", "{arguments}.3"]
                                  // toSnapshot, solutionId,  solutionRecord, actions, sessionState, isUpdate // KASPER TODO fix these
            },
            executeActions: {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    gpii.lifecycleManager.executeActions = function (that, solutionId, solutionRecord, sessionState, actionBlock) {
        ...
        var sequence = fluid.transform(steps, function (action) {
            if (typeof(action) === "string") { // settings.*  where * can be configure, restore, update, start or stop
                if (action.indexOf("settings.") === 0) {
                    var settingsHandlerBlockName = action.substring(9);
                    ...
                    // TODO (GPII-1230) Fix this up so we dont run the full start and stops (including)
            ...
            } else { // TODO KASPER - should be removed
                return function () {
        ...
        return togo;
                    ...
                    //TODO KASPER: Does toSnapshot input actually do anything - is it required
     ...
     * as a side-effect
     */
     //lifecycleBlockKeys {Array} Array of ordered strings denoting which lifecycle blocks to run
    gpii.lifecycleManager.applySolution = function (that, toSnapshot, solutionId, solutionRecord, sessionState, lifecycleBlockKeys, saveSnapshot) {
        var promises = [];
        ...
        promiseSequence.then(function (snapshots) {
            ...
            if (saveSnapshot) {
                ...
                fluid.each(snapshots, function (snapshot) {
                    if (snapshot != undefined) { // can be removed once everything is settingsHandlers
                        // make copy
                        // toSnapshot.settingsHandlers = $.extend(true, {}, toSnapshot.settingsHandlers, snapshot);
                        // copy over undefined values from settings (these will be pruned by $.extend)
                        // var withUndefined = gpii.settingsHandlers.extractSettingsBlocks({a: snapshot});
                        // fluid.each(withUndefined.a, function (handlerBlock, blockKey) {
                        //     toSnapshot.settingsHandlers[blockKey].settings = handlerBlock.settings;
                        // });
    ...
    //solutionRecord: Original record
    gpii.lifecycleManager.removeSolution = function (that, solutionId, solutionRecord, sessionState, lifecycleBlockKeys) {
        var promises = [];
        fluid.each(lifecycleBlockKeys, function (key) {
            promises.push(that.executeActions(solutionId, solutionRecord, sessionState, key));
        });
    ...
    gpii.lifecycleManager.stop = function (that, requestProxy, options, callback) {
        ...
        var promises = fluid.transform(sessionState.originalSettings, function (solutionRecord, solutionId) {
            return that.removeSolution(solutionId, solutionRecord, sessionState, [ "stop", "restore" ]);
        });
    ...
    gpii.lifecycleManager.update = function (that, requestProxy, finalPayload, callback) {
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            ...
            promises.push(that.applySolution(null, solutionId, sol, sessionState, [ "update" ], false));
        });
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, finalPayload, callback) {
        ...
        fluid.each(lifecycleInstructions, function (solution, solutionId) {
            tasks.push(function () {
                ...
                return that.applySolution(togo, solutionId, solution, sessionState,
