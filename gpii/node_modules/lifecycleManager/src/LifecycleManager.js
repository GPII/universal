...
(function () {
    ...
    gpii.lifecycleManager.specFromSettingsHandler = function (handlerResponse, handlerType) {
        var settings = fluid.model.transform(handlerResponse, gpii.lifecycleManager.responseToSnapshotRules);
    };
    ...
    gpii.lifecycleManager.responseToSnapshot = function (handlerResponse) {
        return fluid.model.transform(handlerResponse, gpii.lifecycleManager.responseToSnapshotRules);
    };
   ...
   // Transformer output: http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    gpii.lifecycleManager.invokeSettingsHandlers = function (solutionId, settingsHandlers) {
        ...
        return fluid.transform(settingsHandlers, function (handlerSpec) {
            ...
            var settingsHandlerPayload = gpii.configurationManager.specToSettingsHandler(solutionId, handlerSpec);
            ...
            var settingsSnapshot = gpii.lifecycleManager.specFromSettingsHandler(handlerResponse);
            var invariant = fluid.model.transform(handlerSpec, gpii.lifecycleManager.invariantSettings);
            return $.extend(true, settingsSnapshot, invariant);
        ...
        for (var i = 0; i < actions.length; ++ i) {
            ...
            if (typeof(action) === "string") {
                if (action === "setSettings" || action === "restoreSettings") {
                    togo = gpii.lifecycleManager.invokeSettingsHandlers(solutionId, settingsHandlers);
                }
                else {
                    var expanded = sessionState.localResolver(action);
                    var result = gpii.lifecycleManager.invokeAction(action);
                    if (action.name) {
                        sessionState.actionResults[name] = result;
                    }
                }
            }
        }
    ...
    gpii.lifecycleManager.preInit = function (that) {
        ...
        that.start = function (options, solutions, callback) {
            ...
            sessionState.localResolver = function (material) {
                that.variableResolver(material, sessionState.localFetcher);
            };
            ...
            sessionState.solutions = fluid.transform(solutions, function (solution) {
                ...
                togo.settingsHandlers = gpii.lifecycleManager.executeActions(solution.settingsHandlers, solution.lifecycleManager.start, sessionState);
        ...
        that.stop = function (options, callback) {
            ...
            fluid.each(sessionState.solutions, function (solution) {
                gpii.lifecycleManager.executeActions(sessionState.settingsHandlers, solution.lifecycleManager.stop, sessionState);
            });
