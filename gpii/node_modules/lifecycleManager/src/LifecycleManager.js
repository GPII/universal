...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        qssSettings: "@expand:{qssSettingsLoader}.get()",
        components: {
            variableResolver: {
            ...
            qssSettingsLoader: {
                type: "gpii.lifecycleManager.qssSettingsLoader"
            },
            privateMatchMaker: {
        ...
        members: {
            sessionIndex: {}, // map of gpiiKey to session component member name, managed by gpii.lifecycleManager.sessionIndexer
        ...
        listeners: {
            ...
            "onCreate.createActionactionFunctions": "gpii.lifecycleManager.createActionQueueFunctions",
        ...
        invokers: {
            ...
            errorResponse: {
                ...
                args: ["{arguments}.0", "{arguments}.1", "{arguments}.2"]
                       // requestPromise or requestComponent, message, statusCode
            },

            // get the current active GPII key
            getActiveSessionGpiiKeys: {
                funcName: "gpii.lifecycleManager.getActiveSessionGpiiKeys",
                args: "{that}"
            },
            ...
            /** Accepts an array of GPII keys, of which all but the first will currently be ignored. Returns the session
              * component corresponding to that user, if they have an active session. A typical usage pattern is to call
              * "getActiveSessionGpiiKeys" and send its return to "getSession"
              */
            getSession: {
            ...
            processStart: { // should not be used directly, use the manually created 'start' invoker instead
                ...
                args: [ "{that}", "{that}.options.qssSettings", "{arguments}.0"]
            },
    ...
    gpii.lifecycleManager.performProximityTriggered = function (that, gpiiKey) {
        ...
        var activeGpiiKeys = that.getActiveSessionGpiiKeys();
     ...
     */
    gpii.lifecycleManager.errorResponse = function (togo, message, statusCode) {
        var error = {
    ...
    gpii.lifecycleManager.applyPreferences = function (that, preferences, prefsAppliedLocalEvent) {
        ...
        var session = that.getSession(that.getActiveSessionGpiiKeys());

        // tweak payload to be in preferences set format and update session
        var contextualPrefs = {};
        fluid.each(preferences, function (spec, pref) {
            fluid.set(contextualPrefs, fluid.pathUtil.parseEL(pref), spec.value);
        });

        // Only the user updated preferences will be saved to cloud. This case is identified by firing the movel change request with source === "userUpdate".
        // Note that changes requests on model.preferences can also be triggered by initial user key-in and the context manager.
        ...
        // activeConfiguration doesn't need to be updated at preference change because its initial value has had all
        // configuration for user preferences and QSS supported preferences
     ...
     * @param {Component} that - An instance of gpii.lifecycleManager.
     * @return {Array} The user GPII key.
     */
    gpii.lifecycleManager.getActiveSessionGpiiKeys = function (that) {
        return fluid.keys(that.sessionIndex);
    };

    gpii.lifecycleManager.getSession = function (that, gpiiKeys) {
        gpiiKeys = fluid.makeArray(gpiiKeys);
        if (gpiiKeys.length === 0) {
            fluid.fail("Attempt to get sessions without keys");
        } else {
            var memberKey = that.sessionIndex[gpiiKeys[0]];
    ...
    /**
    * @param {Component} that - The lifecycleManager component
    ...
    */
    ...
    gpii.lifecycleManager.restoreSnapshot = function (that, originalSettings) {
        // TODO: document/ensure that this GPII key, as well as the special "reset", is reserved
    ...
    gpii.lifecycleManager.processStop = function (that, options) {
        var gpiiKey = options.gpiiKey;
        var session = that.getSession([gpiiKey]);
        if (!session) {
    ...
    gpii.lifecycleManager.processUpdate = function (that, finalPayload) {
        ...
        var session = that.getSession([gpiiKey]);
        if (!session) { // if user has logged out since the update was added to action queue
    ...
    gpii.lifecycleManager.processStart = function (that, qssSettings, finalPayload) {
        var gpiiKey = finalPayload.gpiiKey,
            userPreferences = finalPayload.preferences,
            lifecycleInstructions = finalPayload.activeConfiguration.lifecycleInstructions,
            promiseTogo = fluid.promise();

        if (that.sessionIndex[gpiiKey]) {
            var failPromise = fluid.promise();
        ...
        var session = that[that.sessionIndex[gpiiKey]];

        // Get payload for combined user and QSS supported preferences
        var userSettings = fluid.get(userPreferences, ["contexts", finalPayload.activeContextName, "preferences"]);
        var analyzedResult = gpii.lifecycleManager.analyzeCombinedSettings(userSettings, qssSettings);
        var combinedPrefs = {};
        fluid.set(combinedPrefs, ["contexts", finalPayload.activeContextName, "preferences"], analyzedResult.combinedSettings);
        var payloadForCombinedPrefsPromise = that.privateMatchMaker.matchMaking(gpiiKey, combinedPrefs);

        payloadForCombinedPrefsPromise.then(function (payloadForCombinedPrefs) {
            ...
            var filteredPayload = fluid.filterKeys(finalPayload, ["gpiiKey", "preferences", "activeContextName"]);
            filteredPayload = fluid.extend(filteredPayload, {
                "qssCombinedPreferences": combinedPrefs,
                "activeConfiguration": payloadForCombinedPrefs.activeConfiguration,
                "solutionsRegistryEntries": payloadForCombinedPrefs.solutionsRegistryEntries,
                "matchMakerOutput": payloadForCombinedPrefs.matchMakerOutput,
                "provenance": analyzedResult.provenance
            });
            ...
            // original state.
            var tasks = [];
            fluid.each(payloadForCombinedPrefs.lifecycleInstructions, function (solution, solutionId) {
            // fluid.each(lifecycleInstructions, function (solution, solutionId) {
                tasks.push(function () {
                    if (!fluid.isDestroyed(that)) { // See above comment for GPII-580
                        // check the current state of the solution to decide whether we should run the
            ...
            var tasksPromise = fluid.promise.sequence(tasks);
            fluid.promise.follow(tasksPromise, promiseTogo);
        }, function (err) {
            fluid.log("Error at fetching payload for combined preferences via private match maker: " + err.message);
            promiseTogo.reject("Error at fetching payload for combined preferences via private match maker: " + err.message);
        });
        return promiseTogo;
    };

    /** Combine user preferences and QSS supported preferences as well as generating a provenance object tracking the preference
     * source.
     *
     * @param {Object} userSettings - The user settings.
     * @param {Object} qssSettings - The QSS supported settings.
     * @return {Object} The output object includes top-level members:
     *     - {Object} combinedSettings - A combined settings that add QSS supported settings if they are not in user settings
     *     - {Object} provenance - A filtered skeleton of the user's preferences document, just containing the names of `contexts.
     */
    gpii.lifecycleManager.analyzeCombinedSettings = function (userSettings, qssSettings) {
        var combinedSettings = fluid.extend(qssSettings, userSettings);

        var userSettingsProvenance = fluid.transform(userSettings, function () {
            return "preferences";
        });
        var qssSettingsProvenance = fluid.transform(qssSettings, function () {
            return "snapshot";
        });

        var provenance = fluid.extend(qssSettingsProvenance, userSettingsProvenance);
        return {
