...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        events: {
            createSession: null,           // fired with [gradeName, gpiiKey] - times creation of the session
        ...
        listeners: {
            "createSession.relay": {
                ...
                listener: "{lifecycleManager}.events.onSessionStart.fire"
            },
    ...
    gpii.lifecycleManager.performLogin = function (that, gpiiKey) {
        ...
        // 1. The current active key is "noUser";
        // 2. The last logon request in the queue is to key in "noUser". This case sometimes occurs when a key in
        // request is issued on the system start that the initial "noUser" keyin is still in progress or is waiting
        // in the logon request queue to be processed.
        if (activeGpiiKeys[0] === "noUser" || lastLogonRequest && lastLogonRequest.gpiiKey === "noUser" && lastLogonRequest.logonState === "login") {
            that.addToUserLogonRequestQueue({
                gpiiKey: "noUser",
                logonState: "logout"
            });
    ...
    gpii.lifecycleManager.applyPreferences = function (that, preferences, prefsAppliedLocalEvent) {
        ...
        var previousPrefs = fluid.get(session.model.preferences, ["contexts", session.model.activeContextName, "preferences"]);
        ...
        // part of session.model.preferences:
        // 1. When there are new preferences, preform a round of local matchmaking and use its result to apply preferences;
        // 2. When there aren't new preferences, use the previous matchmaking result from session.model to apply preferences.
        gpii.lifecycleManager[hasNewPrefs ? "applyWithNewPrefs" : "applyExistingPrefs"](that, session, contextualPrefs, prefsAppliedLocalEvent);
    };
    ...
    gpii.lifecycleManager.applyWithNewPrefs = function (that, session, contextualPrefs, prefsAppliedLocalEvent) {
        ...
        var matchMakingPromise = that.privateMatchMaker.doMatch(activeGpiiKeys[0], updatedPrefSet);
    ...
    gpii.lifecycleManager.getActiveSessionGpiiKeys = function (that) {
        // TODO: Temporary hack until https://issues.gpii.net/browse/GPII-3594 is resolved
        var rawKeys = fluid.keys(that.sessionIndex);
        if (rawKeys.indexOf("noUser") !== -1 && rawKeys.length > 1) {
            return rawKeys.filter(function (entry) { return entry !== "noUser"; });
        }
        else {
            return rawKeys;
        }
    ...
    */
    ...
    gpii.lifecycleManager.restoreSnapshot = function (that, originalSettings) {
        ...
        that.events.createSession.fire("gpii.lifecycleManager.restoreSession", "restore");
    ...
    gpii.lifecycleManager.processStart = function (that, finalPayload) {
        ...
        that.events.createSession.fire("gpii.lifecycleManager.userSession", gpiiKey);
