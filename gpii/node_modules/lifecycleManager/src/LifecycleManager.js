...
(function () {
    ...
    */
     ...
     * payload which is output from the matchmaking process.
     *
     ...
     * @param {String} [forcePrefsSetName] - [optional] A preferences set name to be selected.
     * @return {Object} The modified fullPayload, now including activePrefsSetName, activeConfiguration
     */
    gpii.lifecycleManager.updateActivePrefsSetName = function (that, fullPayload, forcePrefsSetName) {
        var newActivePrefsSetName = "gpii-default";
        if (forcePrefsSetName === undefined) {
            if (fullPayload.forcePrefsSetName) { // Don't use stored if name has been forced
                newActivePrefsSetName = fullPayload.activePrefsSetName;
            } else {
                var activeSession = that.getSession();
                if (activeSession.model.activePrefsSetName) {
                    newActivePrefsSetName = activeSession.model.activePrefsSetName;
                }
            }
        } else {
            newActivePrefsSetName = forcePrefsSetName;
            fullPayload.forcePrefsSetName = true;
        }
        fullPayload.activePrefsSetName = newActivePrefsSetName;
        return fullPayload;
    };
     ...
     * 2) if the newly calculated active prefsSet is different from the currently applied prefsSet,
     *    the LifecycleManager's update functionality will be called and the applied prefsSet and
     *    configuration will be stored in the active session of the LifecycleManager.
     *
     * @param {Component} that - The lifecycleManager component from which the current
     *    session will be retrieved
     * @param {String} [forcePrefsSetName] - [optional] A prefsSet name to be unconditionally selected
     ...
     **/
    gpii.lifecycleManager.prefsSetNameChanged = function (that, forcePrefsSetName) {
        ...
        // find and update preferences set name:
        that.updateActivePrefsSetName(fullPayload, forcePrefsSetName);

        if (oldPrefsSetName === fullPayload.activePrefsSetName) {
            fluid.log(fluid.logLevel.TRACE, "lifecycleManager: Same preferences set as before (" + oldPrefsSetName + ") so doing nothing");
            return;
        }
        fluid.log("lifecycleManager: Context changed from old context \"", oldPrefsSetName, "\" to new active context \"" + fullPayload.activePrefsSetName, "\" for the GPII key: ", activeGpiiKey);
