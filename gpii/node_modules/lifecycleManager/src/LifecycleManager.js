...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        model: {
            logonChangeInProgressFor: undefined, // will have the value of the user token
            lastLogonChange: {
                userToken: undefined,
                timestamp: 0
            }
        ...
        invokers: {
            ...
            logonChangeStarted: {
                funcName: "gpii.lifecycleManager.onLogonChangeStarted",
                args: ["{that}", "{arguments}.0"] // user token
            },
            logonChangeComplete: {
                funcName: "gpii.lifecycleManager.onLogonChangeComplete",
                args: ["{that}", "{arguments}.0"] // user token
            },
    ...
    // triggered via events on the flowManager. Will record the fact that a logon change started (i.e. login or logout)
    gpii.lifecycleManager.onLogonChangeStarted = function (that, userToken) {
        that.applier.change("logonChangeInProgressFor", userToken);
    };

    // triggered via events on the flowManager. Will record the fact that a logon change has completed (i.e. login or logout)
    gpii.lifecycleManager.onLogonChangeComplete = function (that, userToken) {
        that.applier.change("logonChangeInProgressFor", undefined, "DELETE");
        that.applier.change("lastLogonChange", {
            userToken: userToken,
            timestamp: Date.now()
        });
    };
