...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        invokers: {
            ...
            readPreference: {
                funcName: "gpii.lifecycleManager.readPreference",
                args: ["{that}", "{arguments}.0"]
                                 // preferences
            },
            ...
            getContextedPreferences: {
                funcName: "gpii.lifecycleManager.getContextedPreferences",
                args: ["{arguments}.0", "{arguments}.1"]
                       // contextualPrefs, contextName
            },
    ...
    gpii.lifecycleManager.applyWithNewPrefs = function (that, session, contextualPrefs, prefsAppliedLocalEvent) {
        ...
        matchMakingPromise.then(function (newPayload) {
            ...
            fluid.set(fullPayload, ["currentPreferences"], gpii.lifecycleManager.getContextedPreferences(fullPayload.preferences, fullPayload.activeContextName));
     ...
     * }
     */
    gpii.lifecycleManager.readPreference = function (that, preference) {
        fluid.log("Lifecycle Manager: Received preference to read ", preference);
        ...
        var prefSet = {};
        fluid.each(preference, function (spec, pref) {
            var prefPath = fluid.pathUtil.parseEL(pref);
            fluid.set(contextualPrefs, prefPath, spec.value);
        });
        fluid.set(prefSet, ["contexts", activeContextName, "preferences"], contextualPrefs);

        // Perform a local matchmaking to get matched solutions for the preference.
        var matchMakingPromise = that.privateMatchMaker.doMatch("readSetting", prefSet);
        matchMakingPromise.then(function (payload) {
            fluid.set(payload, ["activeContextName"], activeContextName);
            fluid.set(payload, ["preferences"], prefSet);
            ...
            settingsPromise.then(function (allSettingValues) {
                ...
                // For settings that have "noMatch" defined in the "capabilitiesTransformations" blocks,
                // the matchmaking process will always return them as a matched settings. This step is to
                // remove any of them that the requested preference is not interested.
                var removeNoMatchSettings = gpii.lifecycleManager.removeNoMatchSettings(matchMakerSettings, contextualPrefs, matchMakerSolutionRegistryEntries);

                // Transform settings to preferences
                var transformed = gpii.lifecycleManager.transformSettingsToPrefs(removeNoMatchSettings, matchMakerSolutionRegistryEntries);

                var updatedSessionModel = {
                    currentPreferences: fluid.extend(true, activeSession.model.currentPreferences, transformed),
                    readPreferenceCount: activeSession.model.readPreferenceCount + 1
                };
                activeSession.applier.change("", updatedSessionModel);
            });
        });
    };
     ...
     * @param {Object} contextualPrefs - A contextual preferences.
    ...
    gpii.lifecycleManager.getContextedPreferences = function (contextualPrefs, contextName) {
        var prefs = fluid.get(contextualPrefs, ["contexts", contextName, "preferences"]);
        return prefs || {};
    };
    ...
    */
    ...
    gpii.lifecycleManager.processStart = function (that, userSession, finalPayload) {
        ...
        fluid.set(finalPayload, ["currentPreferences"], gpii.lifecycleManager.getContextedPreferences(finalPayload.preferences, finalPayload.activeContextName));
        var filteredPayload = fluid.filterKeys(finalPayload, ["gpiiKey", "preferences", "activeContextName", "activeConfiguration", "solutionsRegistryEntries", "matchMakerOutput", "currentPreferences"]);
