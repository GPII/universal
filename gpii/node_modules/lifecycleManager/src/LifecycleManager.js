...
(function () {
        ...
        invokers: {
            ...
            applySolution: {
                ...
                args: ["{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2", "{arguments}.3", "{arguments}.4"]
                                  // solution, solutionId,  solutionRecord, actions, sessionState
            },
            executeActions: {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    // Argument 1 (toSnapshot) is the structure which will be modified to form the snapshot record (and stashed in sessionState). This variable
    //      will be null if we're doing a login (in which case we should not snapshot)
    ...
    // as a side-effect
    gpii.lifecycleManager.applySolution = function (that, toSnapshot, solutionId, solutionRecord, actions, sessionState) {
        ...
        promise.then(function (snapshot) {
            ...
            if (!isUpdate) {
                ...
                sessionState.solutionsSnapshot[solutionId] = toSnapshot;
            }
        });
        return promise;
    };
    ...
    gpii.lifecycleManager.stop = function (that, requestProxy, options, callback) {
        ...
        var sessionState = that.activeSessions[token];
        if (!sessionState) {
            callback(false);
            return;
        }

        var promises = fluid.transform(sessionState.solutionsSnapshot, function (solution, solutionId) {
        ...
        sequence.then(function () {
            delete that.activeSessions[token];
            callback(true);
        }, function (error) {
    ...
    gpii.lifecycleManager.update = function (that, requestProxy, options, solutions, callback) {
        ...
        var sessionState = that.activeSessions[token];
        if (!sessionState) {
            fluid.fail("User with token ", token, " has no active session");
        }
        ...
        fluid.each(solutions, function (solution, solutionId) {
            ...
            promises.push(that.applySolution(null, solutionId, sol, actions, sessionState));
        });
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, options, solutions, callback) {
        ...
        sessionState.localFetcher = gpii.combineFetchers(
            gpii.resolversToFetcher({token: token}),
        ...
        // This "start" action builds up a payload in sessionState.solutionsSnapshot which is exactly isomorphic with the one
        ...
        sessionState.solutionsSnapshot = {};
        var tasks = [];
        fluid.each(solutions, function (solution, solutionId) {
            tasks.push(function () {
                // build structure for returned values (for later reset)
                var togo = fluid.copy(solution);
                return that.applySolution(togo, solutionId, solution, solution.lifecycleManager.start, sessionState);
            });
        });
        that.activeSessions[token] = sessionState;
