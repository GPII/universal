...
(function () {
    ...
    */
     ...
     * suitable for being sent to executeSettingsAction, as well as at the corresponding point during the "journal restore" operation
     ...
     * @return {Object} The rejected payload formed by shallow cloning the supplied argument (if it is not an `Error`) and suffixing
     *     its `message` member
     */
     ...
     */
    // TODO: In theory this is a fairly generic promise algorithm, but in practice there are some further subtleties - for example,
    // a better system would instead replace fluid.promise.sequence with a version that allowed some form of "soft rejection" so
    // that there might be a chance to signal failures to the user.
            ...
            if (typeof(action) === "string") {
                ...
                if (action.indexOf("settings.") === 0) {
                    ...
                    if (action === "start" || action === "configure") {
                        ...
                        var unarmoured = gpii.lifecycleManager.transformSolutionSettings(session.model.originalSettings[solutionId],
                            gpii.settingsHandlers.changesToSettings);
     ...
     * @param lifecycleBlockKeys {Array} Array of ordered strings denoting which lifecycle blocks to run (supported values here are
     *     "configure", "start" and/or "update")
     * @param rootAction {String} Either "start", "update" or "stop" depending on the lifecycleManager phase which is executing
     * @return {Promise} The same promise yielded by executeActions - the stateful construction of the session state is tacked onto
     * this promise as a side-effect
     */
