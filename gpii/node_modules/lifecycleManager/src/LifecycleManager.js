...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        members: {
            ...
            queue: [], // queue for high-level lifecycle manager tasks
        ...
        invokers: {
            ...
            stop: {
                func: "{that}.addToQueue",
                args: [ { func: "{that}.processStop", args: "{arguments}.0" } ]
            },
            start: {
                func: "{that}.addToQueue",
                args: [ { func: "{that}.processStart", args: "{arguments}.0" } ]
            },
            update: {
                func: "{that}.addToQueue",
                args: [ { func: "{that}.processUpdate", args: "{arguments}.0" } ]
            },
            applySolution: {
    ...
    */
     ...
     * Each item in the queue should have the format: { func: <functionToCall>, args: <Arguments> }
     * Where <functionToCall> is a function that returns a promise for when it's complete (including side-effects)
     * and the arguments are the arguments that should be passed to the function.
    ...
    gpii.lifecycleManager.processQueue = function (that, queue) {
        ...
        var item = queue.shift();
        var promise = item.func(item.args);
        ...
        promise.then(function (val) {
            ...
            fluid.log(fluid.logLevel.ERROR, "An error occurred in an item of the lifecyclemanagers queue" +
                "(" + item.func + "), so clearing queue. Error was", error);
            item.promise.reject();
    ...
    gpii.lifecycleManager.processStart = function (that, finalPayload) {
        ...
        if (that.sessionIndex[userToken]) {
            var err = "User already logged in when processing start item in queue. Aborting";
