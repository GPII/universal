...
gpii.lifecycleManager.userLogonRequest.handleRequest = function (that, lifecycleManager, logonState) {
    ...
    if (logonState === "logout") {
        if (activeGpiiKey !== gpiiKey) {
            var ignoreUserErrors = activeGpiiKey === "noUser" ? true : false;
            lifecycleManager.errorResponse(that, "Got logout request from user " + gpiiKey + ", but the user " +
                activeGpiiKey + " is logged in. So ignoring the request.", 409, ignoreUserErrors);
        } else {
...
gpii.lifecycleManager.endProcessOneUserLogonRequest = function (lifecycleManager) {
    ...
    // "noUser" keyin into a separate process. The request failure needs to be further investigated.
    fluid.invokeLater(function () {
        ...
        if (queue.length > 0) {
            ...
            if (!activeGpiiKey) {
                ...
                lifecycleManager.events.onQueueEmpty.fire(activeGpiiKey);
            }
        }
    });
};
