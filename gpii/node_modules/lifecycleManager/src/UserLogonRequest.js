...
gpii.lifecycleManager.userLogonRequest.handleRequest = function (that, lifecycleManager, logonState) {
    ...
    // when the the current active key is "noUser".
    if (gpiiKey !== "noUser") {
        gpii.lifecycleManager.userLogonHandling.updateLastNonNoUserLogon(lifecycleManager, gpiiKey, logonState);
    }

    if (gpiiKey === "reset") {
        that.handleResetGpiiKey();
    } else if (logonState === "login") {
