...
gpii.lifecycleManager.userLogonRequest.handleRequest = function (that, lifecycleManager, logonState) {
    ...
    if (logonState === "getGpiiKey") {
        that.withSession(function (session, gpiiKeys) {
            ...
            that.events.onSuccess.fire(gpiiKeys);
...
gpii.lifecycleManager.userLogonRequest.handlerGrades = {
    ...
    getGpiiKey: "gpii.lifecycleManager.getGpiiKeyRequest"
};
...
// A variant of user logon request to handle getGpiiKey requests.
fluid.defaults("gpii.lifecycleManager.getGpiiKeyRequest", {
    gradeNames: [
        "gpii.lifecycleManager.userLogonRequest",
        "gpii.lifecycleManager.sessionAware"
    ],
    listeners: {
        "onCreate.getGpiiKey": {
            listener: "{that}.handleRequest",
...
gpii.lifecycleManager.processUserLogonRequestQueue = function (that, queue) {
    // Pick the first item and process. This item will be removed from the queue when it's finished processing.
    var item = queue[0];
...
gpii.lifecycleManager.endProcessOneUserLogonRequest = function (lifecycleManager) {
    ...
    // Remove the first item from the queue when its processing is finished.
    queue.shift();
    ...
    if (queue.length > 0) {
        ...
        // Use setTimeout() to work around the issue that kettle reports the error "Failing to resume callback for request xxx
        // which has already concluded". The request that fails to resume callback is the user logon request that the current
        // "endProcessOneUserLogonRequest" callback is attached with. It appears that starting the process of "noUser" keyin
        // before the current callback completes will trigger this error.
        setTimeout(function () {
            var activeGpiiKeys = lifecycleManager.getActiveSessionGpiiKeys(); // find currently logged in user:
