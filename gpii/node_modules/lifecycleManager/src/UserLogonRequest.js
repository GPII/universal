...
gpii.lifecycleManager.userLogonRequest.handleRequest = function (that, lifecycleManager, logonState) {
    ...
    // keep track of the last non system default GPII key for /proximityTriggered endpoint to verify its debounce rule
    // even when the the current active key is the system default GPII key.
    if (gpiiKey && gpiiKey !== gpii.defaultGpiiKey) {
    ...
    if (logonState === "login") {
        // if another user, excluding the system default GPII key, is already logged in
        if (activeGpiiKeys.length === 1 && activeGpiiKeys[0] !== gpii.defaultGpiiKey) {
    ...
    if (logonState === "logout") {
        if (activeGpiiKeys[0] !== gpiiKey) {
            var ignoreUserErrors = activeGpiiKeys[0] === gpii.defaultGpiiKey ? true : false;
...
gpii.lifecycleManager.userLogonRequest.handleFailedRequest = function (that, userErrorEvent, err) {
    ...
    if (gpii.userErrors.isConnectionError(err.code)) {
        userErrorEvent.fire({
            isError: true,
...
gpii.lifecycleManager.endProcessOneUserLogonRequest = function (lifecycleManager) {
    ...
    // the system default GPII key keyin before the current callback completes triggers this error. fluid.invokeLater()
    // helps to pull the system default GPII key keyin into a separate process. The request failure needs to be furthur
    // investigated.
    fluid.invokeLater(function () {
        ...
        if (queue.length > 0) {
            ...
            // if no actual user is logged in, login with the system default GPII key
            var activeGpiiKeys = lifecycleManager.getActiveSessionGpiiKeys(); // find currently logged in user:
            ...
            if (activeGpiiKeys && activeGpiiKeys.length === 0) {
                fluid.log("No active keys in GPII. Key in " + gpii.defaultGpiiKey);
