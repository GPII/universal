...
(function () {
    ...
    /** A mixin request grade for requests which require an active user session to do their work. Exposes a method
      ...
      */
    fluid.defaults("gpii.lifecycleManager.sessionAware", {
        invokers: {
            withSession: {
                funcName: "gpii.lifecycleManager.sessionAware.withSession",
                args: ["{that}", "{lifecycleManager}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
            }
        }
    });

    gpii.lifecycleManager.sessionAware.withSession = function (that, lifecycleManager, onSuccess, failMessage, onError) {
