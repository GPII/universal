...
gpii.lifecycleManager.userLogonHandling.handleResetGpiiKey = function (that, lifecycleManager) {
    ...
    // of using the "reset" key is to have the system default GPII key log back in the system.
...
gpii.lifecycleManager.userLogonHandling.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
    ...
    if (that.options.defaultSnapshot && lifecyclePayload.gpiiKey !== gpii.defaultGpiiKey) {
        ...
        fluid.set(lifecyclePayload, ["activeConfiguration", "lifecycleInstructions"], fluid.extend(
            true, {}, that.options.defaultSnapshot, fluid.get(lifecyclePayload, ["activeConfiguration", "lifecycleInstructions"])
        ));
    }
...
gpii.lifecycleManager.getDeviceContext = function (deviceReporterDataSource, event, errorEvent) {
    ...
    promise.then(function (deviceData) {
        ...
        if (!deviceData) {
            errorEvent.fire({message: "Device reporter returned `undefined`." });
...
gpii.lifecycleManager.untrusted.getSettings = function (settingsDataSource, gpiiKey, deviceReporterData, onMatchDone, onError) {
    // Unnecessary to fetch settings for the system default GPII key from the cloud
    var defaultGpiiKeySettings = {
    ...
    var settings = gpiiKey === gpii.defaultGpiiKey ? fluid.promise().resolve(defaultGpiiKeySettings) : settingsDataSource.get(gpiiKey, deviceReporterData);
