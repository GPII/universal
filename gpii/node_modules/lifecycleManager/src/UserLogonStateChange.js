...
gpii.lifecycleManager.userLogonHandling.handleResetGpiiKey = function (that, lifecycleManager) {
    ...
    var activeGpiiKey = activeGpiiKeys[0];
...
gpii.lifecycleManager.userLogonHandling.errorResponse = function (that, message, statusCode) {
...
/**
 * Updates the `lastNonNoUserLogon` portion of the lifecycle manager model.
 *
 ...
 */
gpii.lifecycleManager.userLogonHandling.updateLastNonNoUserLogon = function (lifecycleManager, gpiiKey, logonType) {
    lifecycleManager.applier.change("lastNonNoUserLogon", {
...
gpii.lifecycleManager.userLogonHandling.logoutUser = function (that, gpiiKey) {
...
gpii.lifecycleManager.getDeviceContext = function (deviceReporterDataSource, event, errorEvent) {
...
fluid.defaults("gpii.lifecycleManager.userLogonHandling.stateChangeHandler", {
    invokers: {
        getDeviceContext: { // TODO: remember to close GPII-1770 now this is correctly factored
            ...
            args: ["{flowManager}.deviceReporterDataSource", "{that}.events.onDeviceContext", "{that}.events.onError"]
        },
        startLifecycle: {
        ...
        errorResponse: {
            funcName: "gpii.lifecycleManager.userLogonHandling.errorResponse",
...
// This grade communicate with GPII Cloud to read settings and write preferences from the Cloud.
...
gpii.lifecycleManager.untrusted.getSettings = function (settingsDataSource, gpiiKey, deviceReporterData, onMatchDone, onError) {
    ...
    var settings = gpiiKey === "noUser" ?
        fluid.promise().resolve(noUserSettings) :
        settingsDataSource.get(gpiiKey, deviceReporterData);
