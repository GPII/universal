...
gpii.lifecycleManager.userLogonHandling.handleResetGpiiKey = function (that, lifecycleManager) {
    ...
    if (that.defaultSnapshot) {
        fluid.log("Reset: restoring the system to the default snapshot: ", that.defaultSnapshot);
        resetActions.push(lifecycleManager.restoreSnapshot(that.defaultSnapshot));
    }
...
gpii.lifecycleManager.userLogonHandling.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
    ...
    if (that.defaultLifecycleInstructions && lifecyclePayload.gpiiKey !== "noUser") {
        ...
        fluid.set(lifecyclePayload, ["activeConfiguration", "lifecycleInstructions"], fluid.extend(
            true, {}, that.defaultLifecycleInstructions, fluid.get(lifecyclePayload, ["activeConfiguration", "lifecycleInstructions"])
        ));
    }
...
fluid.defaults("gpii.lifecycleManager.userLogonHandling.stateChangeHandler", {
    members: {
        // Distributed from "gpii.flowManager.local". containing data for reset actions.
        defaultSnapshot: null,
        defaultLifecycleInstructions: null
    },
    invokers: {
        getDeviceContext: {
