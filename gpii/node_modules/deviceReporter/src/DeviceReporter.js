...
fluid.defaults("gpii.deviceReporter.live", {
    ...
    invokers: {
        get: {
            ...
            args: ["{arguments}.0", "{deviceReporter}", "{flowManager}.options.fullSolutionsRegistry"]
        }
    }
});

gpii.deviceReporter.live.get = function (request, deviceReporter, fullSolutionsRegistry) {
    var installedSolutions = [];
    var solutionRegistryEntries = fullSolutionsRegistry[deviceReporter.platformReporter.reportPlatform().id];
    fluid.each(solutionRegistryEntries, function (entry, entryId) {
