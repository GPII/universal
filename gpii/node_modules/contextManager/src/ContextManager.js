...
/**
 * Responsible for weeding out extra information from an object that contains
 * environment properties possibly mixed with other information, and then
 * notifying the applier of the new environment.
 *
 * @param {Component} that - The context manager component
 * @param {Object} environment - Object that is expected to contain only key-value pairs
 ...
 **/
gpii.contextManager.updateCurrentEnvironment = function (that, environment) {
    var newEnvironment = fluid.censorKeys(environment, ["timestamp"]);
...
gpii.contextManager.updateActiveContextName = function (that, fullPayload, forceContextName) {
    ...
    if (forceContextName === undefined) {
        if (fullPayload.forcedContext) { // Don't compute conditions if context has been forced
            ...
            //** JS:  Key to whether can get rid of ContextManager model as "currentEnvironment" may be a valid context
            //** JS:  But, so far, without EnvironmentReporter and /environamentChanged endpoint, that.model is always empty.
            debugger;
