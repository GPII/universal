...
(function () {
    ...
    fluid.defaults("kettle.requests.request.handler.environmentChanged", {
        ...
        invokers: {
            handle: {
                ...
                args: ["{flowManager}.contextManager", "{requestProxy}.events.onSuccess", "{request}.req.body"],
                dynamic: true
            }
        }
    });

    gpii.contextManager.environmentChanged = function (contextManager, event, body) {
    ...
    gpii.contextManager.reEvaluateConditions = function (contextManager, flowManager) {
        ...
        if (activeSessions.length === 0) {
			fluid.log("No users to apply new context for");
            return;
        }
        var activeSession = flowManager.lifecycleManager.getSession(activeSessions);
        var newActiveContexts = gpii.contextManager.utils.findActiveContexts(contextManager.currentContext, activeSession.matchedData);
     ...
     */
    gpii.contextManager.evaluateMatch = function (that, matchData, toSession, event) {
        console.log("gpii.contextManager.evaluateMatch called");

        var activeContexts = gpii.contextManager.utils.findActiveContexts(that.currentContext, matchData);
