...
(function () {
    ...
    gpii.contextManager.evaluateMatch = function (that, matchedData, toSession, event) {
	 ...
	 fluid.defaults("kettle.requests.request.handler.getHolaMundo", {
        gradeNames: ["gpii.request.flowManager.sessionAware", "autoInit"],
        invokers: {
            handle: {
                funcName: "gpii.request.flowManager.onGetHolaMundo",
    ...
    gpii.request.flowManager.onGetHolaMundo = function (that, events) {
		fluid.log("chuscando un hola mundo");
		
		/*
		that.withSession(function (session, userTokens) {
		
			// session tiene por algún lado la respuesta del matchmaker real
		
			session
		
            fluid.log("Reporting currently logged in user token(s) as: " + userTokens);
            events.onSuccess.fire(userTokens);
        }, "chusca No user currently logged in to the system");
		*/
		fluid.log("1");
			events.onSuccess.fire({
			isError : false,
			message : "chusca HOLA MUNDO onSuccess"
		});
		
		fluid.log("2");
	events.onError.fire({
			isError : true,
			message : "chusca HOLA MUNDO onError"
		});
		fluid.log("3");
