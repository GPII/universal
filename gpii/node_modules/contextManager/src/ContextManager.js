...
(function () {
    ...
    fluid.defaults("gpii.contextManager", {
        ...
        components: {
            ...
            CASConnector: {
                type: "gpii.contextManager.CASConnector"
            }
        },
        invokers: {
            evaluateMatch: {
            ...
            checkBroadcasts: {
                funcName: "gpii.contextManager.checkBroadcasts",
                args: [ "{that}", "{arguments}.0"]
            }
        },
        modelListeners: {
    ...
    gpii.contextManager.evaluateConditions = function (that, lifecycleManager, context) {
        ...
        var session = lifecycleManager.getSession();
        var newActiveContexts = gpii.contextManager.utils.findActiveContexts(context, session.matchMakerOutput);
        fluid.log("New active contexts: " + newActiveContexts);
        if (newActiveContexts[0] === session.activeContextName) {
            console.log("Same context as before (" + session.activeContextName + ") so doing nothing");
            return;
        }
        var activeConfiguration = session.matchMakerOutput.inferredConfiguration[newActiveContexts[0]];
        var lifecycleInstructions = that.transformer.configurationToSettings(activeConfiguration, session.solutionsRegistryEntries);
        var finalPayload = {
        ...
        lifecycleManager.update(finalPayload, function (response) {
            if (response.success) {
                ...
                $.extend(true, session, finalPayload);
            }
            that.checkBroadcasts(activeConfiguration.metadata);
            fluid.log("Successfully updated configuration triggered by context changes");
    ...
    gpii.contextManager.evaluateMatch = function (that, matchMakerResponse) {
        ...
        that.checkBroadcasts(fluid.get(contextPayload, "activeConfiguration.metadata"));

        return contextPayload;
    };

    gpii.contextManager.checkBroadcasts = function (that, metadata) {
        // check if we should broadcast message:
        fluid.each(metadata, function (entry) {
            if (entry.type === "helpMessage" || entry.type === "infoMessage" || entry.type === "errorMessage") {
                that.events.broadCastMessage.fire(entry.message.en.message, entry.type, { learnMore: entry.message.en.learnMore }, true);
                console.log("Broadcasting " + entry.message.en);
            }
        });
    };
