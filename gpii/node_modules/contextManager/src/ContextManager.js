...
(function () {
    ...
    gpii.contextManager.reEvaluateConditions = function (that, flowManager) {
        ...
        if (flagEvaluatingContext === true) {
            howManyRetries--;
            if (howManyRetries > 0){
                fluid.log("Still applying former change");
            } else {
                fluid.log("Let's try applying changes again");
                flagEvaluatingContext = false;
                howManyRetries = 20;
            }
        ...
        flagEvaluatingContext = true;
        var activeSession = flowManager.lifecycleManager.getSession(activeSessions);
        activeSession.matchedData = gpii.contextManager.utils.expandContext(activeSession.userToken, activeSession.matchedData);
