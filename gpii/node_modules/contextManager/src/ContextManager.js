...
gpii.contextManager.onEnvironmentChanged = function (that, body) {
    ...
    fluid.log(fluid.logLevel.TRACE, "contextManager: Updating context with: " + JSON.stringify(newContext, null, 4));
 ...
 * @context {Object} The model as reported by the changeApplier (ie. the content of
...
gpii.contextManager.evaluateConditions = function (that, lifecycleManager, context) {
    ...
    fluid.log(fluid.logLevel.TRACE, "contextManager: New active contexts: " + newActiveContexts);
    if (newActiveContexts[0] === activeSession.model.activeContextName) {
        fluid.log(fluid.logLevel.TRACE, "contextManager: Same context as before (" + activeSession.model.activeContextName + ") so doing nothing");
        return;
    }
    var activeConfiguration = activeSession.model.matchMakerOutput.inferredConfiguration[newActiveContexts[0]];
    var lifecycleInstructions = that.transformer.configurationToSettings(activeConfiguration, activeSession.model.solutionsRegistryEntries);
    var finalPayload = {
