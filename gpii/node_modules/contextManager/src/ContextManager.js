...
fluid.defaults("gpii.contextManager", {
    ...
    invokers: {
        updateActiveContextName: {
            ...
            args: ["{that}", "{arguments}.0", "{arguments}.1"] // fullPayload, forceContext
        },
        contextChanged: {
            funcName: "gpii.contextManager.contextChanged",
            args: ["{that}", "{lifecycleManager}", "{arguments}.0"] // forceContext
        },
 ...
 *      will not be considered part of the current context
 **/
// TODO: Why do we confusingly have this thing named "context" which is always just a copy of the environment
// with the timestamp removed?
 ...
 * @param forceContext {String} [optional] A context name to be unconditionally selected
 **/
gpii.contextManager.contextChanged = function (that, lifecycleManager, forceContext) {
    ...
    that.updateActiveContextName(fullPayload, forceContext);
 ...
 * @param forceContext {String} [optional] - A context name to be selected, overriding environment conditions
 * @return {Object} - the modified fullPayload, now including activeContextName, activeConfiguration
 */
gpii.contextManager.updateActiveContextName = function (that, fullPayload, forceContext) {
    var matchMakerOutput = fullPayload.matchMakerOutput;
    var newActiveContext;
    if (forceContext === undefined) {
        if (fullPayload.forceContext) { // Don't compute conditions if context has been forced
            newActiveContext = fullPayload.activeContextName;
        } else {
            ...
            newActiveContext = activeContexts[0];
        }
    } else {
        newActiveContext = forceContext;
        fullPayload.forceContext = true;
    }
    fullPayload.activeContextName = newActiveContext;
    // fullPayload.activeConfiguration = {
    //     inferredConfiguration: fluid.extend(true, {}, matchMakerOutput.inferredConfiguration[newActiveContext])
    // };
    fluid.log("activeContextName is calculated to be: ", newActiveContext);
    // console.log("activeConfiguration is now ", JSON.stringify(fullPayload.activeConfiguration, null, 2)); // TOOD KASPER
    return fullPayload;
};
