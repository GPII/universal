...
(function () {
    ...
    gpii.contextManager.tests.contextPayload = $.extend({
    ...
    fluid.tests.evaluateMatchTest = function (contextManager, event) {
        var response = contextManager.evaluateMatch(gpii.contextManager.tests.contextPayload);
        event.fire(response);
    };

    gpii.contextManager.tests.testMatchEvaluatedResponse = function (evaluatedContext) {
        var expected = $.extend(true, {}, gpii.contextManager.tests.contextPayload, {
            activeContextName: "gpii-default",
            activeConfiguration: gpii.contextManager.tests.standardMMOutput.inferredConfiguration["gpii-default"]
        });

        jqUnit.assertDeepEq("checking return from evaluateMatchTest", expected, evaluatedContext);
    };
