...
(function () {
    ...
    var tests = {
        basicTest: {
            ...
            expectedFile: __dirname +
                "/data/INISettingsHandler/test1_expectedOutput.ini",
            set: {
                payload: {
                    "nvda.screenReader": [{
                        options: {
    ...
    fluid.test.INIHandlerSet = function () {
        fluid.each(tests, function (v, k) {
            setTester(k, v);
        });
    };

    fluid.test.INIHandlerGet = function () {
        fluid.each(tests, function (v, k) {
            getTester(k, v);
        });
    };

    fluid.defaults("fluid.test.INISettingsHandler", {
        gradeNames: ["fluid.test.testEnvironment", "autoInit"],
        components: {
            tester: {
                type: "fluid.test.INISettingsHandlerTester"
            }
        }
    });

    fluid.defaults("fluid.test.INISettingsHandlerTester", {
        ...
        modules: [{
            ...
            tests: [{
                expect: 4,
                name: "INIHandler.set",
                func: "fluid.test.INIHandlerSet"
            }, {
                expect: 2,
                name: "INIHandler.get",
                func: "fluid.test.INIHandlerGet"
            }]
        }]
    });

    if (kettle.tests.allTests) {
        module.exports = "fluid.test.INISettingsHandler";
    } else {
        fluid.test.runTests(["fluid.test.INISettingsHandler"]);
    }
