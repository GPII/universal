...
fluid.defaults("gpii.tests.remoteFileSettingsHandler.testCaseHolder", {
    ...
    modules: [{
        ...
        tests: [{
            ...
            name: "setting 1 file, with invokeBefore (resolve)",
            task: "gpii.tests.remoteFileSettingsHandler.testSetting",
            args: [{
                payload: {
                    "gpii.test.remoteFileSetting": [{
                        ...
                        options: {
                            settings: {
                                f1: {
                                    path: "~file1", url: "http://localhost:28531/content/new-content1.%value",
                                    invokeBefore: {
                                        type: "gpii.tests.remoteFileSettingsHandler.testInvoke",
                                        when: "before",
                                        result: "resolve"
                                    }
                                }
                ...
                expectedInvokes: {
                    value1: {apply: {before: true}, restore: {before:true}}
                }
            }],
            resolve: "fluid.identity"
        }, {
            name: "setting 1 file, with invokeBefore (reject)",
            task: "gpii.tests.remoteFileSettingsHandler.testSetting",
            args: [{
                payload: {
                    "gpii.test.remoteFileSetting": [{
                ...
                expectedFiles: {
                    // invokeBefore rejected, content shouldn't be changed.
                    file1: "original-content"
                },
                expectedInvokes: {
                    value1: {apply: {before: true}}
                }
            }],
            resolve: "fluid.identity"
        }, {
            name: "setting 1 file, with invokeAfter (reject)",
            task: "gpii.tests.remoteFileSettingsHandler.testSetting",
            args: [{
                payload: {
                    "gpii.test.remoteFileSetting": [{
                ...
                expectedFiles: {
                    // invokeAfter rejected, content shouldn't be changed (it should have been restored).
                    file1: "original-content"
                },
                expectedInvokes: {
                    value1: {apply: {after: true}}
                }
            }],
            resolve: "fluid.identity"
        }, {
            name: "setting 4 file, all invokeBefore/After combinations",
            task: "gpii.tests.remoteFileSettingsHandler.testSetting",
            args: [{
                payload: {
                    "gpii.test.remoteFileSetting": [{
                        ...
                        options: {
                            settings: {
                                ...
                                f2: {
                                    path: "~file2", url: "http://localhost:28531/content/new-content2.%value",
                                    invokeBefore: {
                                        type: "gpii.tests.remoteFileSettingsHandler.testInvoke",
                                        when: "before"
                                    }
                                ...
                                f3: {
                                    path: "~file3", url: "http://localhost:28531/content/new-content3.%value",
                                    invokeAfter: {
                                        type: "gpii.tests.remoteFileSettingsHandler.testInvoke",
                                        when: "after"
                                    }
                                ...
                                f4: {
                                    path: "~file4", url: "http://localhost:28531/content/new-content4.%value",
                                    invokeBefore: {
                                        type: "gpii.tests.remoteFileSettingsHandler.testInvoke",
                                        when: "before"
                                    },
                ...
                expectedInvokes: {
                    value2: {apply: {before: true}, restore: {before:true}},
                    value3: {apply: {after: true}, restore: {after:true}},
                    value4: {apply: {before: true, after: true}, restore: {before:true, after:true}}
                }
            }],
            resolve: "fluid.identity"
        }, {
...
gpii.tests.remoteFileSettingsHandler.testSetting = function (testData) {
    ...
    // Expect assertions for the invokeBefore and invokeAfter callbacks.
    if (test.expectedInvokes) {
        fluid.each(test.expectedInvokes, function (setting) {
            fluid.each(setting, function (action) {
                jqUnit.expect((action.before ? 1 : 0) + (action.after ? 1 : 0));
            });
        });
    }
...
/**
 * Tests an invocation of invokeBefore and invokeAfter
 * @param {String} when "before" or "after"
 * @param {String} action "apply" or "restore"
 * @param {String} path The path option.
 * @param {String} value The setting value.
 * @param {String|Object} result The result to return, "resolve", "reject", or something else.
 * @return {Promise|Object} The result.
 */
gpii.tests.remoteFileSettingsHandler.testInvoke = function (when, action, path, value, result) {
    var fileContent = fs.readFileSync(path, "utf8");

    var expectOriginal;
    if (when === "before") {
        expectOriginal = action === "apply";
    } else if (when === "after") {
        expectOriginal = action === "restore";
    } else {
        jqUnit.fail("testInvoke called incorrectly", arguments);
    }

    var expectedContent = expectOriginal ? "original-content" : "new-content";

    jqUnit.assertTrue("beforeInvoke should be called before the file change",
        fileContent.indexOf(expectedContent) > -1);

    fluid.set(gpii.tests.remoteFileSettingsHandler.invokedValues, [value, action, when], true);

    var togo;
    if (result === "resolve") {
        togo = fluid.promise().resolve();
    } else if (result === "reject") {
        togo = fluid.promise().reject();
    } else {
        togo = result;
    }
...
fluid.defaults("gpii.tests.remoteFileSettingsHandler.testInvoke", {
