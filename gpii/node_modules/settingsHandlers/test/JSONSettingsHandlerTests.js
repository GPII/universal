...
(function () {
    ...
    gpii.tests.JSONHandlerSet = function () {
        fluid.each(tests, function (v, k) {
            setTester(k, v);
        });
    };

    gpii.tests.JSONHandlerGet = function () {
        fluid.each(tests, function (v, k) {
            getTester(k, v);
        });
    };

    fluid.defaults("gpii.tests.JSONSettingsHandler", {
        gradeNames: ["fluid.test.testEnvironment", "autoInit"],
        components: {
            tester: {
                type: "gpii.tests.JSONSettingsHandlerTester"
            }
        }
    });

    fluid.defaults("gpii.tests.JSONSettingsHandlerTester", {
        ...
        modules: [{
            ...
            tests: [{
                ...
                func: "gpii.tests.JSONHandlerSet"
            }, {
                expect: 2,
                name: "JSONHandler.get",
                func: "gpii.tests.JSONHandlerGet"
            }]
        }]
    });

    if (kettle.tests.allTests) {
        module.exports = "gpii.tests.JSONSettingsHandler";
    } else {
        fluid.test.runTests(["gpii.tests.JSONSettingsHandler"]);
    }
