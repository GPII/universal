...
jqUnit.test("Checking wsSettingsHandler object before doing anything", function () {
    ...
    jqUnit.assertTrue("There are an object to store the clients",
                      wsSettingsHandlers.options.clients);
...
jqUnit.test("Checking wsSettingsHandler's get/set methods", function () {
    jqUnit.assertDeepEq("There aren't settings yet for 'net.gpii.test'",
                        ...
                        gpii.resolveSync(wsSettingsHandlers.get(webSocketsTests.payloads.getPayload, wsSettingsHandlers)));

    var setReturnPayload = gpii.resolveSync(wsSettingsHandlers.set(webSocketsTests.payloads.setPayload, wsSettingsHandlers));
    jqUnit.assertDeepEq("Set function returns the expected payload",
                        webSocketsTests.payloads.expectedSetReturnPayload,
                        setReturnPayload);

    var restoreReturnPayload = gpii.resolveSync(wsSettingsHandlers.set(webSocketsTests.payloads.restorePayload));
...
jqUnit.test("Checking wsSettingsHandler's add/remove methods", function () {
    ...
    jqUnit.assertDeepEq("Check that the client has been added", webSocketsTests.clients.clientOneExpected, wsSettingsHandlers.options.clients[webSocketsTests.clients.clientOne[0]]);

    wsSettingsHandlers.removeClient(webSocketsTests.clients.clientOne[1]);
    jqUnit.assertDeepEq("Check that the client has been removed", undefined, wsSettingsHandlers.options.clients[webSocketsTests.clients.clientOne[0]]);

    jqUnit.assertDeepEq("Check that the client object is clear", {}, wsSettingsHandlers.options.clients);
