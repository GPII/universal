...
(function () {
        ...
        basicTest: {
            ...
            get: {
                payload: {
                    "nvda.screenReader": [{ 
                        options: {
                            "path": __dirname + "/data/INISettingsHandler/test1_default.ini"
                        },
                        settings: {
        ...
        complexTest: {
            ...
            get: {
                payload: {
                    "nvda.screenReader": [{ 
                        options: {
                            "path": __dirname + "/data/INISettingsHandler/test2_default.ini",
                            "allowHashComments": true,
                            "allowSubSections": true
                        },
                        settings: {
    ...
    var setTester = function (name, test) {
            ...
            // test return payload
            var returnPayload = gpii.settingsHandlers.INISettingsHandler.set(test.set.payload);
            jqUnit.assertDeepEq("Checking return payload for "+name+" INI file: ", test.set.expectedResult, returnPayload);
    ...
    var getTester = function (name, test) {
            var returnPayload = gpii.settingsHandlers.INISettingsHandler.get(test.get.payload);
            jqUnit.assertDeepEq("Checking return payload for "+name+" INI file: ", test.get.expectedResult, returnPayload);
    };

    INIHandlerTester.test("INIHandler.get", function () {
        fluid.each(tests, function (v, k) {
            getTester(k, v);
        });
    });
