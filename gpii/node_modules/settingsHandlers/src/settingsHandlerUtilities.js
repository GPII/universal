...
gpii.settingsHandlers.handleFileSolutionEntry = function (solutionEntry, modifier, parser, isWrite) {
    ...
    var createFile = options.createFile;
    //create file, if file does not exist, and set initial content of the file
    if (createFile) {
        var exists = fs.existsSync(options.filename);
        if (!exists) {
            fs.writeFileSync(options.filename, initialContent, options.encoding || "utf-8");
        }
    }
