...
// This function is similar to numberify, but this function also converts all
// leaf values to numbers which may validly be.
gpii.settingsHandlers.normalize = function (payload) {
    if (typeof(payload) === "string") {
        if (isFinite(payload)) {
        ...
        return fluid.transform(payload, function (value) {
            return gpii.settingsHandlers.normalize(value);
        });
    }
};
...
// An utility to fix the number of decimals to represent a real number.
// This function apply the same criteria in every value from a given payload.
gpii.settingsHandlers.fixDecimals = function (payload) {
    if (typeof(payload) === "number") {
        return payload.toFixed(4);
    } else if (fluid.isPrimitive(payload)) {
        return payload;
    } else {
        return fluid.transform(payload, function (value) {
            return gpii.settingsHandlers.fixDecimals(value);
        });
    }
};
...
gpii.settingsHandlers.comparePayloads = function (payload1, payload2) {
    var settings1 = gpii.settingsHandlers.fixDecimals(gpii.settingsHandlers.extractSettingsBlocks(payload1));
    var settings2 = gpii.settingsHandlers.fixDecimals(gpii.settingsHandlers.normalize(gpii.settingsHandlers.extractSettingsBlocks(payload2)));
