...
gpii.settingsHandlers.transformFilePayload = function (payload, modifier, parser, isWrite) {
    ...
    return fluid.transform(returnObj, function (solution) {
        return fluid.transform(solution, function(solutionEntry) {
            return gpii.settingsHandlers.handleFileSolutionEntry(solutionEntry,
...
gpii.settingsHandlers.invokeSettingsHandler = function (handler, payload) {
    var worklist = [];
    var response = fluid.transform(payload, function (oneSolution, solutionPath) {
    ...
    sequence.then(function (resolved) {
        fluid.each(resolved, function (oneResolve, i) {
            var combined = $.extend(true, { settings: oneResolve}, worklist[i].others);
