 ...
 *
 * @param settings {Object} Object in which each key is a setting name that is to be filtered. This
 *     object will be modified.
 * @param supportedSettings {Object} Object in which each key is a setting name that will be filtered by
...
gpii.settingsHandlers.checkRereadSettings = function (that) {
    if (that.retryOptions.rewriteEvery) {
        fluid.log(fluid.logLevel.WARN, "rewriteEvery is set to " + that.retryOptions.rewriteEvery
 ...
 * @return a promise that will yield the original payload of the invoked GET method
 */
...
/**
 * Called to invoke settings handlers' set function. Based on the first verifySettings option found in the payload,
 ...
 */
gpii.settingsHandlers.dispatchSettingsHandlerSet = function (resolvedName, payload, retryOptions) {
    // TODO "gpii.lifecycleManager.specToSettingsHandler" is the one responsible for this awkward
    //   layout of the settings handler payload - all of this infrastructure will have to be updated
    //   and cleaned up at some point
    try {
...
gpii.settingsHandlers.invokeRetryingHandler = function (resolvedName, payload, retryOptions) {
    // solution entry options overwrites any component options on retrying
    var solutionIds = fluid.keys(payload);
    var solutionRetryOptions = fluid.get(payload, [solutionIds[0], 0, "options", "retryOptions"]);
    var mergedRetryOptions = fluid.extend(true, {}, retryOptions, solutionRetryOptions);
    var that = {
        retryOptions: mergedRetryOptions,
        retries: 1,
        payload: payload,
