 ...
 * @param handler {Function} A function whose signature is (element, [path], oneSolution) where -
 *    - element is the nested payload (i.e. the level containing "settings"/"options")
...
gpii.settingsHandlers.transformOneSolutionSettings = function (element, handler, path) {
    path = path || [];
    var togo = fluid.censorKeys(element, "settings");
    togo.settings = fluid.transform(element.settings, function (oneSetting, settingKey) {
        var innerPath = path.concat(["settings", settingKey]);
        return handler(oneSetting, innerPath, element);
    });
    return togo;
};
...
gpii.settingsHandlers.transformPayloadSettings = function (payload, handler) {
    return gpii.settingsHandlers.transformPayload(payload, function (element, path) {
        return gpii.settingsHandlers.transformOneSolutionSettings(element, handler, path);
    });
};

/** Extract just the settings (eliminating "options") from the nested settingsHandler blocks for a full payload
...
gpii.settingsHandlers.extractSettingsBlocks = function (payload) {
    return gpii.settingsHandlers.transformPayload(payload, function (element) {
        return fluid.filterKeys(element, "settings");
    });
};
