...
gpii.settingsHandlers.comparePayloads = function (payload1, payload2) {
    ...
    console.log("/////*\\\\\\Comparing payload ", JSON.stringify(settings1, null, 2), " with ", JSON.stringify(settings2, null, 2), " equal: " + equal);
    return equal;
};

gpii.settingsHandlers.checkRereadSettings = function (that) {
    var getResponse = fluid.invokeGlobalFunction(that.resolvedName + ".get", [that.payload]);
    var equal = gpii.settingsHandlers.comparePayloads(gpii.resolveSync(getResponse), that.payload);
    if (equal) {
        ...
        if (that.retries === that.retryOptions.numRetries) {
            ...
            console.log("===============RETRYING - will reread settings in " + that.retryOptions.retryInterval + "ms");
...
gpii.settingsHandlers.invokeRetryingHandler = function (resolvedName, payload, retryOptions) {
    ...
    var originalResponse = fluid.invokeGlobalFunction(resolvedName + ".set", [payload]);
