 ...
 *
 *         // optional: Functions to call before/after writing the target file
 *         "invokeBefore": {
 *           "type": "gpii.example.stopProcess",
 *           "args": {
 ...
 * }
 *
 * The invokeBefore/invokeAfter functions are called just before and after the new file has been copied, in order to
 * provide the ability to perform validation/reloading actions on a per-file basis. The following named arguments are
 * available for the functions to consume:
 *     action: The current action being performed: "apply" or "restore"
 *     url: The source URL of the file (when action is apply).
...
fluid.defaults("gpii.settingsHandlers.remoteFileDownload", {
    ...
    members: {
        ...
        stashPath: null,
        // Specifications of the graded functions to be called before/after the apply or restore.
        invokeBefore: null,
        invokeAfter: null,
    ...
    listeners: {
        "applyFile.download": {
            ...
            args: ["{that}.url", "{that}.downloadPath"]
        },
        "applyFile.invokeBefore": {
            priority: "before:stash",
            funcName: "fluid.invokeGradedFunction",
            args: ["{that}.invokeBefore.type", "{that}.invokeBefore"]
        },
        "applyFile.stash": {
        ...
        "applyFile.invokeAfter": {
            priority: "after:write",
            funcName: "fluid.invokeGradedFunction",
            args: ["{that}.invokeAfter.type", "{that}.invokeAfter"]
        },
        "applyFile.end": {
            priority: "last",
        ...
        "restoreFile.invokeBefore": {
            priority: "before:write",
            funcName: "fluid.invokeGradedFunction",
            args: ["{that}.invokeBefore.type", "{that}.invokeBefore"]
        },
        "restoreFile.write": {
...
gpii.settingsHandlers.remoteFileSettingsHandler.applyFile = function (that, targetPath, url, value) {
    ...
    remoteFileSettingsHandler.customInvokes(that, "apply");
...
gpii.settingsHandlers.remoteFileSettingsHandler.restoreFile = function (that, targetPath, stashPath, value) {
    ...
    that.value = value;
    remoteFileSettingsHandler.customInvokes(that, "restore");
    return fluid.promise.fireTransformEvent(that.events.restoreFile);
};

/**
 * Setup the custom invokers, invokeBefore and invokeAfter.
 * @param {Component} that The gpii.settingsHandlers.remoteFileDownload instance.
 * @param {String} action The action being performed by this instance: "apply" or "restore"
 */
gpii.settingsHandlers.remoteFileSettingsHandler.customInvokes = function (that, action) {
    var funcArgs = {
        action: action,
        url: that.url,
        path: that.path,
        value: that.value,
        newFile: action === "apply" ? that.downloadPath : that.stashPath
    };

    var ev = that.events[action + "File"];
    if (that.invokeBefore && that.invokeBefore.type) {
        that.invokeBefore = Object.assign({}, funcArgs, that.invokeBefore);
    } else {
        ev.removeListener("invokeBefore");
    }

    if (that.invokeAfter && that.invokeAfter.type) {
        that.invokeAfter = Object.assign({}, funcArgs, that.invokeAfter);
    } else {
        ev.removeListener("invokeAfter");
    }
};
 ...
 */
gpii.settingsHandlers.remoteFileSettingsHandler.downloadFile = function (url, downloadTo) {
    var promise = fluid.promise();
    ...
    fluid.each(payload.settings, function (settingValue, key) {
        if (settingValue) {
            ...
            var remoteFileDownload = gpii.settingsHandlers.remoteFileDownload({
                members: {
                    invokeBefore: options.invokeBefore,
                    invokeAfter: options.invokeAfter
                }
            });
