...
settingsHandlers.extractELPaths = function (obj) {
    var extracted = {};

    var extract = function (rt, relPath) {
        fluid.each(rt, function (val, key) {
            var currentPath = fluid.model.composePath(relPath, key);
            if (fluid.isPrimitive(val)) {
                extracted[currentPath] = val;
            } else {
                extract(val, currentPath);
            }
        });
    }
    extract (obj, "");
...
settingsHandlers.getSettings = function (solutionEntry, currentSettings) {
    ...
    //consider just returning currentSettings if solutionEntry.settings = null
    if (userRequestedSettings === null) {
        return {
            settings: settingsHandlers.extractELPaths(currentSettings)
        }
    }
...
/**
 * Handle individual solution entry data. The 'modifier' function passed as
 ...
 */
settingsHandlers.handleSolutionEntrry = function (solutionEntry, modifier, parser, isWrite) {
    var options = solutionEntry.options;
 ...
 * entry.
 */
settingsHandlers.transformPayload = function (payload, modifier, parser, isWrite) {
    ...
    return fluid.transform(returnObj, function (solution) {
        return fluid.transform(solution, function(solutionEntry) {
            return settingsHandlers.handleSolutionEntrry(solutionEntry,
                modifier, parser, isWrite);
        });
    });
};

settingsHandlers.makeGet = function (parser) {
    return function (payload) {
        return settingsHandlers.transformPayload(payload, settingsHandlers.getSettings,
            parser);
    };
};

settingsHandlers.makeSet = function (parser) {
    return function (payload) {
        return settingsHandlers.transformPayload(payload, settingsHandlers.setSettings,
            parser, true);
    };
};
