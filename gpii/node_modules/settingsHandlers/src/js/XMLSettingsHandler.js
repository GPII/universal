...
(function () {
     ...
     * {b: { b1: { v: v1 }}, { b2: { v: v2 }}}
     */
    gpii.settingsHandlers.XMLHandler.arrayToHash = function (value, expander, expandSpec) {
        ...
        var arr = fluid.model.transform.getValue(expandSpec.inputPath, undefined, expander);
        ...
        for (var i=0; i<arr.length; ++i) {
            var tmp = arr[i][expandSpec.options.key];
            //check that tmp is valid exist
            if (!tmp) {
                fluid.fail("arrayToHash encountered unhashable array due to missing key", arr[i]);
            }
            //use the value of the key element as key and use the remaining content as value
            delete arr[i][expandSpec.options.key];
            newHash[tmp] = arr[i];
        }
