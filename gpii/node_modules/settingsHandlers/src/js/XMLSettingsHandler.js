...
(function () {
    ...
    gpii.settingsHandlers.XMLHandler.arrayToHash = function (arr, expander, expandSpec) {
        ...
        fluid.each(arr, function (v, k) {
            ...
            if (expandSpec.options.innerValue) {
                ...
                inputPrefixOp.push(""+k); //push doesn't accept ints
    ...
    gpii.settingsHandlers.XMLHandler.arrayToHash.inverse = function (hash, expander, expandSpec) {
        ...
        fluid.each(hash, function (v, k) {
            ...
            if (expandSpec.options.innerValue) {
                var expanded = {};
                expander.inputPrefixOp.push(expandSpec.inputPath);
                expander.inputPrefixOp.push(k);
                expander.outputPrefixOp.push(expandSpec.inputPath);
                expander.outputPrefixOp.push(""+newArray.length); 
                for (var i=0; i<expandSpec.options.innerValue.length; ++i) {
                    fluid.merge("preserve", expanded, expander.expand(expandSpec.options.innerValue[i]));
                }

                expander.outputPrefixOp.pop(expandSpec.inputPath);
                expander.outputPrefixOp.pop(""+newArray.length);
                expander.inputPrefixOp.pop(k);
                expander.inputPrefixOp.pop(expandSpec.inputPath);
                if (expanded !== undefined) {
                    v = expanded;
                }
            }
