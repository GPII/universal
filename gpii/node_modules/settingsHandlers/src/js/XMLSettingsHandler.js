...
(function () {
    ...
    /**
     * Given an array, checks that _all_ the objects within that array have a 
     * key of the name given in pivot. 
     ...
     * @return true if all objects in the array have a key with the
     *         name given in the pivot parameter
     */
    var isPivotableArray = function (arr, pivot) {
    ...
    /**
     * walks through json structure and transforms arrays to json objects where possible.
     * To qualify for transformation, all objects within the array needs to contain a key
     * "pivot". Any array with pivot element named "name" will automatically be detected,
     * all other must be defined via the knownArrays parameter, with their el-paths including
     * the pivot element
     */
        ...
        var pathFinderImpl = function (list, path) {
            return fluid.transform(list, function (v, k) {
                ...
                if (fluid.isPrimitive(v)) {
                    ...
                    if (isPivotableArray(v, pivot)) {
                        if (!known) {
                            transformedPaths.push(newPath+"."+pivot);
                        }
                        // fluid.log("XML Settings handler transforming path: "+newPath);
    ...
    var revertArray = function(json, pivot) {
        // fluid.log("XMLHandler - reverting: "+JSON.stringify(json)+" based on "+pivot);
        var arr = [];
        fluid.each(json, function(v, k) {
            //The below three lines should be replaced by:
            // v[pivot.replace(/\\./g, ".")] = k;
            //but due to the xm.toxml function being dependent on the order of objects (sigh)
    ...
    /**
     * Changes object given in 'knownArrays' to arrays. Opposite of transformArrays
     */
    ...
    */
    ...
    /** 
     * Used by XMLHandler.get when a value has been requested that isn't a 'leaf'
     * Given an elPath, returns an object with all (the (leaf) sub-elpaths as keys,
            ...
            return fluid.transform(solution, function (solutionEntry) {
                ...
                if (options && options.filename) {
                    ...
                    fluid.each(settings, function(v, k) {
                        settingsFound = fluid.merge("replace", settingsFound, getELSubpaths(json, k));
                    });
