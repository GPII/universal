...
(function () {
    ...
    var fixArray = function (arr, pivot) {        
        ...
        for (var i=0; i<arr.length; ++i) {
            console.log("transforming: "+JSON.stringify(arr[i]));
        ...
        var pathFinderImpl = function (list, fn, path) {
            return fluid.transform(list, function (v, k) {
                ...
                if (fluid.isPrimitive(v)) {
                    ...
                    v = fixArray(v, "name");
                    return pathFinderImpl(v, fn, newPath)
                }
                return pathFinderImpl(v, fn, newPath);
            });
        };
        console.log("pathArray: "+JSON.stringify(transformedPaths));
        return { pathArray: transformedPaths, root: pathFinderImpl(list, fn, path).root};
    };

    gpii.settingsHandlers.XMLHandler.transform = function (input, pivot) {
        console.log("Hello world!!!!!!!!");
        var tmp = pathFinder({root: input}, function () {
            return;
        });
