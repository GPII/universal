...
(function () {
    ...
    gpii.tests.canopyMatchMaker.runTests = function () {
        ...
        testCase.test("Fitness computation", function() {
            var fitness = gpii.matchMaker.computeFitness(sammyLeaves, magnifierSkeleton);
            ...
            var solns = gpii.matchMaker.sortSolutions(expandFitness(fitnesses));
        ...
        testCase2.test("Rank and dispose solution", function() {
            var solutions = [lesserMagnifier, magnifier];
            var ranked = gpii.matchMaker.rankSolutions(sammyLeaves, solutions);
            var indices = extractIndices(ranked);
            var expected = [1, 0];
            jqUnit.assertDeepEq("Ranked solutions", expected, indices);
            
            var disposed = gpii.matchMaker.disposeSolutions(sammyLeaves, solutions);
            var itions = extractDispositions(disposed);
