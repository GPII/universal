...
(function () {
    ...
    request.proxyMatchPost = function (requestProxy, model, matchMakerSource, matchMakerPathMap) {
        var matchMakerType = model.preferences.matchMakerType;
        if (!matchMakerType) {
            requestProxy.events.onError.fire("Match Maker type is missing.");
            return;
        }
        delete model.preferences.matchMakerType;
        var matchMakerPath = matchMakerPathMap[matchMakerType];
