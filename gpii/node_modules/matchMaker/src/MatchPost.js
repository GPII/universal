        ...
        options: {
            invokers: {
                handle: {
                    ...
                    args: ["{that}", "{requestProxy}", "{request}.req.body", "{gpii.matchMaker}"]
                }
            }
        }
    });

    gpii.requests.request.handler.matchPost = function (that, requestProxy, model, matchMaker) {
        ...
        solutionsReporter.get({
            ...
            that.when(matchMaker.match(model.preferences, solutions, strategy), function (matchedSolutions) {
                return transform({
