...
// Should offer "filter by grade(s)" as configuration, and define one configuration for each of the legacy files.
// TODO: Update the codex generator to be built by the "builder" or something similar.
...
gpii.solutionsRegistry.schemaGenerator.settingsFromSolutions = function (that) {
    ...
    fluid.each(srFiles, function (filename) {
        ...
        fluid.each(srFileData, function (solutionDef, solutionKey) {
            ...
            // If there is not an explicit "enabled" setting for a solution, add it now.
            if (fluid.get(solutionSchema, "properties.enabled") === undefined) {
                var solutionName = fluid.get(solutionDef, "name");
                var title = solutionName ? "Enable " + solutionName : "Enable";
                fluid.set(solutionSchema, "properties.enabled", {
                    title: title,
                    description: title + ".",
                    type: "boolean",
                    default: false
...
fluid.defaults("gpii.solutionsRegistry.schemaGenerator", {
    ...
    solutionsRegistryPath: "%solutionsRegistry/src/generated",
