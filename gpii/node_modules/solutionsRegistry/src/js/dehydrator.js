...
gpii.solutionsRegistry.dehydrator.separateSolutionsEntries = function (that) {
    ...
    fluid.each(solutionFiles, function (solutionFile) {
        if (solutionFile.match(/\.json(5)$/)) {
            var parsedFilename = path.parse(solutionFile);
            var contextGrade = that.options.contextMarkers[parsedFilename.name];
            ...
            fluid.each(singleFileSolutions, function (solutionDefinition, solutionKey) {
                ...
                if (fileExists && !that.options.replaceExisting) {
                    ...
                    // Add the "context" for this file.
                    revisedSolutionDef.parents = ["gpii.solutionsRegistry.solution", contextGrade];

                    // Stuff the "key" for the solution into an option.
                    revisedSolutionDef.key = solutionKey;
                    ...
                    fluid.each(solutionDefinition.settingsHandlers, function (settingsHandlerDef, settingsHandlerKey) {
                        ...
                        // Stuff the "key" for the settings handler into an option.
                        revisedSettingsHandlerDef.key = settingsHandlerKey;
                        ...
                        fluid.each(settingsHandlerDef.supportedSettings, function (settingsDef, settingKey) {
                            ...
                            // Stuff the "key" for the setting into an option.
                            revisedSettingDef.key = settingKey;
...
fluid.defaults("gpii.solutionsRegistry.dehydrator", {
    ...
    rules: {
        ...
        settingsHandler: {
            "": "",
            // Required to avoid problems when putting together complex grades that use inheritance.
