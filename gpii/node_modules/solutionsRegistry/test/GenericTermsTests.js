...
gpii.tests.universal.solutionsRegistry.genericTerms.validateCommonTerms = function (that, globalValidator) {
    ...
    jqUnit.test("Ensure that generic preference term defaults can be transformed to valid application-specific settings.", function () {
        ...
        // TODO: Need some way to only checks transforms from common terms and not intra-application transforms.
        ...
        fluid.each(codex, function (solutionSchema, solutionKey) {
            var toValidate = transformedDefaults[solutionKey];
            if (toValidate) {
                var validationResults = globalValidator.validate(solutionSchema, toValidate);

                // Log the failure details.
                if (validationResults.isError || !validationResults.isValid) {
                    fluid.log("Error validating transformed default(s) for " + solutionKey + ":");
                    var localisedErrors = gpii.schema.validator.localiseErrors(validationResults.errors, toValidate);

                    fluid.each(localisedErrors, function (singleError) {
                        validationErrorCount++;
                        // TODO: Track down bogus data paths like `.properties['BounceKeysInterval']`
                        var dataPath = (singleError.dataPath && singleError.dataPath.join) ? singleError.dataPath : [];
