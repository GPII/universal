...
gpii.tests.universal.solutionsRegistry.genericTerms.validateCommonTerms = function (that, globalValidator) {
    ...
    jqUnit.test("Ensure that generic preference term defaults can be transformed to valid application-specific settings.", function () {
        ...
        var srPath = fluid.module.resolvePath(that.options.srPath);
        fluid.each(that.options.contextFiles, function (filename) {
            var contextFilePath = path.resolve(srPath, filename);
            var solutionsDefs = require(contextFilePath);
        ...
        fluid.each(codex, function (solutionSchema, solutionKey) {
            var defaultsToValidate = transformedDefaults[solutionKey];
            fluid.each(defaultsToValidate, function (settingValue, settingKey) {
                // Validate per setting so that we can show much clearer error messages.
                var singleDefaultPayload = {};
                fluid.set([singleDefaultPayload], settingKey, settingValue);
                var defaultValidationResults = globalValidator.validate(solutionSchema, singleDefaultPayload);
                ...
                if (defaultValidationResults.isError || !defaultValidationResults.isValid) {
                    ...
                    var localisedDefaultErrors = gpii.schema.validator.localiseErrors(defaultValidationResults.errors, singleDefaultPayload);

                    // Log the failure details.
                    fluid.log("Error validating generic preference term default(s) for '" + solutionKey + "' -> '" + settingKey + "':");
                    gpii.tests.universal.solutionsRegistry.genericTerms.logValidationErrors(localisedDefaultErrors, singleDefaultPayload);
                }
            });

            var maximumsToValidate = transformedMaximums[solutionKey];

            // Validate per setting so that we can show much clearer error messages.
            fluid.each(maximumsToValidate, function (maximumToValidate, settingKey) {
                var singleMaxValuePayload =  {};
                fluid.set(singleMaxValuePayload, [settingKey], maximumToValidate);
                var maxValidationResults =  globalValidator.validate(solutionSchema, singleMaxValuePayload);

                if (maxValidationResults.isError || !maxValidationResults.isValid) {
                    validationErrorCount += maxValidationResults.errors.length;
                    var localisedMaxErrors = gpii.schema.validator.localiseErrors(maxValidationResults.errors, singleMaxValuePayload);

                    // Log the failure details.
                    fluid.log("Error validating generic preference term maximum value(s) for " + solutionKey + ":");
                    gpii.tests.universal.solutionsRegistry.genericTerms.logValidationErrors(localisedMaxErrors, singleMaxValuePayload);
                }
            });

            var minimumsToValidate = transformedMinimums[solutionKey];

            // Validate per setting so that we can show much clearer error messages.
            fluid.each(minimumsToValidate, function (minimumToValidate, settingKey) {
                var singleMinValuePayload = {};
                fluid.set(singleMinValuePayload, [settingKey], minimumToValidate);
                var minValidationResults =  globalValidator.validate(solutionSchema, singleMinValuePayload);

                if (minValidationResults.isError || !minValidationResults.isValid) {
                    validationErrorCount += minValidationResults.errors.length;
                    var localisedMinErrors = gpii.schema.validator.localiseErrors(minValidationResults.errors, singleMinValuePayload);

                    // Log the failure details.
                    fluid.log("Error validating generic preference term minimum value(s) for " + solutionKey + ":");
                    gpii.tests.universal.solutionsRegistry.genericTerms.logValidationErrors(localisedMinErrors, singleMinValuePayload);
                }
            });
        });
...
fluid.defaults("gpii.tests.universal.solutionsRegistry.genericTerms", {
    ...
    srPath: "%solutionsRegistry/src/generated/",
    // Make this list dynamic if we ever start adding / renaming supported platforms.
    // contextFiles: ["win32.json5", "darwin.json5"],
    contextFiles: ["win32.json5"],
    listeners: {
