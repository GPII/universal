 ...
 */
gpii.tests.iod.testInstaller1.startInstaller = function (that, iod) {
...
jqUnit.asyncTest("test installation storage", function () {
    ...
    iod.installations = {};
    iod.installations[testData.existingInst.id] = testData.existingInst;
    ...
    p.then(function () {
        ...
        var file = gpii.iod.writeInstallation(iod, dir, testData.newInst);

        jqUnit.assertEquals("writeInstallation should write to the correct file",
            path.join(dir, "iod-installation." + testData.newInst.id + ".json"), file);

        // Check it was written correctly.
        var writtenContent = fs.readFileSync(file);
        var writtenObject = JSON.parse(writtenContent);

        jqUnit.assertDeepEq("writeInstallation should write the correct data", testData.newInst, writtenObject);
        ...
        gpii.iod.readInstallations(iod, dir).then(function () {
            jqUnit.assertDeepEq("readInstallations should read the correct data",
                testData.newInst, iod.installations[testData.newInst.id]);

            // Overwrite the existing file.
            var file = gpii.iod.writeInstallation(iod, dir, testData.updatedInst);

            // Check it was written correctly.
            var writtenContent = fs.readFileSync(file);
            var writtenObject = JSON.parse(writtenContent);
            jqUnit.assertDeepEq("writeInstallation should overwrite with the correct data",
                testData.updatedInst, writtenObject);

            // Check the updated file gets loaded.
            gpii.iod.readInstallations(iod, dir).then(function () {
                jqUnit.assertDeepEq("readInstallations should update with the correct data",
                    testData.updatedInst, iod.installations[testData.newInst.id]);
                jqUnit.start();
            });
        });
    });
