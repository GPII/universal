...
fluid.defaults("gpii.tests.iod", {
    ...
    listeners: {
        ...
        "{lifecycleManager}.events.onSessionStop": null
    },
    components: {
        "testInstaller1": {
...
jqUnit.asyncTest("test requirePackage", function () {
    ...
    var nextTest = function () {
        ...
        p.then(function () {
            jqUnit.assertTrue("startInstaller must have been called" + suffix, !!iod.funcCalled.startInstaller);
...
jqUnit.asyncTest("test installation storage", function () {
    ...
    var iod = gpii.tests.iod({
        invokers: {
            uninstallPackages: {
                funcName: "gpii.tests.iod.testInstaller1.testFunctionCalled",
                args: ["{that}", "{iod}", "uninstallPackages"]
            }
        }
    });
    ...
    p.then(function () {
        ...
        jqUnit.assertFalse("uninstallPackages should not have been called", iod.funcCalled.uninstallPackages);
        ...
        gpii.iod.readInstallations(iod, dir).then(function () {
            ...
            jqUnit.assertTrue("uninstallPackages should have been called", !!iod.funcCalled.uninstallPackages);
            iod.funcCalled.uninstallPackages = null;
            ...
            gpii.iod.readInstallations(iod, dir).then(function () {
                ...
                jqUnit.assertTrue("uninstallPackages should have been called again",
                    !!iod.funcCalled.uninstallPackages);
...
jqUnit.asyncTest("test uninstallation", function () {
    ...
    iod.requirePackage(packageName).then(function () {
        ...
        jqUnit.assertTrue("Package should have been set to be removed", installation.remove);

        iod.uninstallPackages(0);

        var retries = 10;
        var waitForRemoval = function () {
            // There's no promise or event, so just poll.
            if (iod.installations[installation.id]) {
                if (--retries > 0) {
                    setTimeout(waitForRemoval, 100);
                } else {
        ...
        process.nextTick(waitForRemoval);
...
jqUnit.asyncTest("test uninstallation after restart", function () {
    ...
    iod.requirePackage(packageName).then(function () {
        ...
        waitForUninstall().then(function () {
