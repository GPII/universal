...
/**
 * Reads the stored installations from a previous instance.
 *
...
gpii.iod.writeInstallation = function (that, directory, installation) {
    var content = JSON.stringify(installation);
 ...
 *
 * @param {Component} that The gpii.iod instance.
 * @param {object} installation The installation state.
...
/**
 * Used while abusing launch handlers in order to provide a way to invoke IoD from the solutions registry.
 * @param method {string} Method of gpii.iod to invoke
 * @param args {array} Arguments for the method.
 * @return {Promise} Resolves with the return value.
 */
gpii.iod.invoke = function (method, args) {
    ...
    process.nextTick(function () {
        var result = iod[method].apply(iod, args);
        fluid.promise.follow(fluid.toPromise(result), promise);
    });
...
fluid.defaults("gpii.iod.invoke", {
...
fluid.defaults("gpii.iod.isInstalled", {
    gradeNames: "fluid.function",
    argumentMap: {
        packageName: 0
    }
