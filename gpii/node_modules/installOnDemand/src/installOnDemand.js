 ...
 * @property {packageInfo} packageName - packageInfo.name
...
fluid.defaults("gpii.iod", {
    ...
    dynamicComponents: {
        installers: {
            createOnEvent: "onInstallerStart",
            type: "{arguments}.0",
            options: {
    ...
    events: {
        ...
        onInstallerStart: null // [ packageInstaller grade name, installation ID ]
    },
    listeners: {
...
gpii.iod.requirePackage = function (that, packageRequest) {
    ...
    that.getPackageInfo(packageRequest).then(function (packageInfo) {
        ...
        if (installerGrade) {
            // Start the installer.
            that.events.onInstallerStart.fire(installerGrade, installation.id);
