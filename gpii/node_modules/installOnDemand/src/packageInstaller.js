...
gpii.iod.initialise = function (that, iod) {
    ...
    that.tempDir = tempDir.fullPath;
    that.cleanupPaths.push(tempDir.createdPath);
};
...
gpii.iod.downloadPackage = function (that) {
    ...
    if (that.packageInfo.url.startsWith("https://")) {
        ...
        var downloadPromise = gpii.iod.httpsDownload(that.packageInfo.url, that.localPackage);
 ...
 *
 * @param url {string} The remote uri.
 * @param localPath {string} Destination path.
