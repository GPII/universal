...
(function () {
     ...
     *     paths using the floydWarshallPath function
     */
    gpii.ontologyHandler.floydWarshallAlgorithmWithReconstruction = function ( edges ) {
        var vertices = {};
        fluid.each(edges, function(edge) {
            vertices[edge.source] = true;
            vertices[edge.target] = true;
        });
        // Assign each vertex an integer key for the lookup arrays
        fluid.each(Object.keys(vertices), function(vert, i) {
            vertices[vert] = i;
        });
        var numVertices = Object.keys(vertices).length;
        // Create |V|x|V| dist and next arrays and initialize to Infinity and null
        var dist = fluid.generate(numVertices, function() {
            return fluid.generate(numVertices, Infinity, false);
        }, true);
        var next = fluid.generate(numVertices, function() {
            return fluid.generate(numVertices, null, false);
        }, true);
        fluid.each(edges, function(edge) {
            var u = vertices[edge.source], 
                v = vertices[edge.target];
            dist[u][v] = 1; // All weights currently 1
            next[u][v] = v;
        });
    ...
    gpii.ontologyHandler.floydWarshallPath = function ( data, v1, v2 ) {
        ...
        var getKey = function(val) { // This hash should have unique values
            var togo;                // by virtue of it's construction.
            fluid.each(data.vertices, function (vert, key) {
                if (u === vert) {
                    togo = key;
                }
            });
