...
(function () {
    ...
    gpii.ontologyHandler.transformMetadata = function (metadata, transformSpec) {
        ...
        fluid.each(metadata, function (entry) {
            ...
            var transformed = fluid.model.transformWithRules(toTransform, transformSpec);
            var target = [];
    ...
    gpii.ontologyHandler.prefsToOntology = function (that, prefs, fromView, toView) {
        ...
        //translate each of the context blocks based on the transformSpec
        fluid.each(prefs.contexts, function (context, contextId) {
            var tmp = transformed.contexts[contextId] = {};
            gpii.ontologyHandler.addIfDefined(tmp, "name", context.name);
            gpii.ontologyHandler.addIfDefined(tmp, "priority", context.priority);
            gpii.ontologyHandler.addIfDefined(tmp, "preferences",
                fluid.model.transformWithRules(context.preferences, transformSpec));
            gpii.ontologyHandler.addIfDefined(tmp, "metadata",
                gpii.ontologyHandler.transformMetadata(context.metadata, transformSpec));
            gpii.ontologyHandler.addIfDefined(tmp, "conditions", context.conditions);
        });
        //translate the context independent metadata block:
        gpii.ontologyHandler.addIfDefined(transformed, "metadata",
            gpii.ontologyHandler.transformMetadata(prefs.metadata, transformSpec));

        return transformed;
    };

    /*
     * Quick function for adding a key-value pair to an object only if the value is not 'undefined'
     *
     * @obj Object to add the key-value pair to - this object will be modified
     * @key String/Number the key to use when adding the key-value pair
     * @value The value to add for the key. If the value === undefined, the 'obj' parameter will
     *      stay unmodified
     */
    gpii.ontologyHandler.addIfDefined = function (obj, key, value) {
        if (value !== undefined) {
            obj[key] = value;
        }
    };
    ...
    /*
     * Function used to merge two metadata arrays. The input should be two arrays (can be undefined
     * as well. The output will be the two arrays concatenated. The reason Array.concat function is
     * not just used direcly, is that it will add 'undefined' as an entry in the resulting array if
     * one of the arrays used is undefined.
     *
     * @meta1 First array to be merged
     * @meta2 Second array to be merged
     *
     * @return the concatenation of the two arrays. If meta1 is undefined, meta2 is returned. If
     * meta2 is undefined, meta1 is returned. If both are defined, their concatenation is returned.
     */
    gpii.ontologyHandler.concatArrays = function (meta1, meta2) {
        return !meta1 ? meta2 : (!meta2) ? meta1 : meta1.concat(meta2);
    };
     ...
     * use $.extend, as this would override array entries in the metadata section, so a more
    ...
    gpii.ontologyHandler.mergePrefsSets = function (master, secondary) {
        ...
        fluid.each(secondary.contexts, function (context, contextId) {
            ...
            $.extend(true, mcontext.preferences, context.preferences);
            mcontext.metadata = gpii.ontologyHandler.concatArrays(mcontext.metadata, context.metadata);
        });

        //copy outer metadata block
        gpii.ontologyHandler.addIfDefined(master, "metadata",
            gpii.ontologyHandler.concatArrays(master.metadata, secondary.metadata));
        return master;
    };
            ...
            fluid.each(contexts, function (context, cname) {
                if (context.preferences && existingPrefs.contexts[cname] && existingPrefs.contexts[cname].preferences) {
                    ...
                    if (filteredPrefs === undefined) { //no more preferences left in this ontology
                        delete togo[existingView].contexts[cname].preferences;
                        return;
                    }
    ...
    gpii.ontologyHandler.doSuppliedTransform = function (preferences, transform) {
        ...
        var toTransform = preferences.contexts["gpii-default"].preferences;
        var transformed = fluid.model.transformWithRules(toTransform, transform);
        return transformed;
    };
})();