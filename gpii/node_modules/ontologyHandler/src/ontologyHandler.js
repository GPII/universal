...
(function () {
    ...
    gpii.ontologyHandler.floydWarshall.algorithmInitVertices = function (data, edges) {
        ...
        fluid.each(edges, function (edge) {
            gpii.ontologyHandler.floydWarshall.incrementingHashAppend(data.vertices, edge.source);
            gpii.ontologyHandler.floydWarshall.incrementingHashAppend(data.vertices, edge.target);
        });
     ...
     *
     * @hash (Object) The set we are assigning incrementing integers to.
     * @item (Object) The next value to add to the set. If this item is already
     *     in the set, the existing value for it will be unchanged.
     * @returns Does not return a value, operates directly on the hash.
     */
    gpii.ontologyHandler.floydWarshall.incrementingHashAppend = function (hash, item) {
        if (!hash[item]) {
            hash[item] = Object.keys(hash).length + 1;
        }
    };
     ...
     * ontology.
     *
     * @transformSpec (Object) The transform spec used to go between ontologies.
