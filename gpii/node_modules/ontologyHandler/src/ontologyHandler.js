...
(function () {
     ...
     *
     * @ontologyMapping (String) mapping in source-target format.
    ...
    gpii.ontologyHandler.loadOntologyTransformSpecs = function (that, ontologyDir, ontologyTransformsList) {
        ...
        that.options.ontologyTransformRoutes =
            gpii.ontologyHandler.floydWarshallAlgorithmWithReconstruction(edges);
    };
     ...
     *
     * @edges (Array) - An array of arrays of length 2, consisting of the named
     *     edges such as [['A','B'], ['B','C']]
     * @returns (Object) - An object containing the necessary structures to resolve
     *     paths using the floydWarshallPath function. Contains 4 members:
     *     vertices (Object) key/value pair where key is the name of an ontology, and value
     *         is an integer to be used in the algorithm marking it as a vertice
     ...
     */
    gpii.ontologyHandler.floydWarshallAlgorithmWithReconstruction = function ( edges ) {
        var data = {};
        gpii.ontologyHandler.floydWarshallAlgorithmInitVertices(data, edges);
        gpii.ontologyHandler.floydWarshallAlgorithmInitMatrices(data, edges);
        gpii.ontologyHandler.floydWarshallAlgorithmCore(data.dist, data.next, data.numVertices);
        return data;
    };

    /**
     * Internal Use only
     * Function used in gpii.ontologyHandler.floydWarshallAlgorithmWithReconstruction
     * to initialize the mapping of vertices named by ontology to integers used in
     * the algorithm.
     *
     * @data (Object) Data structure used in main function.
     * @edges (Array) List of edges where each is an Object contains a directed source and
     *      target parameter.
     * @returns Does not a return a value, operates directly on the data structure.
     */
    gpii.ontologyHandler.floydWarshallAlgorithmInitVertices = function (data, edges) {
        data.vertices = {};
        fluid.each(edges, function (edge) {
            data.vertices[edge.source] = true;
            data.vertices[edge.target] = true;
        });
        // Assign each vertex an integer key for the lookup arrays
        fluid.each(Object.keys(data.vertices), function (vert, i) {
            data.vertices[vert] = i;
        });
        data.numVertices = Object.keys(data.vertices).length;
    };
     ...
     * @returns Does not a return a value, operates directly on the data structure.
     */
    gpii.ontologyHandler.floydWarshallAlgorithmInitMatrices = function (data, edges) {
     ...
     *
     * @dist (2D array) See algorithm documentation
     * @next (2D array) See algorithm documentation
     * @numVertices (Integer) total number of vertices
     * @returns Does not a return a value, operates directly on the dist and next matrices.
     */
    gpii.ontologyHandler.floydWarshallAlgorithmCore = function (dist, next, numVertices) {
    ...
    /*
     * Uses the data generated in floydWarshallAlgorithmWithReconstruction to
     * generate the shortest path as an array of vertex names.
     *
     * @data (Object) - The data structure returned from
     *     floydWarshallAlgorithmWithReconstruction
     * @v1 (String) - The starting vertice
     * @v2 (String) - The ending vertice
     * @returns (Array) - An array of vertices for the shortest path, or an
    ...
    gpii.ontologyHandler.floydWarshallPath = function ( data, v1, v2 ) {
        ...
        var getKey = function (val) {
            // This hash should have unique values by virtue of it's construction
            return fluid.find(data.vertices, function (obj, idx) {
                if (val === obj) {
                    return idx;
                }
            });
