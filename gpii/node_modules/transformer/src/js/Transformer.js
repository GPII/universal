...
(function () {
    /**
     * Helper function to find out if the capabilitiesTransformations for a settingsHandler block has inputPaths
     * to produce output for given setting. Return true if has, otherwise, return false. Setting paths that are
     * used to compare:
     * 1. All individual keys in the settings object;
     * 2. All concatenated settings nested key paths.
     * In an example of this input settings:
     * {
     *     "http://registry.gpii.net/applications/com.microsoft.windows.magnifier": {
     *         "invertColours": true
     *     }
     * }
     * Compared setting paths include:
     * [
     *     "http://registry.gpii.net/applications/com.microsoft.windows.magnifier",
     *     "invertColours",
     ...
     */
    gpii.transformer.hasSupportedSettings = function (capabilitiesTransformations, settings) {
        if (!capabilitiesTransformations || !settings) {
            return false;
        }
        var supportedSettingPaths = fluid.model.transform.collectInputPaths(capabilitiesTransformations);
        // find concatenated settings nested key paths
        var actualSettingPaths = gpii.matchMakerFramework.utils.computeLeaves(settings, true);

        // add each key as a path to actualSettingPaths
        ...
        // Find out if one or more actual settings are supported
        var originalLength = supportedSettingPaths.length;
        gpii.transformer.removeElement(supportedSettingPaths, actualSettingPaths);
    ...
    gpii.transformer.transformOneSettingsHandler = function (settingsHandler, oneUserSolution, solutionId) {
        ...
        var hasSupportedSettings = gpii.transformer.hasSupportedSettings(settingsHandler.capabilitiesTransformations, oneUserSolution.settings);
     ...
     * @param {String|Array} - A string, or an array of strings, to be removed.
     */

    gpii.transformer.removeElement = function (array, valuesToRemove) {
        if (!array || !valuesToRemove) {
            return;
        }
        fluid.makeArray(valuesToRemove);
        fluid.remove_if(array, function (i) {
            return valuesToRemove.indexOf(i) !== -1;
        });
    };
    ...
    gpii.transformer.adjustSettingsHandlerRelatedData = function (oneSolution) {
        fluid.each(oneSolution.settingsHandlers, function (settingsHandler, key) {
            if ($.isEmptyObject(settingsHandler.settings)) {
                ...
                gpii.transformer.removeElement(oneSolution.update, elementToRemove);
                gpii.transformer.removeElement(oneSolution.configure, elementToRemove);
                gpii.transformer.removeElement(oneSolution.restore, elementToRemove);
            }
        });
    };
