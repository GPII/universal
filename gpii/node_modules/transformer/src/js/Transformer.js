...
(function () {
    ...
    gpii.transformer.hasSupportedSettings = function (capabilities, capabilitiesTransformations, settings) {
        ...
        // 1. find concatenated settings nested key paths from settings
        var actualSettingPaths = gpii.matchMakerFramework.utils.computeLeaves(settings, true);

        // 2. find if any actualSettingPaths is in capabilities
        var capabilitiesArray = fluid.copy(fluid.makeArray(capabilities));
        var originalCapabilitiesLength = capabilitiesArray.length;
        gpii.transformer.arrayDifference(capabilitiesArray, actualSettingPaths);

        if (capabilitiesArray.length < originalCapabilitiesLength) {
            return true;
        } else {
            // 3. find if any actualSettingPaths can be handled by capabilitiesTransformations
