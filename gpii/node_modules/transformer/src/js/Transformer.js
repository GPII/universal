...
(function () {
     ...
     *
     * Important: The input elements should always be a subset of the elements of the
     * block "elems" from the 'transformSpec', otherwise 'fluid.fail' will be called.
     *
     * NOTE: This requirement is imposed because otherwise it wont be possible
     * to reverse the filtering of the non contained elements in the "elems" block.
     *
     * Example parameters:
     *
     *  "arr": [
     ...
     *  This should always be a subset of the elements of the block "elems" present in the 'transformSpec'
     *  parameter.
    ...
    gpii.transformer.JSONArrayToTaggedArray = function (arr, transformSpec) {
        ...
        // Filter the used elements
        var foundElems = [];
        fluid.each(arr, function (elem) {
            var elemIndex = transformSpec.elems.indexOf(elem);
            if (elemIndex === -1) {
                fluid.fail("JSONArrayToTaggedArray value '" + elem + "' is not present in the 'elems' array of the 'transformSpec'.");
            } else {
                foundElems.push(elem);
            }
        });

        // Filter the unused elements
        var unusedElems = transformSpec.elems.filter(
            function (elem) { return foundElems.indexOf(elem) === -1; }
        );

        var pushNewElem = function (elem, foundVal) {
            var arrayElem = {};

            arrayElem[transformSpec.key] = elem;
            arrayElem[transformSpec.tag] = transformSpec.hasElemVal[foundVal];

            output.push(arrayElem);
        };

        fluid.each(foundElems, function (elem) {
            pushNewElem(elem, "isTrue");
        });

        fluid.each(unusedElems, function (elem) {
            pushNewElem(elem, "isFalse");
        });
    ...
    fluid.defaults("gpii.transformer.taggedArrayToJSON", {
        gradeNames: "fluid.standardTransformFunction"
    });

    /**
     * Realizes the inverse operation than 'gpii.transformer.JSONArrayToTaggedArray'. Receives
     ...
     * @param {Object} transformSpec The required spec for the transform. It's required
     *      to have a key element that holds the identifier which holds the value to be extracted
    ...
    gpii.transformer.taggedArrayToJSON = function (arr, transformSpec) {
        ...
        // Check the transformSpec
        gpii.transformer.taggedArrayToJSON.checkSpec(transformSpec);

        fluid.each(arr, function (val) {
            var tag = fluid.get(val, transformSpec.tag);
     ...
     * @param {Object} transformSpec The transform spec to be validated.
     */
    gpii.transformer.taggedArrayToJSON.checkSpec = function (transformSpec) {
        if (!transformSpec.tag) {
            fluid.fail("taggedArrayToJSON requires a 'tag' block set");
        }
        if (!transformSpec.key) {
            fluid.fail("taggedArrayToJSON requires a 'key' block set");
        }
    };
})();
