...
(function () {
    ...
    gpii.transformer.jsonArrayToTaggedArray = function (arr, transformSpec) {
        ...
        if (!arr || !fluid.isArrayable(arr)) {
            fluid.fail("jsonArrayToTaggedArray didn't find array at inputPath nor passed as value.", transformSpec);
        }
        if (!transformSpec.tags) {
            fluid.fail("jsonArrayToTaggedArray requires a 'tags block' set");
        }

        var tags = transformSpec.tags;
        fluid.each(arr, function (value) {
            var arrayElem = {};
            fluid.each(tags, function (tagVal, tagKey) {
                fluid.set(arrayElem, tagKey, tagVal);
                fluid.set(arrayElem, "$t", value);
            });
