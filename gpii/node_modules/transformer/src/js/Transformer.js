...
(function () {
    ...
    /**
     * Creates a tagged array from a json array, using the tag specified in the 'transformSpec'.
     * If the values in the array are present in the 'elems' array of the 'transformSpec',
     * the tag that will be added for that element will be the value specified in
     * 'transformSpec.hasElemVal.isTrue', otherwise the value will be 'transformSpec.hasElemVal.isFalse'.
     *
     ...
     *     }
     *   }
     * @returns {Array} A tagged array result of processing the input array with the provided 'transformSpec'.
     */
    gpii.transformer.jsonArrayToTaggedArray = function (arr, transformSpec) {
        ...
        fluid.each(transformSpec.elems, function (elem) {
            ...
            fluid.set(arrayElem, "$t", elem);

            if (hasElem) {
                fluid.set(arrayElem, transformSpec.tag, transformSpec.hasElemVal.isTrue);
            } else {
                fluid.set(arrayElem, transformSpec.tag, transformSpec.hasElemVal.isFalse);
            }
    ...
    gpii.transformer.jsonArrayToTaggedArray.checkSpec = function (transformSpec) {
        if (!transformSpec.tag) {
            fluid.fail("jsonArrayToTaggedArray requires a 'tags' block set");
        }
     ...
     * @returns {Array} An array with the values of the provied tagged array.
     */
    gpii.transformer.jsonArrayToTaggedArray.invert = function (arr) {
        var output = [];

        fluid.each(arr, function (val) {
            var value = fluid.get(val, "$t");
            output.push(value);
        });
