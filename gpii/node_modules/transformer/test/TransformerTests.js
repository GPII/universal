...
var JSONArrayToTaggedArrayPayload = {
    "transform": {
        "taggedArray": {
            "transform": {
                "type": "gpii.transformer.JSONArrayToTaggedArray",
                "input": [
                    "Buttons",
                    "Lists",
                    "Menus"
...
jqUnit.test("gpii.transformer.JSONArrayToTaggedArray() tests", function () {
    // Check if the transform returns the expected value
    var result = fluid.model.transform(JSONArrayToTaggedArrayPayload.model,
        JSONArrayToTaggedArrayPayload.transform);
    jqUnit.assertDeepEq("gpii.transformer.JSONArrayToTaggedArray", JSONArrayToTaggedArrayPayload.expected, result);

    // Check if the the inverse of the transform is preformed correctly
    var inverseRules = fluid.model.transform.invertConfiguration(JSONArrayToTaggedArrayPayload.transform);
    if (inverseRules === fluid.model.transform.uninvertibleTransform) {
        jqUnit.fail("Transform failed to be inverted.");
    } else {
        var inverseTransformed = fluid.model.transform(result, inverseRules);

        // The inverseTransformed value should be equal to the original input
        jqUnit.assertDeepEq(
            "Check the result of the inverse transformation",
            JSONArrayToTaggedArrayPayload.transform.taggedArray.transform.input,
            inverseTransformed
        );
    }
});
