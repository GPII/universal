...
jqUnit.test("gpii.transformer.JSONArrayToTaggedArray() tests", function () {
    fluid.each(JSONArrayToTaggedArrayPayload, function (payload) {
        // Check if the transform returns the expected value
        var result = fluid.model.transform(payload.model, payload.transform);
        jqUnit.assertDeepEq("gpii.transformer.JSONArrayToTaggedArray", payload.expected, result);

        // Check if the the inverse of the transform is preformed correctly
        var inverseRules = fluid.model.transform.invertConfiguration(payload.transform);
        if (inverseRules === fluid.model.transform.uninvertibleTransform) {
            ...
            jqUnit.assertDeepEq(
                ...
                payload.transform.taggedArray.transform.input,
                inverseTransformed
            );
        }
    });
