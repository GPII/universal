...
gpii.tests.iodPackages.getPackageDataTests = fluid.freezeRecursive([
    ...
    },
    {
        ...
        expect: JSON5.parse(fs.readFileSync(__dirname + "/testPackages/package1.json5", "utf8"))
    },
    {
...
jqUnit.asyncTest("testing package data signature verification", function () {
    // Create a key pair
    var passphrase = "test";
    ...
    // The key is already base64 encoded - just remove the PEM header and footer.
    var re = new RegExp(".*(:?\n|^)-----BEGIN[^\n]*\n(.*\n)?-----END.*", "s");
    var publicKey = re.exec(keyPair.publicKey)[2];

    // Get the key fingerprint
    var fingerprint = crypto.createHash("sha256").update(Buffer.from(publicKey, "base64")).digest("base64");

    // The object to be signed
    var obj = {
        a: "this object will be signed",
        b: {
            c: "extra",
            d: Math.random()
        },
    ...
    var buffer = Buffer.from(json, "utf8");
    ...
    var tests = [
        function () {
            // Verify it.
            return gpii.iod.verifySignedJSON(json, signature, [fingerprint]).then(function () {
                jqUnit.assert("verifySignedJSON should resolve with correctly signed data");
            });
        },
        function () {
            // Verify it, without a valid fingerprint.
            return gpii.iod.verifySignedJSON(json, signature, ["xxx"]).then(function () {
                jqUnit.fail("verifySignedJSON should have rejected with unknown fingerprint");
            },
            function () {
                jqUnit.assert("verifySignedJSON should reject with unknown fingerprint");
            });
        },
        function () {
            // Run it through checkPackageSignature
            /** @type PackageResponse */
            var packageResponse = {
                installer: true,
                packageData: json,
                packageDataSignature: signature
            };
            return gpii.iod.checkPackageSignature(packageResponse, [fingerprint]).then(function (value) {
                jqUnit.assertDeepEq("packageData resolved by checkPackageSignature should be de-serialised",
                    obj, value.packageData);
            });
        },
        function () {
            // Make a change to the signed data
            var modified = json.replace(/this object/, "that object");
            var promise = fluid.promise();
            gpii.iod.verifySignedJSON(modified, signature, [fingerprint]).then(function () {
                jqUnit.fail("verifySignedJSON should have rejected with modified data");
                promise.reject();
            }, function () {
                jqUnit.assert("verifySignedJSON should reject with modified data");
            });
            return promise;
        }
    ];

    fluid.promise.sequence(tests).then(jqUnit.start, jqUnit.fail);
