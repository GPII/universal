...
fluid.defaults("gpii.tests.iod", {
    gradeNames: [ "gpii.iod", "gpii.lifecycleManager", "gpii.userListeners.usb" ],

    listeners: {
        //"onCreate.discoverPackageSources": null,
    ...
    installerGrades: {
        "testPackageType1": "gpii.tests.iod.testInstaller1",
        "testPackageType2a": "gpii.tests.iod.testInstaller2",
        "testPackageType2b": "gpii.tests.iod.testInstaller2",
        "testFailPackageType": "gpii.tests.iod.testInstallerFail"
    },
    keyPair: "@expand:gpii.tests.iod.generateKeyPair()",
    config: {
        trustedKeys: {
            packagesTest: "{that}.options.keyPair.fingerprint"
        },
        packageSources: [
            path.join(__dirname, "localPackages")
        ]
    }
});
    ...
    var nextTest = function () {
        ...
        p.then(function (value) {
            ...
            if (test.expect !== "reject") {
                fluid.log("reject reason: ", reason);
            }
...
jqUnit.asyncTest("test service discovery", function () {
    ...
    server.on("request", function (req, res) {
        fluid.log("request: ", req.url);
        jqUnit.assert("request made");
        res.end("hello");
    });

    server.on("listening", function () {
        var localUrl = "http://" + server.address().address + ":" + server.address().port + "/";
        fluid.log("listening: ", localUrl);

        // try checkService directly
        var successPromise = gpii.iod.checkService(localUrl).then(function () {
            jqUnit.assert("checkService should resolve");
        }, function (err) {
            fluid.log(err);
            jqUnit.fail("checkService should not reject");
        });

        var failPromise = fluid.promise();
        // Check a local port (Gopher) which is probably closed.
        gpii.iod.checkService("http://127.0.0.3:70").then(function () {
            jqUnit.fail("checkService (fail test) should not resolve");
        }, function () {
            jqUnit.assert("checkService (fail test) should reject");
            failPromise.resolve();
        });
