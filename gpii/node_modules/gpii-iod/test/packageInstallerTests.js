...
jqUnit.asyncTest("test downloadInstaller", function () {

    var tempDir = path.join(os.tmpdir(), "gpii-downloadInstaller-tests" + Math.random());
    mkdirp.sync(tempDir);
    gpii.tests.iodInstaller.teardowns.push(function () {
        rimraf.sync(tempDir);
    });
    ...
    var tests = [
        {
            id: "successful local file URL",
            installation: {
                tempDir: "tempDir"
            },
            packageData: {
                installerSource: "file://installer-source",
                installer: "installer.msi"
            },
            fileDownload: "the hash",
            expect: {
                disposition: "resolve",
                fileDownload: true,
                installation: {
                    tempDir: "tempDir",
                    installerFile: "tempDir/installer.msi",
                    installerFileHash: "the hash"
                }
            }
        }, {
            id: "successful remote file",
            installation: {
                tempDir: "tempDir"
            },
            packageData: {
                installerSource: "https://installer-source",
                installer: "installer.msi"
            },
            fileDownload: "the hash",
            expect: {
            ...
            id: "unsuccessful local file",
            installation: {
                tempDir: "tempDir"
            },
            packageData: {
                installerSource: "file://installer-source-reject",
                installer: "installer.msi"
            },
            expect: {
                disposition: "reject",
    ...
    // Mock fileDownload
    var fileDownloadOrig = gpii.iod.fileDownload;
    gpii.tests.iodInstaller.teardowns.push(function () {
        gpii.iod.fileDownload = fileDownloadOrig;
    });
    gpii.iod.fileDownload = function (address) {
        var promise = fluid.promise();
        if (!currentTest.expect.fileDownload) {
            jqUnit.fail("Call to fileDownload was unexpected" + suffix);
        }

        jqUnit.assertEquals("fileDownload must be called with the correct address",
            currentTest.packageData.installerSource, address);

        if (address.endsWith("reject")) {
            promise.reject();
        } else {
            promise.resolve(currentTest.fileDownload);
        }
    ...
    var nextTest = function (testIndex) {
        ...
        suffix = " - test " + testIndex + "(" + currentTest.id + ")";

        var installation = fluid.copy(currentTest.installation) || {};
        if (!installation.tempDir) {
            installation.tempDir = tempDir;
        }
        var packageData = fluid.copy(currentTest.packageData);
        if (!packageData.installCommands) {
            packageData.installCommands = {};
        }


        installer = gpii.tests.iodInstaller.loggingInstaller();

        var p = gpii.iod.downloadInstaller(installer, installation, packageData);
        jqUnit.assertTrue("fileDownload must return a promise" + suffix, fluid.isPromise(p));

        p.then(function () {
            jqUnit.assertEquals("downloadInstaller must only resolve if expected" + suffix,
                currentTest.expect.disposition, "resolve");

            jqUnit.assertDeepEq("downloadInstaller must only resolve if expected" + suffix,
                currentTest.expect.installation, installation);

            nextTest(testIndex + 1);
        }, function (err) {
            if (currentTest.expect.disposition !== "reject") {
                fluid.log(err);
            }
            jqUnit.assertEquals("downloadInstaller must only reject if expected" + suffix,
                currentTest.expect.disposition, "reject");
