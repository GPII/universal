...
gpii.iod.customCommand = function (that, when) {
    ...
    var togo;
    if (command) {
        var promises = fluid.transform(fluid.makeArray(command), function (c) {
            return function () {
                return that.executeCommand(c);
            };
        });
        togo = fluid.promise.sequence(promises);
    }
    return togo;
};
...
gpii.iod.fileDownload = function (address, localPath, options) {
    ...
    var downloadUrl = new url.URL(address);

    var stream;
    if (downloadUrl.protocol === "file:") {
        var offset = parseInt(downloadUrl.searchParams.get("offset")) || 0;
        var file = url.fileURLToPath(address);
 ...
 * @return {Promise} Resolves when complete.
 */
gpii.iod.installComplete = function (that, iod, installation, packageData) {
    var promise;
    ...
    if (packageData.installCommands.complete) {
        ...
        // Check if the application is detected, to see if the (un)installation process really worked.
        var installed = iod.packages.checkInstalled(packageData);
        var installing = (that.currentAction === "install");
        promise = fluid.promise();
        ...
        if (installed === installing) {
            ...
            promise.reject({
                isError: true,
                message: packageData.name +
                    (installing ? " was not detected after installing" : " was still detected after uninstalling")
            });
        }
    }
    return promise;
...
gpii.iod.executeCommand = function (that, execOptions, command, args) {
    ...
    if (!execOptions.command) {
        processPromise = fluid.promise().reject({
            isError: true,
        ...
        processPromise = that.startProcess(execOptions.command, execOptions.args);
    }
    ...
    processPromise.then(function (result) {
        ...
        if (fluid.isValue(execOptions.success)) {
            success = fluid.makeArray(execOptions.success).includes(result.exitCode);
        } else if (fluid.isValue(execOptions.failure)) {
            success = !fluid.makeArray(execOptions.failure).includes(result.exitCode);
        } else {
            success = true;
        }
        ...
        if (success) {
            ...
            promiseTogo.reject({
                message: "Command returned " + (execOptions.success ? "non-success" : "failure")
