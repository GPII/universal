...
gpii.iod.downloadInstaller = function (that) {
    ...
    fluid.log("IoD: Downloading installer " + that.packageData.url);

    var promise = fluid.promise();

    that.installation.localPackage = path.join(that.installation.tempDir, that.packageData.filename);

    if (that.packageData.url.startsWith("https://")) {
        // Warning: Taking a url from an external source, downloading it, and then later executing it (as admin).
        var downloadPromise = gpii.iod.fileDownload(that.packageData.url, that.installation.localPackage);
        fluid.promise.follow(downloadPromise, promise);
    } else {
        fs.copyFile(that.packageData.url, that.installation.localPackage, function (err) {
            if (err) {
                promise.reject({
                    isError: true,
