...
fluid.defaults("gpii.iod.packageInstaller", {
    ...
    invokers: {
        ...
        executeCommand: {
            ...
            // PackageInvocation, command, args
            args: ["{that}", "{iod}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
        },
...
gpii.iod.downloadInstaller = function (that, installation, packageData) {
    ...
    if (packageData.installCommands.download) {
        ...
        if (packageData.installerSource) {
            if (/^https?:\/\//.test(packageData.installerSource)) {
                ...
                fs.copyFile(packageData.url, installation.installerFile, function (err) {
                    if (err) {
                        promise.reject({
                            isError: true,
 ...
 * @param {Component} that The gpii.iod.installer instance.
 * @param {Component} iod The gpii.iod instance.
 * @param {PackageInvocation} invocation How the command is invoked.
 * @param {String} command The command.
 * @param {Array<String>|String} args [optional] The arguments (overrides `invocation.args`).
 * @return {Promise} Resolves when complete.
 */
gpii.iod.executeCommand = function (that, iod, invocation, command, args) {

    if (typeof(invocation) === "string") {
        // Can be expressed as a string, where it only specifies the arguments.
        invocation = { args: invocation };
    } else {
        // Take a copy to modify.
        invocation = Object.assign({}, invocation);
    }

    if (args) {
        invocation.args = fluid.makeArray(args);
    }

    invocation = gpii.iod.expand(invocation, that.installation);
    command = command ? gpii.iod.expand(command, that.installation) : invocation.command;

    var promise;
    if (invocation.elevate && that.invokeElevated) {
        promise = that.invokeElevated(invocation, command, invocation.args);
    } else {
        if (invocation.elevate) {
            fluid.log(fluid.logLevel.WARN, "Running elevated commands is not supported on this operating system.");
        }

        promise = fluid.promise();
        fluid.log("spawning: " + command + " ", invocation.args);
        var child = child_process.spawn(command, fluid.makeArray(invocation.args), {
            stdio: "inherit"
        });
