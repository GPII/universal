...
gpii.iod.requirePackage = function (that, packageRequest) {
    ...
    that.packages.getPackageInfo(packageRequest).then(function (packageInfo) {
        var isInstalled = that.packages.checkInstalled(packageInfo);
        if (isInstalled) {
            promise.resolve(false);
        } else {
            that.initialiseInstallation(packageInfo).then(function (installation) {
 ...
 * @param {Component} that The gpii.iod instance.
 * @param {PackageInfo} packageInfo The package data.
 * @return {Promise} Resolves with a gpii.iod.installer instance.
 */
gpii.iod.initialiseInstallation = function (that, packageInfo) {
    fluid.log("IoD: Initialising installation for " + packageInfo.name);

    // See if it's already been loaded
    var installation = fluid.find(that.installations, function (inst) {
        return inst.packageName === packageInfo.name ? inst : undefined;
    });
...
gpii.iod.uninitialiseInstallation = function (that, wait) {
    ...
    var uninstall = function () {
        ...
        if (!inSession) {
            ...
            var installation = fluid.find(that.installations, function (inst) {
                return !inst.required && !inst.removed ? inst : undefined;
            });
...
gpii.iod.uninstallPackage = function (that, installation) {
    ...
    if (installation && installation.installer) {
        ...
        initPromise = that.initialiseInstallation(installation.packageInfo);
    }
