...
fluid.defaults("gpii.metrics", {
    ...
    members: {
        sessionSolutions: {},
        // Is the noUser (or nobody) logged on?
        isAnonymousUser: true,
        // Incrementing sub-session id.
        subSessionIncrementer: 0,
        // When the current sub-session began
        subSessionStart: undefined
    },
    events: {
        "onStartMetrics": null,
        "onStopMetrics": null,
    ...
    listeners: {
        "onActive": [{
            func: "{that}.logMetric",
            args: ["inactive-stop", {duration: "{arguments}.0"}]
        }, {
        ...
        "{lifecycleManager}.events.onSessionStart": [
            {
                ...
                args: [
                    ...
                    "SessionStart",
                    {
                        ...
                        gpiiKey: "{arguments}.1",
                        session: "{that}.eventData.sessionID"
                    }
                ]
            }, {
                func: "{that}.startSubSession"
 ...
 * @param {Object} current - The current preferences map for the active context.
 * @param {Object} previous - The previous preferences map for the active context.
 */
 ...
 */
gpii.metrics.sessionStarted = function (that, eventLog, gpiiKey) {
    eventLog.eventData.sessionID = fluid.allocateGuid();
    that.isAnonymousUser = (gpiiKey === "noUser");
    if (that.isAnonymousUser) {
        delete eventLog.eventData.gpiiKey;
    } else {
        eventLog.eventData.gpiiKey = gpiiKey;
    }
};
 ...
 * @param {String} sessionID - Session ID.
 */
gpii.metrics.sessionStopped = function (that, eventLog, sessionID) {
    that.isAnonymousUser = true;
    delete eventLog.eventData.gpiiKey;
...
gpii.metrics.startSubSession = function (that, eventLog) {
    that.subSessionStart = process.hrtime();
...
gpii.metrics.stopSubSession = function (that, eventLog) {
    that.logMetric("subsession-end", {
        ...
        duration: process.hrtime(that.subSessionStart)[0]
    });
