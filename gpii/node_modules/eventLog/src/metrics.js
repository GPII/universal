...
gpii.metrics.logonStateChanged = function (that, eventLog, lifecycleManager, oldValue, newValue) {
    if (newValue.type === "login") {
        if (newValue.inProgress) {
            ...
            that.isAnonymousUser = (newValue.gpiiKey === "noUser");
            ...
            eventLog.eventData.keyIn = true;
        } else {
            // Login is complete, so all further events are unrelated to key-in. There's a 10 second grace period for
            // events that may occur shortly after, however.
            eventLog.eventData.keyIn = "after";
            setTimeout(function () {delete eventLog.eventData.keyIn;}, 10000);
        }
