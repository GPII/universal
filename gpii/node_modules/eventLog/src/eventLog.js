...
fluid.defaults("gpii.eventLog", {
    ...
    listeners: {
        "onCreate.logFile": {
            ...
            args: ["{that}","{metrics}.options.disable"]
        },
        "onCreate.log": {
 ...
 *
 * @param {Component} that - The gpii.eventLog instance.
 * @param {Boolean|String} disable true-like to disable logging, "errors-only" to only log errors.
 * @return {String} The path to the new log file.
 */
gpii.eventLog.getLogFile = function (that, disable) {
    var logPath;
    if (!process.env.GPII_EVENT_LOG_DIRECTORY) {
        logPath = process.env.GPII_EVENT_LOG || that.logFilePath || that.options.logDestination;
    }
    var errorsOnly = disable === "errors-only";
    if (errorsOnly) {
        that.logLevel = fluid.logLevel.FAIL;
    }

    if (disable && !errorsOnly) {
        fluid.log(fluid.logLevel.WARN, "Event logging is disabled");
        that.logLevel = fluid.logLevel.FATAL;
...
gpii.eventLog.writeLog = function (that, level, event) {
    ...
    fluid.log(fluid.logLevel.INFO, event);
...
gpii.eventLog.recordDuration = function (that, event) {
    var endEvent = that.metrics.options.durationEvents[event.event];
    if (endEvent) {
