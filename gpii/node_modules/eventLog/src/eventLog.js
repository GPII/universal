...
gpii.eventLog.logTimed = function (module, event, data, timerId, timeField) {
    data = data ? fluid.copy(data) : {};
    timeField = timeField || "time";
    data[timeField] = gpii.eventLog.stopTimer(timerId);
...
gpii.eventLog.writeLog = function (event) {
    ...
    if (!gpii.eventLog.logFilePath) {
        gpii.eventLog.logFilePath = gpii.eventLog.getLogFileName();
    }
...
gpii.eventLog.stopTimer = function (id) {
    var now = Date.now();
    var startTime;
    if (gpii.eventLog.hasOwnProperty(id)) {
