...
fluid.defaults("gpii.eventLog", {
    ...
    invokers: {
        ...
        createLinkId: "fluid.allocateGuid"
    },
    members: {
 ...
 * @param level {Object} [optional] Level of the log, see fluid.logLevelsSpec [FATAL,FAIL,WARN,IMPORTANT,INFO,TRACE].
 */
gpii.eventLog.log = function (that, moduleName, event, data, closeEvent, level) {
    ...
    if (that.pendingLogs[id]) {
        // There's already a pending entry for this event
        gpii.eventLog.logDeferred(that, moduleName, event, data);
    } else {
        var linkId = null;
        if (closeEvent) {
            // Defer the close event, so it always gets called (and with the same linkId).
            linkId = that.createLinkId();
            gpii.eventLog.defer(that, moduleName, closeEvent, data, linkId);
        }
 ...
 * @param level {Object} [optional] Level of the log, see fluid.logLevelsSpec [FATAL,FAIL,WARN,IMPORTANT,INFO,TRACE].
 */
gpii.eventLog.defer = function (that, moduleName, event, data, linkId, level) {
    if (!level && fluid.isLogLevel(linkId)) {
        level = linkId;
        linkId = null;
    }
    var id = moduleName + "#" + event;
    that.pendingLogs[id] = gpii.eventLog.createLogObject(moduleName, event, data, linkId, level);
};
 ...
 */
gpii.eventLog.logDeferred = function (that, moduleName, event, data) {
    if (arguments.length > 1) {
        var id = moduleName + "#" + event;
        var eventObject = that.pendingLogs[id];
        if (eventObject) {
            delete that.pendingLogs[id];
            fluid.extend(eventObject.data, data);
            gpii.eventLog.writeLog(that, eventObject.level, eventObject);
        } else {
            fluid.log("logDeferred: Unknown deferred event", id);
        }
    } else {
        // Do all of them, LIFO.
        var events = fluid.flatten(that.pendingLogs).reverse();
 ...
 * @param err The error.
 */
gpii.eventLog.gotError = function (err) {
    var eventLog = fluid.queryIoCSelector(fluid.rootComponent, "gpii.eventLog");
    if (eventLog.length > 0) {
        eventLog[0].logError(eventLog[0], null, "Failure", err);
    }
};
