...
fluid.defaults("gpii.eventLog", {
    ...
    listeners: {
        ...
        "{lifecycleManager}.events.onSessionStop": [{
            ...
            namespace: "eventLog.session",
            funcName: "gpii.eventLog.sessionStopped",
            args: ["{that}", "{arguments}.1.id"]
        }],
        "{lifecycleManager}.events.onSessionSnapshotUpdate": {
            namespace: "eventLog",
            funcName: "gpii.eventLog.snapshotUpdate",
            args: ["{that}", "{arguments}.1.id", "{arguments}.2"]
        }
    }
});

gpii.eventLog.snapshotUpdate = function (that, sessionID, originalSettings) {
    ...
    fluid.each(ids, function (id) {
        if (!solutionIDs[id]) {
            gpii.eventLog.log(that, "lifecycle", "solution-applied", {
                solutionID: id
            });
            solutionIDs[id] = true;
        }
...
gpii.eventLog.sessionStopped = function (that, sessionID) {
