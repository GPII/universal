...
fluid.defaults("gpii.flowManager.untrusted", {
    ...
    invokers: {
        setSettings: {
            funcName: "gpii.flowManager.untrusted.setSettings",
            args: [
                "{that}.untrustedSettingsDataSource",
                "{arguments}.0", // gpiiKey
                "{arguments}.1", // preferences
                "{that}.events.onSaveDone",
                "{that}.events.onPrefsUpdateError"
            ]
        }
    },
    events: {
        onSaveDone: null,
        onPrefsUpdateError: null
    },
    listeners: {
        "{lifecycleManager}.events.onPrefsUpdated": {
            namespace: "savePrefsToCloud",
            listener: "{that}.setSettings"
        },
        "onSaveDone.log": {
            listener: "fluid.log",
            args: ["FlowManager: save preferences to the cloud for the GPII key: ", "{arguments}.0.gpiiKey", ": ", "{arguments}.0.message"]
        },
        "onPrefsUpdateError.log": {
            listener: "fluid.log",
            args: ["FlowManager: error at saving preferences to the cloud: ", "{arguments}.0"]
        }
    }
});
...
gpii.flowManager.untrusted.setSettings = function (untrustedSettingsDataSource, gpiiKey, preferences, onSaveDone, onError) {
    fluid.log("=== setSettings", gpiiKey, preferences);
    var saveResult = untrustedSettingsDataSource.set(gpiiKey, preferences);
    saveResult.then(function (result) {
        fluid.log("=== save result", result);
        onSaveDone.fire(result);
    }, onError.fire);
};
