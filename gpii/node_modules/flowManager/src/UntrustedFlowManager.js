...
gpii.flowManager.untrusted.settingsDataSource.findValidAccessToken = function (that, userToken) {
    var accessToken = fluid.get(that, [userToken, "accessToken"]);
    var expiresIn = gpii.oauth2.getExpiresIn(fluid.get(that, [userToken, "timestampExpires"]));
    ...
    if (!accessToken || !expiresIn || expiresIn < that.options.minAccessTokenLifeTimeInSecond) {
        ...
        var mapper = function (accessTokenObj) {
            ...
            var timestampExpiresFromCloud = gpii.oauth2.getTimestampExpires(accessTokenObj.expiresIn);
 ...
 * @return {Promise} A promise whose resolved value is a valid access token
 */
gpii.flowManager.untrusted.settingsDataSource.save = function (that, userToken, accessToken, timestampExpires) {
    fluid.set(that, [userToken, "accessToken"], accessToken);
    fluid.set(that, [userToken, "timestampExpires"], timestampExpires);
};
