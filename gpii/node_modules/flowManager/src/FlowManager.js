...
fluid.defaults("gpii.flowManager.local", {
    ...
    members: {
        ...
        */
        defaultSettings: null, // {Object}. The content of the reset to default file
        defaultLifecycleInstructions: null, // {Object}. For the reset action at key-in
        defaultSnapshot: null // {Object}. For reset actions at system startup and explicit reset request
    },
    contextAwareness: {
    ...
    distributeOptions: {
        ...
        "userLogonHandling.defaultSnapshot": {
            record: "{gpii.flowManager.local}.defaultSnapshot",
            target: "{that gpii.lifecycleManager.userLogonHandling.stateChangeHandler}.options.members.defaultSnapshot"
        },
        "userLogonHandling.defaultLifecycleInstructions": {
            record: "{gpii.flowManager.local}.defaultLifecycleInstructions",
            target: "{that gpii.lifecycleManager.userLogonHandling.stateChangeHandler}.options.members.defaultLifecycleInstructions"
        },
    ...
    listeners: {
        ...
        // 2. Calculate defaultLifecycleInstructions and defaultSnapshot based on the default settings and assign all
        // these data to member options.
        ...
        "getDefaultSettingsData.convertDefaultSettings": {
            ...
            args: ["{that}", "{arguments}.0", "{that}.events.flowManagerReady"],
        ...
        "noUserLoggedIn.resetAtStart": {
            ...
            args: ["{lifecycleManager}", "{that}.options.resetAtStart", "{that}.defaultSnapshot", "{that}.events.resetAtStartSuccess", "{that}.events.resetAtStartError"]
        },
...
gpii.flowManager.convertDefaultSettingsToSnapshot = function (that, defaultSettings, flowManagerReadyEvent) {
    if (defaultSettings) {
        ...
        matchMakingPromise.then(function (payload) {
            ...
            that.defaultLifecycleInstructions = gpii.transformer.configurationToSettings(payload.matchMakerOutput.inferredConfiguration[activePrefsSetName], payload.solutionsRegistryEntries);
            // convert lifecycle instructions to snapshots
            that.defaultSnapshot = fluid.transform(that.defaultLifecycleInstructions, function (handlerResponse) {
                return gpii.settingsHandlers.settingsPayloadToChanges(handlerResponse);
            });
            that.defaultSettings = defaultSettings;
            flowManagerReadyEvent.fire();
        });
    } else {
        flowManagerReadyEvent.fire();
    }
