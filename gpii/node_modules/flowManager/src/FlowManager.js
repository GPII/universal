...
fluid.defaults("gpii.flowManager.local", {
    ...
    listeners: {
        ...
        // Squirrel away a reference to this flowManager
        "onCreate.stashFlowManager": {
            funcName: "gpii.flowManager.local.stashFlowManager",
            args: ["{that}"]
        },
...
/**
 * FOR DEBUGGING:  Stash a reference to the local flow manager (using array in
 * case there are multiples.
 * @param {Component} that - An instance of gpii.flowManager.local
 */
gpii.flowManager.local.stashFlowManager = function (that) {
    debugger;
    if (!gpii.flowManager.local.flowManagers) {
        gpii.flowManager.local.flowManagers = [];
    }
    gpii.flowManager.local.flowManagers.push(that);
    debugger;
};
