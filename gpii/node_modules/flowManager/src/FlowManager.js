...
fluid.defaults("gpii.flowManager.local", {
    ...
    // For retrieving the solutions registry from the source code repository,
    // distributed from gpii.flowManager.config.local.base
    solutionsRegistryRepositoryPrefix: "",
    solutionsRegistryRepositorySuffix: "",
    solutionsRegistryRepositoryFile: "",
    members: {
    ...
    listeners: {
        ...
        "onCreate.computeSolutionRegistryUrl": {
            funcName: "gpii.flowManager.local.computeSolutionRegistryUrl",
            args: ["{that}"]
        },
 ...
 * @param {Component} that - An instance of gpii.flowManager.local.
 * @return {Promise} - A promise: when resolved with no error, sets the LFM's
 * solutionRegistryDataSource URL.  If there is an error, the URL left undefined.
 */
gpii.flowManager.local.computeSolutionRegistryUrl = function (that) {
    var revisionPromise = that.gpiiRevisionRequester.getRevision();
    revisionPromise.then(function (revision) {
        if (!revision.isError) {
            that.solutionsRegistryDataSource.options.URL =
                that.options.solutionsRegistryRepositoryPrefix +
                revision.sha256 +
                that.options.solutionsRegistryRepositorySuffix +
                that.options.solutionsRegistryRepositoryFile;
        }
    });
