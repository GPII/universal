...
fluid.defaults("gpii.flowManager.local", {
    ...
    listeners: {
        ...
        //
        // Note 2: GPII-4273 - After the default settings have been converted,
        // this used to trigger a "flowManagerReady.loginWithNoUser" event. With
        // the potential loading of the solutions registry from the source code
        // repository, that needs to happen just before the loginWithNoUser, and
        // is handled via the getDefaultSettingsData.loadSolutionsFromRepository
        // listener below.
        ...
        "getDefaultSettingsData.loadSolutionsFromRepository": {
            listener: "gpii.flowManager.local.loadSolutionsFromRepository",
            args: ["{that}", "{that}.events.defaultSettingsDataLoaded"],
            priority: "after:convertDefaultSettings"
        },
...
gpii.flowManager.local.noUserLoggedIn = function (lifecycleManager, noUserLoggedInEvent) {
    fluid.log("TRACE:  LFM flowManagerReady.loginWithNoUser listener ... START");
...
gpii.flowManager.convertDefaultSettingsToSnapshot = function (that, defaultSettings) {
...
gpii.flowManager.local.resetAtStart = function (lifecycleManager, resetAtStart, defaultSettingsDataPromise, resetSuccessEvent, resetErrorEvent) {
    fluid.log("TRACE:  LFM noUserLoggedIn.resetAtStart listener ... START");
    defaultSettingsDataPromise.then(function (defaultSettingsData) {
        ...
        if (resetAtStart && defaultSnapshot) {
            fluid.log("TRACE: Resetting on system startup to default settings: ", defaultSnapshot);
            var resetPromise = lifecycleManager.restoreSnapshot(defaultSnapshot);
            resetPromise.then(function () {
                fluid.log("TRACE: Resetting on system startup completes successfully.");
                resetSuccessEvent.fire();
            }, function (error) {
                fluid.log("TRACE: Resetting on system startup fails with the error: ", error);
                resetErrorEvent.fire(error);
            });
        } else {
            fluid.log("TRACE:  LFM noUserLoggedIn.resetAtStart listener success ... END");
            resetSuccessEvent.fire();
        }
    });
};
...
/**
 * Retrieve the revision of the repository from the CBFM and construct the full
 * URL of the associated solutions registry document
 * @param {Component} that - An instance of gpii.flowManager.local.
 * @param {Component} that.gpiiRevisionRequester - An instance of
 *      gpii.flowManager.revisionRequester used to access the Cloudbased
 *      FlowManager's "/revision" endpoint.
 * @param {Component} that.deviceReporter - An instance of
 *      gpii.deviceReporter used to for the platform information.
 * @param {Component} that.repositorySolutionsLoader - An instance of
 *      gpii.flowManager.repositorySolutionsLoader that does the actual fetch.
 * @param {Component} that.solutionsRegistryDataSource - An instance of
 *      kettle.dataSource whose fullSolutionsRegistry member will be set to the
 *      solutions registry fetched by the repositorySolutionsLoader, on success.
 *      If repositorySolutionsLoader is unsuccessful, the
 *      solutionsRegistryDataSource is left untouched.
 * @param {Event} defaultSettingsDataLoaded - Event that means that the default
 *      settings are configured, and that the solutions registry retrieval from
 *      the source code repository has been dealt with.  This is fired at the
 *      resolution of the promise returned.
 * @return {Promise} - A promise: when resolved with no error, loads the relevant
 *      solutions registry file from the source code repository.  It is stored
 *      in the respositorySolutionsLoader's solutionsRegistry member.
 */
gpii.flowManager.local.loadSolutionsFromRepository = function (that, defaultSettingsDataLoaded) {
    fluid.log("TRACE:  LFM onCreate gets github revision from CBFM ... START");
    var revisionPromise = that.gpiiRevisionRequester.getRevision();
    revisionPromise.then(function (revision) {
        fluid.log("TRACE:  LFM gets github revision from CBFM ... END");
        if (!revision.isError) {
            var platform = that.deviceReporter.platformReporter.reportPlatform();
            var gpiiRevision = revision.sha256;
            var fileName = platform.id + ".json5";
            fluid.log("TRACE:  LFM loads solutions registry from github ... START");
            var loadingPromise = that.repositorySolutionsLoader.getSolutions(gpiiRevision, fileName);
            loadingPromise.then(function () {
                fluid.log("TRACE:  LFM loads solutions registry from github (", that.solutionsRegistryDataSource.id, ") ... END");
                // If the repository's solutions registry was loaded
                // successfully, use it.  Otherwise use the older technique of
                // loading from the local hard drive.
                if (that.repositorySolutionsLoader.solutionsRegistry !== null) {
                    that.solutionsRegistryDataSource.fullSolutionsRegistry = that.repositorySolutionsLoader.solutionsRegistry;
                }
                defaultSettingsDataLoaded.fire();
            });
        } else {
            defaultSettingsDataLoaded.fire();
        }
