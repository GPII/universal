...
fluid.defaults("gpii.flowManager.local", {
    ...
    events: {
        ...
        defaultGpiiKeyLoggedIn: null,
        defaultSnapshotCreated: null,
        readyToReset: {
            events: {
                "defaultGpiiKeyLoggedIn": "defaultGpiiKeyLoggedIn",
    ...
    listeners: {
        ...
        // Login with the system default GPII key when GPII starts so that QSS is still operable
        "{kettle.server}.events.onListen": {
            listener: "gpii.flowManager.local.defaultGpiiKeyLoggedIn",
            args: ["{lifecycleManager}", "{that}.events.defaultGpiiKeyLoggedIn"],
            namespace: "loginWithDefaultGpiiKey"
        },
...
gpii.flowManager.local.defaultGpiiKeyLoggedIn = function (lifecycleManager, defaultGpiiKeyLoggedInEvent) {
    var promise = lifecycleManager.addToUserLogonRequestQueue({
        gpiiKey: gpii.defaultGpiiKey,
        logonState: "login"
    });
    promise.then(defaultGpiiKeyLoggedInEvent.fire);
};

gpii.flowManager.convertDefaultSettingsToSnapshot = function (that, defaultSettings, defaultSnapshotCreatedEvent) {
    if (defaultSettings) {
        var matchMakingPromise = that.privateMatchMaker.doMatch(gpii.defaultGpiiKey, defaultSettings);
        matchMakingPromise.then(function (payload) {
            ...
            var settings = gpii.transformer.configurationToSettings(payload.matchMakerOutput.inferredConfiguration[activeContextName], payload.solutionsRegistryEntries);
            // convert lifecycle instructions to snapshots
            that.options.defaultSnapshot = fluid.transform(settings, function (handlerResponse) {
...
gpii.flowManager.local.savePreferences = function (that, gpiiKey, preferences) {
    // Don't support the save when the active user is the system default GPII key
    if (gpiiKey && preferences && gpiiKey !== gpii.defaultGpiiKey) {
