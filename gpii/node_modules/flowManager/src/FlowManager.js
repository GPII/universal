...
fluid.defaults("gpii.flowManager.local", {
    ...
    listeners: {
        ...
        "{lifecycleManager}.events.preferencesUserUpdated": {
            ...
            listener: "gpii.flowManager.local.savePreferences",
...
gpii.flowManager.local.savePreferences = function (that, gpiiKey, preferences) {
    ...
    if (fluid.isPromise(savePromise)) {
        // For the untrusted flow manager that saves the updated preferences to the cloud
        savePromise.then(that.events.preferencesSavedSuccess.fire, that.events.preferencesSavedError.fire);
    } else {
        // For the all-in-local flow manager that does not save user updated preferences
        // and always fires preferencesSavedSuccess so that the PSPChannel can proceed.
