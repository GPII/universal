...
(function () {
    ...
    gpii.request.flowManager.onUserLogin = function (userToken, proxy, handler) {
        handler.events.onUserToken.fire(userToken);
    };

    gpii.request.flowManager.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
        ...
        lifecycleManager.start(lifecyclePayload,
            gpii.request.flowManager.logAndNotify("Lifecycle manager returned: ", event, function () {
                var msg = "User with token " + lifecyclePayload.userToken + " was successfully logged in.";
                that.notifyLogin(msg, lifecyclePayload.userToken);
                return msg;
            }
        ));
    };
    ...
    gpii.request.flowManager.notifyLogin = function (status, userToken, pcpInterface) {
        pcpInterface.notifyLogin(userToken);
        pcpInterface.sendUserMessage(status);
    };

    gpii.request.flowManager.notifyError = function (status, pcpInterface) {
        var message = (status && status.message) ? status.message : status;
        pcpInterface.sendUserMessage(message, "errorMessage");
    };
    ...
    fluid.defaults("kettle.requests.request.handler.userLogin", {
        ...
        invokers: {
            ...
            handle: {
                ...
                args: ["{request}.req.params.userToken", "{requestProxy}", "{that}"]
            },
            notifyLogin: {
                funcName: "gpii.request.flowManager.notifyLogin",
                args: ["{arguments}.0", "{arguments}.1", "{flowManager}.pcpInterface"]
            },
            notifyError: {
                funcName: "gpii.request.flowManager.notifyError",
