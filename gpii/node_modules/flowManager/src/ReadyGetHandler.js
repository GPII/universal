...
gpii.flowManager.cloudBased.ready.handler.reportError = function (that, prefsServerURL, reason) {
    that.events.onError.fire({
        isError: true,
        message: "Error connecting to Preferences Server at " + prefsServerURL + "; " + reason,
        statusCode: 503
    });
};
...
gpii.flowManager.cloudBased.ready.handler.isLive = function (prefsServerDataSource, that) {
    ...
    if (isSameHost) {
        gpii.flowManager.cloudBased.ready.handler.reportError(that, prefsServerDataSource.options.prefsServerURL, "FlowManager and Preferences server are the same host");
    } else {
        ...
        isLivePromise.then(function (result) {
            if (result.isReady) {
                ...
                gpii.flowManager.cloudBased.ready.handler.reportError(that, prefsServerDataSource.options.prefsServerURL, result.reason);
            }
        }, function (error) {
            gpii.flowManager.cloudBased.ready.handler.reportError(that, prefsServerDataSource.options.prefsServerURL, "'is live' failure: " + error);
        });
    }
};
