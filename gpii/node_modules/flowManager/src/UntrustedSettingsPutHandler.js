...
gpii.flowManager.cloudBased.untrustedSettings.put.handleRequest = function (preferencesDataSource, request, userToken, preferences, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiToken === userToken && authorization.allowUntrustedSettingsPut) {
            ...
            fluid.log("Unauthorized PUT request at cloudbased flowmanager /untrusted-settings due to one of these reasons: 1. authorization record is missing; 2. gpiiToken associated with the authorization does not match the in-used token " + userToken + "; 3. the token is unauthorized for using PUT method at /untrusted-settings endpoint.");
            request.events.onError.fire(gpii.oauth2.errors.unauthorized);
            return;
        }
    }, function () {
        fluid.log("Unauthorized PUT request at cloudbased flowmanager /untrusted-settings: the authorization record is not found for the token (" + userToken + ").");
        request.events.onError.fire(gpii.oauth2.errors.unauthorized);
        return;
    });
};
