...
gpii.flowManager.cloudBased.untrustedSettings.put.handleRequest = function (preferencesDataSource, request, userToken, preferences, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiKey === userToken && authorization.allowUntrustedSettingsPut) {
            ...
            fluid.log("CloudBased flowManager: unauthorized PUT request at /untrusted-settings due to one of these reasons: 1. authorization record is missing; 2. gpiiKey associated with the authorization does not match the in-used token " + userToken + "; 3. the access token is unauthorized for using PUT method at /untrusted-settings endpoint.");
