...
gpii.flowManager.cloudBased.untrustedSettings.put.handleRequest = function (preferencesDataSource, request, gpiiKey, preferences, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiKey === gpiiKey && authorization.allowUntrustedSettingsPut) {
            ...
            var updatePromise = preferencesDataSource.set(directModel, preferences, { writeMethod: "PUT" });
            fluid.log("The preferences set to be merged for the GPII key (", gpiiKey, ") is: ", preferences);
            ...
            fluid.log("CloudBased flowManager: unauthorized PUT request at /untrusted-settings due to one of these reasons: 1. authorization record is missing; 2. gpiiKey associated with the authorization does not match the in-used token " + gpiiKey + "; 3. the access token is unauthorized for using PUT method at /untrusted-settings endpoint.");
