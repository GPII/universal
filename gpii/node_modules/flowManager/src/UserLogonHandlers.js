...
fluid.defaults("gpii.flowManager.userLogin.handler", {
    ...
    invokers: {
        handleRequest: {
            ...
            args: ["{request}.req.params.gpiiKey", "{request}", "{lifecycleManager}", "{that}"]
        }
    }
});
 ...
 */
gpii.flowManager.userLogin.handleRequest = function (gpiiKey, request, lifecycleManager, that) {
...
fluid.defaults("gpii.flowManager.userLogout.handler", {
    ...
    invokers: {
        handleRequest: {
            ...
            args: ["{request}.req.params.gpiiKey", "{request}", "{flowManager}.lifecycleManager", "{that}"]
        }
    }
});
...
gpii.flowManager.userLogout.handleRequest = function (gpiiKey, request, lifecycleManager, that) {
    var activeGpiiKeys = lifecycleManager.getActiveSessionGpiiKeys(); // find currently logged in user:

    // if no user is logged in, or, if the logged-in user is "noUser"
    if (activeGpiiKeys.length === 0 || activeGpiiKeys.length === 1 && activeGpiiKeys[0] === "noUser") {
