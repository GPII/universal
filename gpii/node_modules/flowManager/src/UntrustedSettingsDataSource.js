...
fluid.defaults("gpii.flowManager.untrustedSettingsDataSource", {
    ...
    // This option should be distributed down from, e.g., gpii.flowManager.untrusted.config.development
    flowManagerBaseUrl: null,

    untrustedSettingsGetUrlTemplate: "%flowManagerBaseUrl/%gpiiKey/untrusted-settings/%device",
    untrustedSettingsPutUrlTemplate: "%flowManagerBaseUrl/%gpiiKey/untrusted-settings",
    accessTokenUrlTemplate: "%flowManagerBaseUrl/access_token",

    // The URLs below are calculated from the flow manager base URL and their templates.
    // These URLs are in the "options" block instead of "members" block is because distributeOptions source
    // paths must beginning with "options".
    untrustedSettingsGetUrl: {
        expander: {
            funcName: "fluid.stringTemplate",
            args: ["{that}.options.untrustedSettingsGetUrlTemplate", {
                flowManagerBaseUrl: "{that}.options.flowManagerBaseUrl"
            }]
        }
    },
    untrustedSettingsPutUrl: {
        expander: {
            funcName: "fluid.stringTemplate",
            args: ["{that}.options.untrustedSettingsPutUrlTemplate", {
                flowManagerBaseUrl: "{that}.options.flowManagerBaseUrl"
            }]
        }
    },
 ...
 */
gpii.flowManager.untrustedSettingsDataSource.get = function (that, gpiiKey, device) {
    var accessTokenPromise = gpii.flowManager.untrustedSettingsDataSource.findValidAccessToken(that, gpiiKey);
 ...
 */
gpii.flowManager.untrustedSettingsDataSource.set = function (that, gpiiKey, preferences) {
    var accessTokenPromise = gpii.flowManager.untrustedSettingsDataSource.findValidAccessToken(that, gpiiKey);
    var promiseTogo = fluid.promise();
 ...
 * @param that {Component} An instance of gpii.flowManager.untrustedSettingsDataSource
 * @param gpiiKey {String} The GPII key that the access token associates with
 * @param accessToken {String} The access token to be saved
 * @param timestampExpires {String} A timestampExpires to be saved
 * @return {Promise} A promise whose resolved value is a valid access token
 */
gpii.flowManager.untrustedSettingsDataSource.save = function (allAccessTokens, gpiiKey, accessToken, timestampExpires) {
