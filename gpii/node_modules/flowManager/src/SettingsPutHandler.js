    ...
    invokers: {
        handleRequest: {
            ...
            args: [
                ...
                "{gpii.flowManager.cloudBased}.oauth2DataStore",
                "{that}",
...
gpii.flowManager.cloudBased.settings.put.handleRequest = function (prefsServerDataSource, authGrantFinder, dataStore, request, gpiiKey, preferences) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiKey === gpiiKey && authorization.allowSettingsPut) {
            ...
            gpiiKeyPromise.then(function (data) {
                // If the GPII key already exists, update its prefs safe. Otherwise, create the GPII key and its prefs safe.
                if (data) {
                    handlerPromise = prefsServerDataSource.update(gpiiKey, "true", preferences);
                } else {
                    handlerPromise = prefsServerDataSource.create(gpiiKey, preferences);
                }
