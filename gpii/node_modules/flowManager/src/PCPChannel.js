...
gpii.pcpChannel.updatePreferences = function (pcpChannel, lifecycleManager, newPrefs, event) {
    ...
    fluid.each(newPrefs, function (spec, pref) {
        var path =  fluid.pathUtil.parseEL(pref);
        fluid.set(contextualPrefs, path, spec.value);
        // GPII-2739: The framework currently expect scoped common terms to be doubly nested
        //     This is a hack to get around that - but it should be properly fixed, meaning
        //     that either the doubly nested common term should be gone (probably preferable)
        //     or the singly nested common term should be removed. In either case, scoped terms
        //     should not appear twice.
        if (path[0].startsWith("http://registry.gpii.net/applications/")) {
            var extraPath = [path[0]].concat(path);
            fluid.set(contextualPrefs, extraPath, spec.value);
        }
        // end of GPII-2739 hack
    });
