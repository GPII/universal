...
gpii.flowManager.pcpChannel.filterSchema = function (schema) {
...
gpii.flowManager.pcpChannel.sessionToPCP = function (model, transformSpec) {
    ...
    var outModel = fluid.filterKeys(model, ["userToken", "preferences", "activeContextName"]);
    ...
    fluid.each(activeCommonTerms, function (troo, preference) {
        var outSchema = gpii.flowManager.pcpChannel.filterSchema(ontologyMetadata.flat.schemas[preference]);
        if (outSchema) {
            fluid.set(outModel, ["commonTermsMetadata", preference, "schema"], outSchema);
        } else {
            delete outModel.preferences.contexts["gpii-default"].preferences[preference];
        }
    });
