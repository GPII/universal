...
fluid.defaults("gpii.pcpChannel.sessionBinder", {
    ...
    modelListeners: {
        updatePreferences: {
            ...
            args: ["{flowManager}", "{pcpChannel}", "{flowManager}.lifecycleManager", "{change}.value"]
        },
        updateContext: {
...
gpii.pcpChannel.updatePreferences = function (flowManager, pcpChannel, lifecycleManager, newPrefs) {
    ...
    // fluid.log("Applying changes to session in updatePreferences"); // TODO KASPER
    // fluid.log(["preferences", "contexts", session.model.activeContextName, "preferences"],contextualPrefs ) // TODO KASPER
    var prefcpy = fluid.copy(pcpChannel.model.preferences);
    fluid.set(prefcpy, ["contexts", session.model.activeContextName, "preferences"], contextualPrefs);
    ...
    var inferredConfiguration = session.model.matchMakerOutput.inferredConfiguration;
    var newInferred = gpii.matchMakerFramework.utils.updateInferredConfiguration(session.model.preferences, inferredConfiguration);
    ...
    fullPayload.activeConfiguration = {
        inferredConfiguration: fluid.extend(true, {}, newInferred[session.model.activeContextName])
    };
    ...
    session.applier.change("forceContext", true);

    var updatePromise = lifecycleManager.update(fullPayload);
};
...
gpii.pcpChannel.sessionToPCP = function (model, transformSpec) {
    // console.log("Original model: "+JSON.stringify(model, null, 2)); TODO KASPER
    ...
    // console.log("Return model: "+JSON.stringify(outModel, null, 2)); TODO KASPER
...
/**
 * Adjusts the preference message payload to match the local model, e.g. from:
 * [ "preferences", "http://registry\\.gpii\\.net/common/magnification" ] to
 ...
 */
gpii.pcpChannel.adjustPreferenceMessage = function (message, pcpChannel) {
...
gpii.pcpChannel.receiveMessage = function (message, pcpChannel) {
    ...
    if (message.path && message.path[0] === "preferences") {
        gpii.pcpChannel.adjustPreferenceMessage(message)
    }
    // Modify to fit model if it is a change in preferences
