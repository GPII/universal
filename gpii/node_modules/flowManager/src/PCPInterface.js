...
(function () {
    ...
    */
    gpii.pcpInterface.sendUserMessage = function (socket, text, messageType) {
        if (socket === null) {
            fluid.log("No PCP connected, so discarding message: " + text);
            return;
        }
    ...
    /**
     * Notify the PCP client that a user has logged in.
     *
     * @param socket {Object} client socket connection
     * @param userToken {String} the token of the user who has just logged in
     * @param adjusters {Object} Adjusters can be passed to the PCP via the adjusters argument. If
     *    this is left empty, a ddefault set of adjusters will be passed to
     *    the PCP. The format of the adjusters is a map of settings names as keys and the deafult
     *    value as values.
     */
    gpii.pcpInterface.notifyLogin = function (socket, userToken, adjusters) {
        fluid.fail("Kasper, you bloody genious");
        if (socket === null) {
            fluid.log("No PCP connected, so is not attempting to notify of a login");
            return;
        }
    ...
    /**
     * Notify the PCP client that a user has logged out.
     *
     * @param socket {Object} client socket connection
     */
