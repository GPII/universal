...
fluid.defaults("gpii.flowManager.capture.handler", {
    gradeNames: ["kettle.request.http"], //, "gpii.flowManager.sessionAware"],
    invokers: {
        handleRequest: {
            func: "{flowManager}.capture.events.onPrepareCapture.fire",
            args: ["{request}.events.onSuccess", "{request}.events.onError"]

            // funcName: "gpii.flowManager.captureHandler",
            // args: ["{flowManager}", "{request}.events.onSuccess", "{request}.events.onError"]
        }
    }
});

gpii.flowManager.captureHandler = function (flowManager, onSuccess, onError) {
    flowManager.capture.getInstalledSolutions().then(onSuccess.fire, onError.fire);
};
