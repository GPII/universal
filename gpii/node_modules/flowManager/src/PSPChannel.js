...
gpii.pspChannel.sessionToPSP = function (model, transformSpec) {
    ...
    // Force a PSPChannel model change even if a read preference request receives an unchanged setting value
    outModel.readPreferenceCount = model.readPreferenceCount || 0;
    return outModel;
};
...
gpii.pspChannel.receiveMessage = function (message, pspChannel, lifecycleManager) {
    ...
    if (message.type === "pullModel") {
        lifecycleManager.readPreference(message.value.settingControls, pspChannel.events.preferenceReadSuccess, pspChannel.events.preferenceReadFail);
    }
};
