...
fluid.defaults("gpii.pspChannel", {
    ...
    members: {
        outputBlocked: null,
        // "defaultSettingControls" holds schemas containing default preferences with values
    ...
    invokers: {
        generateDefaultSettingControls: {
            ...
            args: ["{flowManager}.options.defaultSettings", "{that}.options.settingsSchema"]
        }
    },
    listeners: {
...
gpii.pspChannel.getPreferenceInfo = function (settingsSchemas, prefsKey, prefsValue) {
    var togo = {};
    var schemas = settingsSchemas.properties;

    if (!schemas || !prefsKey || prefsValue === undefined) {
        return undefined;
    };

    if (gpii.matchMakerFramework.utils.isApplicationTerm(prefsKey)) {
        // Handle application terms
        var solutionId = gpii.matchMakerFramework.utils.applicationIdFromTerm(prefsKey);
        fluid.each(prefsValue, function (val, innerPrefsKey) {
            ...
            if (gpii.matchMakerFramework.utils.isCommonTerm(innerPrefsKey)) {
                ...
                schema = fluid.get(schemas, [innerPrefsKey]);
                solutionName = fluid.get(schemas, [innerPrefsKey, "title"]);
                ...
                schema = fluid.get(schemas, [solutionId, "properties", innerPrefsKey]);
                solutionName = fluid.get(schemas, [solutionId, "title"]);
        ...
        // Handle common terms
        togo = {
            schema: fluid.get(schemas, [prefsKey]),
            solutionName: fluid.get(schemas, [prefsKey, "title"]),
