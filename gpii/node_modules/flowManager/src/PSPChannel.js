...
fluid.defaults("gpii.pspChannel", {
    ...
    // GPII-3828: this model relay reports default settings to psp clients when model.settingControls is empty.
    // This will be replaced with GPII-3318 (https://issues.gpii.net/browse/GPII-3318) implementation.
    modelRelay: {
        pspChannel: {
            // Passing the entire that.model rather than that.model.settingControls into the relay function is to
            // deal with the case when that.model === undefined. Passing that.model.settingControls in this case
            // will cause model to be auto-converted to undesired empty object.
            source: "{that}.model",
            target: "{that}.model",
            singleTransform: {
                type: "gpii.pspChannel.respondDefaultSettings",
                defaultSettings: "{flowManager}.options.defaultSettings",
                ontologyMetadata: "{ontologyHandler}.ontologyMetadata"
            },
            backward: "never",
            forward: {
...
gpii.pspChannel.generateDefaultSettingControls = function (defaultSettings, schemas) {
    ...
    // Set the setting defualt values from reset to standard file to schema.defalt field for each setting in
    ...
    fluid.each(defaultPreferences, function (defaultPrefsVal, defaultPrefsKey) {
        if (schemas[defaultPrefsKey]) {
            ...
            schemaWithDefault["default"] = defaultPrefsVal;
...
gpii.pspChannel.respondDefaultSettings = function (model, transformSpec) {
    var defaultSettings = transformSpec.defaultSettings;
    if (!model || !defaultSettings) {
        return model;
    } else {
        var originalSettingControls = fluid.copy(model.settingControls);
        if ($.isEmptyObject(originalSettingControls)) {
            var schemas = transformSpec.ontologyMetadata.flat.schemas;
            var defaultSettingControls = gpii.pspChannel.generateDefaultSettingControls(defaultSettings, schemas);
            fluid.set(model, ["settingControls"], defaultSettingControls);
        } else {
            var defaultPreferences = fluid.get(defaultSettings, ["contexts", "gpii-default", "preferences"]);
            var defaultPrefKeys = fluid.keys(defaultPreferences);
            fluid.each(originalSettingControls, function (settingBlock, settingTerm) {
                var unescapedSettingTerm = settingTerm.replace(/\\/g, "");

                // Match actual setting key with default keys. If there's a match, set schema.default to what's from the
                // default setting.
                if (defaultPrefKeys.includes(unescapedSettingTerm)) {
                    fluid.set(model, ["settingControls", settingTerm, "schema", "default"], defaultPreferences[unescapedSettingTerm]);
                }
            });
        }
        return model;
