...
fluid.defaults("gpii.pspChannel", {
    ...
    members: {
        outputBlocked: null,
        // The resolved value of "defaultSettingControlsPromise" holds schemas containing default preferences with
        // values defined in the reset to default file.
        defaultSettingControlsPromise: "@expand:gpii.pspChannel.generateDefaultSettingControlsPromise({flowManager}.defaultSettingsDataPromise, {that}.options.settingsSchema)",
        // Holds the resolved value of "defaultSettingControlsPromise". The resolution happens later when handling
        // the first pspChannel request instead of at the component construction.
 ...
 * }
 */
gpii.pspChannel.generateDefaultSettingControlsPromise = function (defaultSettingsDataPromise, schemas) {
    var defaultSettingControls = {};
    var promiseTogo = fluid.promise();

    defaultSettingsDataPromise.then(function (defaultSettingsData) {
        var defaultSettings = defaultSettingsData.defaultSettings;
...
gpii.pspChannel.getPreferenceInfo = function (solutionSchemas, prefsKey, prefsValue) {
    ...
    if (gpii.matchMakerFramework.utils.isApplicationTerm(prefsKey)) {
        ...
        fluid.each(prefsValue, function (val, innerPrefsKey) {
            var schema, solutionName, presentedCommonTerm;
            if (gpii.matchMakerFramework.utils.isCommonTerm(innerPrefsKey)) {
                ...
                solutionName = fluid.get(schema, ["title"]);
