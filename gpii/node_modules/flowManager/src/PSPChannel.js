...
fluid.defaults("gpii.pspChannel.sessionBinder", {
    ...
    modelListeners: {
        updatePreferences: {
            ...
            includeSource: "PSP",
            funcName: "gpii.pspChannel.updatePreferences",
            args: ["{pspChannel}", "{flowManager}.lifecycleManager", "{change}.value", "{pspChannel}.events.preferencesApplied"]
        },
        updateContext: {
...
gpii.pspChannel.updatePreferences = function (pspChannel, lifecycleManager, newPrefs, event) {
    fluid.log("Received preferences update from PSP UI of ", newPrefs);

    // Grab current session
    var session = lifecycleManager.getSession(lifecycleManager.getActiveSessionGpiiKeys());

    // tweak payload to be in preference set format and update session
    var contextualPrefs = {};
...
gpii.pspChannel.filterSchema = function (schema) {
...
gpii.pspChannel.emitSettingControl = function (settingControls, schema, prefVal, keySegs, liveness, solutionName) {
    ...
    if (schema) {
        settingControls[fullKey] = {
            value: prefVal,
            schema: gpii.pspChannel.filterSchema(schema),
 ...
 *     @member {Object} settingControls - A hash keyed by preference path, whose values are {SettingControl} objects.
 *     @member {Object} preferences - A filtered skeleton of the user's preferences document, just containing the names
 * of `contexts.
 */
