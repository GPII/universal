...
gpii.flowManager.cloudBased.oauth2settings.handleRequest = function (request, handler, dataStore) {
    ...
    if (!accessToken) {
        ...
        var authorizationPromise = dataStore.findWebPrefsConsumerAuthorizationByAccessToken(accessToken);
        authorizationPromise.then(function (authorization) {
            if (!authorization) {
                gpii.oauth2.sendUnauthorized(request);
            } else {
                fluid.log("Got AUTH DECISION: " + JSON.stringify(authorization, null, 2));
                ...
                handler.events.onUserToken.fire(authorization.userGpiiToken);
