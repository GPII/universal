...
// Add the liveness (/ready) and the readiness (/health) endpoints only when the cloud based flow manager is running as a
// standalone WITHOUT having the preferences server running on the same server.
gpii.flowManager.cloudBased.cloudStatusGradeFinder = function (preferencesServer) {
    return !preferencesServer ? "gpii.flowManager.cloudBased.cloudStatus" : undefined;
};

// The addon grade that adds the liveness (/ready) and the readiness (/health) endpoints to the cloud based flow manager only
// when it runs as a separate server from the preferenes server. /ready endpoint on CBFM will ping the /ready endpoint
