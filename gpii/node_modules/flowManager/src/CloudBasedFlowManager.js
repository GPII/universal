...
fluid.defaults("gpii.flowManager.cloudBased.oauth2", {
    ...
    distributeOptions: {
        setDataStoreOnAuthGrantFinder: {
            ...
            target: "{that > authGrantFinder}.options.components.authorizationService.options.components.dataStore"
        },
        setDataStoreOnUserService: {
            record: "{gpii.oauth2.dataStore}",
            target: "{that > userService}.options.components.dataStore"
        }
    },
    requestHandlers: {
        ...
        // The endpoint for GPII app installations. The filetered lifecycle instructions
...
gpii.flowManager.cloudBased.untrustedSettings.handleRequest = function (onError, request, userToken, deviceString, userService, authGrantFinder) {
    request.privacySettings = {};
    ...
    promisesSequence.then(function (responses) {
        ...
        if (!authorization || authorization.gpiiToken !== userToken || !authorization.allowUntrustedSettings) {
            fluid.log("Unauthorized due to one of these reasons: 1. authorization record is missing; 2. gpiiToken " + authorization.gpiiToken + " associated with the authorization does not match the in-used token " + userToken + "; 3. the token is unauthorized for /untrusted-settings endpoint.");
            request.events.onError.fire(gpii.oauth2.errors.unauthorized);
            return;
        }
