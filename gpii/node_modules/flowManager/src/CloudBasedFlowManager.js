...
fluid.defaults("gpii.flowManager.cloudBased.oauth2", {
    ...
    components: {
        ...
        // e.g. at the paths /access_token as well as mounting infusion at /infusion
    ...
    requestHandlers: {
        // The endpoint that returns the lifecycle instructions.
        untrustedSettings: {
        ...
        untrustedPreferences: {
            route: "/untrusted-preferences/:userToken",
            method: "put",
            type: "gpii.flowManager.cloudBased.untrustedPreferences.handler"
        },
        userLogin: null,
        userLogout: null,
        getUserToken: null
...
// The handler for endpoint /untrusted-settings that returns the lifecycle instructions.
...
gpii.flowManager.cloudBased.untrustedSettings.handleRequest = function (request, userToken, deviceString, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiToken === userToken && authorization.allowUntrustedSettings) {
            gpii.flowManager.cloudBased.settings.handleRequest(request.events.onError, userToken, request, deviceString);
        } else {
            fluid.log("Unauthorized at /untrusted-settings due to one of these reasons: 1. gpiiToken associated with the authorization (" + userToken + ") does not match the in-used token (" + userToken + "); 2. the token is unauthorized for using /untrusted-settings endpoint.");
            request.events.onError.fire(gpii.oauth2.errors.unauthorized);
            return;
        }
    }, function (error) {
        fluid.log("Unauthorized at /untrusted-settings for the token (" + userToken + ") with error: " + error);
        request.events.onError.fire(gpii.oauth2.errors.unauthorized);
        return;
    });
};
