...
// TODO do the privacy filtering in matchMakerOutput rather than
// activeConfiguration, and before gpii.contextManager.evaluateMatch

// TODO filter all configurations in matchMakerOutput

// TODO ensure that gpii.flowManager.cloudBased.privacy.filter is
// usable by both the OAuth2 protected endpoint and the endpoint used
// by the untrusted Flow Manager

// TODO currently we pass in a single selectedPreferences, we will
// need to instead provide a map of solutionId to selectedPreferences

// TODO unit test gpii.flowManager.cloudBased.privacy.filter
...
gpii.flowManager.cloudBased.privacy.filter = function (contextPayload, selectedPreferences) {
    ...
    var configurations = contextPayload.matchMakerOutput.inferredConfiguration;
    fluid.each(configurations, function (config, configName) {
        fluid.log("|||||| PRIVACY FILTER configName: " + configName);
        var filteredApplications = fluid.transform(config.applications, function (application) {
            // TODO use selectedPreferences for the application's solution id
