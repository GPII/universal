...
fluid.defaults("kettle.requests.request.handler.settings", {
    gradeNames: ["fluid.littleComponent", "gpii.request.flowManager.nonPrivacyFilteredMatchMakingRequest", "autoInit"],
    invokers: {
        handle: {
...
// TODO Move the privacy and untrusted settings code to the bottom of the file

gpii.request.flowManager.onUntrustedSettings = function (onError, request, handler, userToken, deviceString, dataStore) {
    var authorizedClients = dataStore.findAuthorizedClientsByGpiiToken(userToken);
    ...
    fluid.log("**** privacySettings **** " + JSON.stringify(request.privacySettings));
    handler.events.onPrivacySettings.fire();
    // TODO Is this the best way to reuse the existing onSettings?
    gpii.request.flowManager.onSettings(onError, userToken, handler, deviceString);
};
...
fluid.defaults("kettle.requests.request.handler.untrustedSettings", {
    gradeNames: ["fluid.littleComponent", "gpii.request.flowManager.privacyFilteredMatchMakingRequest", "autoInit"],
    invokers: {
        handle: {
            funcName: "gpii.request.flowManager.onUntrustedSettings",
            args: [
                ...
                "{gpii.flowManager.cloudBased.oauth2}.oauth2DataStore"
            ],
            dynamic: true
        },
