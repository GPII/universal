...
fluid.defaults("gpii.flowManager.cloudBased.untrustedPreferences.handler", {
    gradeNames: ["kettle.request.http"],
    invokers: {
        handleRequest: {
            funcName: "gpii.flowManager.cloudBased.untrustedPreferences.handleRequest",
            args: [
...
gpii.flowManager.cloudBased.untrustedPreferences.messages = {
    success: "Successfully updated."
};

gpii.flowManager.cloudBased.untrustedPreferences.handleRequest = function (preferencesDataSource, request, userToken, preferences, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiToken === userToken && authorization.allowUntrustedPreferences) {
            ...
            queryPromise.then(function (origPreferences) {
                ...
                updatePromise.then(function (response) {
                    ...
                    request.events.onSuccess.fire({
                        ...
                        message: gpii.flowManager.cloudBased.untrustedPreferences.messages.success
                    });
            ...
            fluid.log("Unauthorized due to one of these reasons: 1. authorization record is missing; 2. gpiiToken associated with the authorization does not match the in-used token " + userToken + "; 3. the token is unauthorized for /untrusted-preferences endpoint.");
            request.events.onError.fire(gpii.oauth2.errors.unauthorized);
            return;
        }
    }, function () {
        fluid.log("Unauthorized at /untrusted-preferences: the authorization record is not found for the token (" + userToken + ").");
        request.events.onError.fire(gpii.oauth2.errors.unauthorized);
        return;
    });
};
