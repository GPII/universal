...
gpii.flowManager.cloudBased.settings.get.handleRequest = function (request, gpiiKey, deviceString, authGrantFinder) {
    ...
    authorizationPromise.then(function (authorization) {
        if (authorization && authorization.gpiiKey === gpiiKey && authorization.allowSettingsGet) {
            ...
            request.events.onGpiiKey.fire(gpiiKey);
            request.events.onDeviceContext.fire(deviceContext);
        } else {
...
//     ]);
//     fluid.log("cloudBased flowManager: /settings endpoint sending settings ", settings);
//     event.fire(settings);
// };
