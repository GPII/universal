...
(function () {
    ...
    request.onMatchUpdateHandler = function (lifecycleManagerServerDataSource, event, token, match, preferences) {
        lifecycleManagerServerDataSource.set({
            ...
            var unfilteredPreferences = {};
            fluid.each(match, function (solution) {
                var result = response[solution.id];
                if (typeof result === "undefined") {
                    return;
                }
                fluid.each(solution.settingsHandlers, function (sh) {
                    fluid.each(sh.capabilities, function (capability) {
                        unfilteredPreferences[capability] = result;
                    });
                });
            });
            // TODO map from nested to flat.
            // match = match;
            // preferences = preferences;
            request.logAndNotify("Lifecycle manager returned: ", event)(
                unfilteredPreferences);
        });
    };
