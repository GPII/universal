...
(function () {
    ...
    //     gpii.matchMakerFramework.match(preferences, device, event);
    ...
    gpii.request.flowManager.getMatch = function (that, matchMakerFramework, preferences, device) {
        matchMakerFramework.preProcess(that.userToken, preferences, device);
        // matchMakerDataSource.set(null, {
        //     preferences: preferences,
        //     device: device
        // }, gpii.request.flowManager.logAndNotify(
    ...
    fluid.defaults("gpii.request.flowManager.token", {
        ...
        invokers: {
            ...
            getMatch: {
                funcName: "gpii.request.flowManager.getMatch",
                args: [ "{that}", "{gpii.matchMakerFramework}", "{arguments}.0", "{arguments}.1"]
                                                               //prefs, device
            }
        },
        events: {
    ...
    fluid.defaults("gpii.request.flowManager.sessionAware", {
        ...
        components: {
            lifecycleManager: "{gpii.lifecycleManager}"
        },
        invokers: {
            getActiveSessionTokens: {
                func: "{that}.lifecycleManager.getActiveSessionTokens"
            },
            withSession: {
