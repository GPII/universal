...
(function () {
     ...
     */
    gpii.request.flowManager.getSolutions = function (solutionsRegistryDataSource, deviceContext, event, webEvent) {
        ...
        if (deviceContext.OS.id !== "web") {
            // if we're getting solutions for an installed system, fetch web solutions as well
            solutionsRegistryDataSource.get({
                os: "web"
            }, function (solutions) {
                // var solutionsRegistryEntries = gpii.matchMakerFramework.filterSolutions(solutions, deviceContext);
                fluid.log("Fetched web solutions registry entries: " + JSON.stringify(solutions, null, 4));
                webEvent.fire(solutions);
            });
        } else {
            webEvent.fire({});
        }
    ...
    fluid.defaults("gpii.request.flowManager.matchMakingRequest", {
        ...
        invokers: {
            ...
            getSolutions: {
                ...
                args: [ "{flowManager}.solutionsRegistryDataSource", "{arguments}.0", "{that}.events.onSolutions", "{that}.events.onWebSolutions"]
            }
        },
        events: {
            ...
            onMatchDone: null,
            // Events signalling successful login and logout, respectively
            ...
            onReadyToMatch: {
                ...
                args: [{
                    ...
                    webSolutionsRegistryEntries: "{arguments}.webSolutions.0"
                }]
            }
        },
        listeners: {
    ...
    gpii.request.flowManager.sessionAware.withSession = function (that, lifecycleManager, events, callback, failMessage) {
        var userTokens = lifecycleManager.getUserTokens();
