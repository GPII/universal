...
(function () {
    ...
    gpii.flowManager.loadSolutionsRegistry = function (flowManager) {
        var resolvedFolder = fluid.module.resolvePath(flowManager.options.solutionsRegistrySourceDir);
        flowManager.options.fullSolutionsRegistry = require(resolvedFolder);
    };
     ...
     *
     * @fullSolutionsRegistry (Object) - a folder containing the full solutions registry
     ...
     */
    gpii.flowManager.getSolutions = function (fullSolutionsRegistry, deviceContext, event) {
        var os = fluid.get(deviceContext, "OS.id");
            // version = fluid.get(deviceContext, "OS.version"); // TODO: Take into account

        var filteredEntries = gpii.matchMakerFramework.filterSolutions(fullSolutionsRegistry[os], deviceContext);
        fluid.log("Fetched filtered solutions registry entries: " + JSON.stringify(filteredEntries, null, 4));
        event.fire(filteredEntries);
    };

    gpii.flowManager.getDeviceContext = function (deviceReporterDataSource, event, errorEvent) {
        fluid.log("getDeviceContext for dataSource with options ", deviceReporterDataSource.options);
        var promise = deviceReporterDataSource.get();
        promise.then(function (deviceData) {
            fluid.log("getDeviceContext got deviceData ", deviceData);
    ...
    gpii.flowManager.transformLifecycle = function (transformer, contextPayload) {
        ...
        fluid.log("transformer got lifecycleInstructions " + JSON.stringify(lifecycleInstructions, null, 2) + " from contextPayload " + JSON.stringify(contextPayload, null, 2));
        var transformedPayload = fluid.extend({
    ...
    fluid.defaults("gpii.flowManager.matchMakingRequest", {
        invokers: {
            ...
            getSolutions: {
                ...
                args: [ "{flowManager}.options.fullSolutionsRegistry", "{arguments}.0", "{that}.events.onSolutions"]
            },
            getDeviceContext: {
    ...
    gpii.flowManager.privacy.privacyToSettingsTransformRules = fluid.require("%universal/testData/ontologies/privacy-flat.json");
