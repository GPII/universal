...
gpii.flowManager.addLifecycleInstructions = function (flowManager, fullPayload, event) {
    ...
    var finalPayload = fluid.extend(true, {}, fullPayload, {
        activeConfiguration: {
            lifecycleInstructions: lifecycleInstructions
        }
    });
    event.fire(finalPayload);
};
...
fluid.defaults("gpii.flowManager.userLogonHandling.stateChangeHandler", {
    ...
    invokers: {
        ...
        addLifecycleInstructions: {
            funcName: "gpii.flowManager.addLifecycleInstructions",
            args: ["{flowManager}", "{arguments}.0", "{arguments}.1"]
                                    // full payload, success event
        },
        startLifecycle: {
...
fluid.defaults("gpii.flowManager.userLogonHandling.matchMakingStateChangeHandler", {
    ...
    events: {
        onContextAdded: null,
        onLifecycleInstructions: null
    },
    listeners: {
        "onUserToken.getDeviceContext": "{that}.getDeviceContext",
        "onMatchDone.runContextManager": {
            listener: "{flowManager}.contextManager.addActiveContext",
            args: ["{arguments}.0", "{that}.events.onContextAdded"]
        },
        "onContextAdded.addLifecycleInstructions": {
            listener: "{that}.addLifecycleInstructions",
            args: ["{arguments}.0", "{that}.events.onLifecycleInstructions"]
        },
        "onLifecycleInstructions.startLifecycle": "{that}.startLifecycle"
    }
});
