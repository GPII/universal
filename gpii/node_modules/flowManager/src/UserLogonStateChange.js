...
(function () {
    ...
    gpii.request.flowManager.userLogonStateChange.logoutUser = function (that, lifecycleManager, event, userToken) {
        var activeTokens = lifecycleManager.getUserTokens();
        ...
        lifecycleManager.stop({ userToken: userToken }, function onSuccess(response) {
            ...
            if (remainingToken) {
                ...
                // fluid.log(that.responseMsg || message);
                // that.responseMsg = null;
                that.responseMsg = that.responseMsg || message;
                event.fire(userToken);
            }
        });
    };

    gpii.request.flowManager.userLogonStateChange.logoutComplete = function (that, requestEvent, userToken) {
        requestEvent.fire(that.responseMsg);
        that.responseMsg = null;
    };
    ...
    gpii.request.flowManager.userLogonStateChange.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
        ...
        lifecycleManager.start(lifecyclePayload, function () {
            that.responseMsg = that.responseMsg || "User with token " + lifecyclePayload.userToken + " was successfully logged in.";
            // console.log(message);
            // that.responseMsg = null;
    ...
    gpii.request.flowManager.userLogonStateChange.loginComplete = function (that, requestEvent, userToken) {
        requestEvent.fire(that.responseMsg);
        that.responseMsg = null;
    };
    ...
    fluid.defaults("gpii.request.flowManager.userLogonStateChange.stateChangeHandler", {
        ...
        invokers: {
            ...
            logoutUser: {
                ...
                args: ["{that}", "{flowManager}.lifecycleManager", "{that}.events.onLogoutComplete", "{arguments}.0"],
                dynamic: true
            },
            loginUser: {
            ...
            loginComplete: {
                ...
                args: ["{that}", "{requestProxy}.events.onSuccess", "{arguments}.0"] // userToken
            },
            logoutComplete: {
