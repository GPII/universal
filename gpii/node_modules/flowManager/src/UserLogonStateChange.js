...
gpii.flowManager.userLogonHandling.errorResponse = function (request, userErrors, message, statusCode) {
 ...
 * @param {Boolean} isProximity - Indicate if this is /proximityTriggered handler is requesting a logout of a previous
 * key before logging in the new key. In this case, noUser should not be logged in at logging out the previous key.
 * @return {Promise} promise for the logout action.
 */
gpii.flowManager.userLogonHandling.logoutUserWithoutResponse = function (that, lifecycleManager, gpiiKey, isProximity) {
...
fluid.defaults("gpii.flowManager.userLogonHandling.stateChangeHandler", {
    ...
    invokers: {
        ...
        errorResponse: {
            ...
            args: ["{request}", "{flowManager}.userErrors", "{arguments}.0", "{arguments}.1"]
                                // message, statusCode
        },
        isBounce: {
...
fluid.defaults("gpii.flowManager.proximityTrigger.handler", {
    ...
    listeners: {
        "onError.loginNoUser": {
            listener: "gpii.flowManager.proximityTrigger.handler.loginNoUser",
            args: ["{lifecycleManager}", "{arguments}.0"],
            priority: "last"
        }
    }
});

gpii.flowManager.proximityTrigger.handler.loginNoUser = function (lifecycleManager, error) {
...
gpii.flowManager.userLogonHandling.handleProximityTriggerRequest = function (that, lifecycleManager, gpiiKey) {
    ...
    if (activeGpiiKeys.length === 1) {
        ...
        if (previousUser !== gpiiKey) {
            var promise = that.logoutUserWithoutResponse(previousUser, true);
            promise.then(function () {
                that.loginUser(gpiiKey);
            }, function () {
