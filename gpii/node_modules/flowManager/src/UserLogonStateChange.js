...
gpii.flowManager.userLogonHandling.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
    ...
    lifecycleManager.start(lifecyclePayload,
        gpii.flowManager.logAndNotify("Lifecycle manager startLifecycle returned: ", event, function () {
            that.events.userLoginComplete.fire(lifecyclePayload.userToken);
            return "User with token " + lifecyclePayload.userToken + " was successfully logged in.";
        })
    );
};

gpii.flowManager.userLogonHandling.logoutUser = function (that, request, lifecycleManager, userToken) {
    that.events.userLogoutInitiated.fire(userToken);
    lifecycleManager.stop({
        userToken: userToken
    }, function onSuccess(response) {
        fluid.log("userLoginStateChange.logoutUser: Lifecycle manager returned: ", response);
        that.events.userLogoutComplete.fire(userToken);
        request.events.onSuccess.fire("User with token " + userToken + " was successfully logged out.");
    });
};

gpii.flowManager.userLogonHandling.loginUser = function (that, userToken) {
    that.events.userLoginInitiated.fire(userToken);
...
fluid.defaults("gpii.flowManager.userLogonHandling.stateChangeHandler", {
    ...
    invokers: {
        ...
        logoutUser: {
            ...
            args: ["{that}", "{request}", "{flowManager}.lifecycleManager", "{arguments}.0" ]
        },
        loginUser: {
    ...
    events: {
        userLoginInitiated: null,
        userLogoutInitiated: null,
        userLoginComplete: null,
        userLogoutComplete: null
    },
    listeners: {
        "userLogoutInitiated.propagate": "{flowManager}.events.userLogoutInitiated.fire",
        "userLogoutComplete.propagate": "{flowManager}.events.userLogoutComplete.fire",
        "userLoginInitiated.propagate": "{flowManager}.events.userLoginInitiated.fire",
        "userLoginComplete.propagate": "{flowManager}.events.userLoginComplete.fire"
    }
});
...
fluid.defaults("gpii.flowManager.userLogonHandling.handler", {
...
/* Returns true if the proxityTrigger should be ignored (i.e. is considered a bounce)
 *
...
gpii.flowManager.userLogonHandling.isBounce = function (lifecycleManager, debounceTimeMs, userToken) {
    ...
    if (lifecycleManager.model.logonChangeActiveFor === userToken) {
        return true;
    } else if (lifecycleManager.model.lastLogonChange.userToken === userToken &&
               Date.now() - lifecycleManager.model.lastLogonChange.timestamp < debounceTimeMs) {
        return true;
    } else {
        return false;
    }
...
gpii.flowManager.userLogonHandling.handleProximityTriggerRequest = function (that, lifecycleManager, userToken) {
    ...
    // if a different user is logged in, log that user out and log the new user in
