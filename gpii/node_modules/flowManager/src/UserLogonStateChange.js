 ...
 *
 * @param lifecycleManager {Object}: the lifecycleManager module
 * @param debounceTimeMs {int}: the time in milliseconds for which to debounce after last proximityTrigger was fired
...
gpii.flowManager.userLogonHandling.isBounce = function (lifecycleManager, debounceTimeMs, userToken) {
    ...
    if (lifecycleManager.model.logonChange.inProgress && lifecycleManager.model.logonChange.userToken === userToken) {
        return true;
    } else if (lifecycleManager.model.logonChange.userToken === userToken &&
               Date.now() - lifecycleManager.model.logonChange.timeStamp < debounceTimeMs) {
        return true;
    } else {
        return false;
    }
