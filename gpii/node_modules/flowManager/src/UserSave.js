...
console.log("=== in gpii.flowManager.userSave.handleRequest");
    if (writeMethod === "POST") {
        promiseTogo = preferencesServer.createPreferences(preferences, null, gpiiKey);
    } else {
        promiseTogo = preferencesServer.updatePreferences(gpiiKey, preferences);
    }
    promiseTogo.then(request.events.onSuccess.fire, request.events.onError.fire);
};
