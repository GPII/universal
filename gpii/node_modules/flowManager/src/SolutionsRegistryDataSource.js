...
// Solutions registry datasource used by the cloud based flow manager.  It loads
// solutions only from the local file system
fluid.defaults("gpii.flowManager.solutionsRegistry.dataSource", {
    ...
    listeners: {
        ...
        "loadSolutions.loadFromLocalDisk": {
            listener: "gpii.flowManager.solutionsRegistry.dataSource.loadFromLocalDisk",
            args: ["{that}"],
            priority: "first"
        },
...
fluid.defaults("gpii.flowManager.solutionsRegistry.dataSource.local", {
    ...
    listeners: {
        // The loadSolutions sequence inherits the relevant listeners from the
        // "gpii.flowManager.solutionsRegistry.dataSource" base grade.  The
        // resulting sequence is: loadFromLocalDisk (base), getRevision,
        // loadFromRepository, solutionsLoaded (base).
        "onCreate.loadSolutions": {
            listener: "fluid.promise.fireTransformEvent",
            args: ["{that}.events.loadSolutions"]
        },
...
/**
 * Handler for get requests of solutions registry. It will return either a full
 * solution registry, or if an 'os' is provided in the requestOptions, only the
 * entries for that os will be returned.
 *
 * @param {Object} that - The gpii.flowManager.solutionsRegistry.dataSource.
...
gpii.flowManager.solutionsRegistry.dataSource.local.loadFromRepository = function (that, revision, platformId) {
    ...
    togo.then(function () {
        // Either the repositorySolutionsLoader has retrieved the remote
        // solutions registry or it is null.
 ...
 *          rejected on error.
 */
gpii.flowManager.solutionsRegistry.dataSource.local.handle = function (that) {
