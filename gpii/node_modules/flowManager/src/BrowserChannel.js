(function () {

    "use strict";

    var fluid = require("infusion");
    var gpii = fluid.registerNamespace("gpii");

    fluid.registerNamespace("gpii.request.flowManager");

    fluid.defaults("gpii.flowManager.browserChannel", {
        gradeNames: ["fluid.standardRelayComponent", "autoInit"]
    });

    fluid.defaults("kettle.requests.request.handler.browserChannel", {
        gradeNames: ["gpii.request.flowManager.sessionAware", "autoInit"],
        invokers: {
            handle: {
                funcName: "gpii.request.flowManager.browserChannelHandle",
                args: ["{request}", "{requestProxy}.events",
                       "{flowManager}.solutionsRegistryDataSource",
                       "{deviceReporter}.platformReporter"],
                dynamic: true
            }
        }
    });

    gpii.request.flowManager.browserChannelHandle = function (fluidRequest, events, solutionsRegistryDataSource, platformReporter) {
        solutionsRegistryDataSource.get({os: platformReporter.reportPlatform().id}, function onSuccess (entries) {
            var solutionId = fluidRequest.data;

            if (!(solutionId in entries)) {
                var msg = "Rejecting a connection request from '" + solutionId +
                          "'. The solution id was not found in the solutions registry";

                fluidRequest.socket.emit("error", msg);
                fluidRequest.socket.disconnect("unauthorized");

                gpii.request.flowManager.logAndNotify("Lifecycle manager returned: ", events.onError)(msg);
            } else {
                var initialSettings = gpii.settingsHandlers.WebSockets.getSettingsForId(solutionId);
                fluidRequest.socket.emit("connectionSucceeded", initialSettings);
                gpii.settingsHandlers.WebSockets.addClient(solutionId, fluidRequest.socket);

                fluidRequest.socket.on("disconnect", function () {
                    gpii.settingsHandlers.WebSockets.removeClient(fluidRequest.socket);
                });

                gpii.request.flowManager.logAndNotify("Lifecycle manager returned: ", events.onSuccess)("A request to browserChannel has been successfully processed");
            }
        });
    };

})();
