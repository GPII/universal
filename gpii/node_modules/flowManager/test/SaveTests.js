...
gpii.tests.flowManager.savePrefs.configRecord = {
    configName: "gpii.config.development.local.json",
    configPath: "%universal/gpii/configs"
};

gpii.tests.flowManager.savePrefs.testGpiiKey1 = "testGpiiKey1";
...
gpii.tests.flowManager.savePrefs.testCreateNewResponse = function (responseText, expectedPrefs) {
    var response = JSON.parse(responseText);
    jqUnit.assertNotUndefined("A GPII key is created", response.gpiiKey);
    jqUnit.assertDeepEq("The saved preferences are correct", expectedPrefs,
        response.preferences.flat);
};

gpii.tests.flowManager.savePrefs.testSaveResponse = function (responseText) {
    console.log("=== response", responseText);
    jqUnit.assertDeepEq("Response is correct", gpii.tests.flowManager.savePrefs.testGpiiKey1Set, data.preferences);
    jqUnit.assertDeepEq("Saved preferences are correct", gpii.tests.flowManager.savePrefs.testGpiiKey1Set,
        savedPrefs.flat);
};
...
gpii.tests.flowManager.savePrefs.testDefs = [{
    name: "Flow Manager create new preferences",
    // expect: 4,
    ...
    sequence: [{
        ...
        listener: "gpii.tests.flowManager.savePrefs.testCreateNewResponse",
    ...
    // config: gpii.tests.flowManager.savePrefs.configRecord,
    // components: {
    //     saveRequest: { // TODO: this test is faulty - a PUT should be supplied for an existing GPII key
    //         type: "kettle.test.request.http",
    //         options: {
    ...
    //             method: "PUT",
    //             termMap: {
    //                 gpiiKey: gpii.tests.flowManager.savePrefs.testGpiiKey1
    //             }
    //         }
    //     },
    //     saveRequestNoGpiiKey: {
    //         type: "kettle.test.request.http",
    //         options: {
    //             port: "{configuration}.options.mainServerPort",
    //             path: "/user/preferences",
    ...
    //     // args: gpii.tests.flowManager.savePrefs.testGpiiKey1Set
    // }, {
    //     event: "{saveRequest}.events.onComplete",
    //     listener: "gpii.tests.flowManager.savePrefs.testSaveResponse"
    // }, {
    //     func: "{saveRequestNoGpiiKey}.send",
    //     args: gpii.tests.flowManager.savePrefs.testGpiiKey1Set
    // }, {
    //     event: "{saveRequestNoGpiiKey}.events.onComplete",
    //     listener: "gpii.tests.flowManager.savePrefs.testSaveResponse"
    // }]
// }, {
...
module.exports = kettle.test.bootstrapServer(fluid.transform(gpii.tests.flowManager.savePrefs.testDefs, function (testDef) {
    testDef.gradeNames = ["gpii.test.pouch.pouchTestCaseHolder"];
