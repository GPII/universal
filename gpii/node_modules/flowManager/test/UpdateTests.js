...
update.token = "testUser1";

update.tokenDefaultPrefs = require(path.resolve(__dirname,
    "../../../../testData/preferences/testUser1.json"));

update.tokenPrefs = $.extend(true, {}, update.tokenDefaultPrefs, {
    "http://registry.gpii.net/common/setting1": [{ value: 14 }]
});

update.tokenPrefsUpdate = {
    success: true
};

update.tokenPrefsUpdated = $.extend(true, {}, update.tokenDefaultPrefs, {
    "http://registry.gpii.net/common/setting1": [{ value: 16 }]
});

update.testLoginResponse = function (data) {
    jqUnit.assertEquals("Response is correct", "User with token " +
        update.token + " was successfully logged in.", data);
};

update.testLogoutResponse = function (data) {
    jqUnit.assertEquals("Response is correct", "User with token " +
        update.token + " was successfully logged out.", data);
};

update.testInvalidUpdateRequest = function () {
...
update.testFirstUpdateRequest = function (data) {
    testUpdateRequest(update.tokenPrefsUpdate, data);
};
...
update.testUpdateRequestFailure = function (data) {
    ...
    jqUnit.assertTrue("Message is correct",
        data.message.indexOf("none was active") > -1);
