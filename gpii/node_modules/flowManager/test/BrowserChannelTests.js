...
gpii.tests.flowManager.browserChannel.testLoginResponseChromeAndFirefox = function (data) {
    jqUnit.assertEquals("Response is correct", "User with token " +
        gpii.tests.flowManager.browserChannel.chromeAndFirefoxUserToken + " was successfully logged in.", data);
};

gpii.tests.flowManager.browserChannel.testLogoutResponseChromeAndFirefox = function (data) {
    jqUnit.assertEquals("Response is correct", "User with token " +
        gpii.tests.flowManager.browserChannel.chromeAndFirefoxUserToken + " was successfully logged out.", data);
};
...
gpii.tests.flowManager.browserChannel.checkConnectionRequest = function (data, client) {
    ...
    // Let's connect to 'connectionSuceeded' in order to be able to check whether we
    // receive the initial settings at connection time.
...
gpii.tests.flowManager.browserChannel.checkSettingsForChrome = function () {
    jqUnit.assertDeepEq("The settings for 'org.chrome.cloud4chrome' are the expected",
        gpii.tests.flowManager.browserChannel.payloads["org.chrome.cloud4chrome"].settings,
        gpii.settingsHandlers.webSockets.getSettingsForId("org.chrome.cloud4chrome")
    );
};
...
gpii.tests.flowManager.browserChannel.checkSettingsForFirefox = function () {
    jqUnit.assertEquals("The settings for 'org.mozilla.cloud4firefox' are the expected",
        JSON.stringify(gpii.tests.flowManager.browserChannel.payloads["org.mozilla.cloud4firefox"].settings),
        JSON.stringify(gpii.settingsHandlers.webSockets.getSettingsForId("org.mozilla.cloud4firefox"))
    );
};

gpii.tests.flowManager.browserChannel.checkErrorResponse = function (data) {
    ...
    jqUnit.assertEquals("Received error code 409", 409, data.statusCode);
};

gpii.tests.flowManager.browserChannel.checkNoClients = function () {
    jqUnit.assertDeepEq("No clients are registered yet", {}, gpii.settingsHandlers.webSockets.clients);
};
...
gpii.tests.flowManager.browserChannel.connectionSucceeded = function (data, solutionId) {
    var expectedPayload = {
        options: gpii.tests.flowManager.browserChannel.payloads[solutionId].options
    };

    if (gpii.settingsHandlers.webSockets.getSettingsForId(solutionId)) {
        expectedPayload.settings = gpii.settingsHandlers.webSockets.getSettingsForId(solutionId);
    }

    jqUnit.assertDeepEq("Right after connecting, the client receives a payload with the options block " +
                        "and the initial settings, and these settings should be the expected",
        expectedPayload,
        data
    );
};
...
var testDefs = [{
    ...
    components: {
        ...
        loginChromeAndFirefox: {
            ...
            options: {
                ...
                termMap: {
                    userToken: gpii.tests.flowManager.browserChannel.chromeAndFirefoxUserToken
                }
            }
        },
    ...
    sequence: [{
        ...
        event: "{loginChromeAndFirefox}.events.onComplete",
        listener: "gpii.tests.flowManager.browserChannel.testLoginResponseChromeAndFirefox"
    }, {
        func: "gpii.tests.flowManager.browserChannel.checkSettingsForChrome"
    }, {
        ...
        listener: "gpii.tests.flowManager.browserChannel.testLogoutResponseChromeAndFirefox"
    }, {
