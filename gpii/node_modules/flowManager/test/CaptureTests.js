...
fluid.defaults("gpii.tests.flowManager.capture.platformReporter", {
...
gpii.tests.flowManager.capture.adjustSolutions = function (solutionsRegistryDataSource, revision, platformReporter) {
    var platformId = fluid.invokeGlobalFunction(platformReporter).id;
    ...
    // The full block is a read only data structure
    var adjustedSolutions = fluid.copy(solutionsRegistryDataSource.fullSolutionsRegistry[platformId]);
    adjustedSolutions.fakemag2.settingsHandlers.configuration.options.filename = testDataFile2;
    adjustedSolutions.fakemag1.settingsHandlers.configuration.options.filename = testDataFile;
    adjustedSolutions.fakemag1.settingsHandlers.configuration1.options.filename = testDataFile;

    var adjustedRegistry = {};
    adjustedRegistry[platformId] = adjustedSolutions;
    solutionsRegistryDataSource.repositorySolutionsRegistry = fluid.freezeRecursive(adjustedRegistry);
    return fluid.promise().resolve(solutionsRegistryDataSource.repositorySolutionsRegistry);
};
    ...
    modules: [
        {
            ...
            tests: [{
                ...
                expect: 4,
                sequence: [{
                    event: "{config}.server.flowManager.solutionsRegistryDataSource.events.solutionsRegistryReady",
                    listener: "jqUnit.assert",
                    args: ["Solutions registry ready"]
                }, {
                    funcName: "{config}.server.flowManager.solutionsRegistryDataSource.get",
                    args: [{os: "darwin"}]
                }, {
                    event: "{config}.server.flowManager.solutionsRegistryDataSource.events.onRead",
                    listener: "jqUnit.assertValue",
                    args: ["Solutions registry read solutions", "{arguments}.0"]
                }, {
