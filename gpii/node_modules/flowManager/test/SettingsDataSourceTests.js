...
gpii.tests.settingsDataSource.setupNock = function (config) {
    ...
    fluid.each(config, function (eachConfig, index) {
        // mock POST requests to "/access_token"
        if (index === "accessToken") {
            cloudMock.post(eachConfig.url, "\"" + querystring.stringify(eachConfig.data) + "\"")
                .reply(eachConfig.status, eachConfig.response);
        } else if (index === "untrustedSettings") {
            // mock GET requests to "/untrusted-settings"
            cloudMock.get(eachConfig.url)
...
gpii.tests.settingsDataSource.teardownNock = function () {
    nock.isDone();
    nock.cleanAll();
    nock.restore();
};
    ...
    modules: [{
        ...
        tests: [{
            ...
            sequence: [{
                ...
                args: ["No saved access token", "{settingsDataSource}.accessToken"]
            }, {
                ...
                args: ["The access token is received and saved", "{settingsDataSource}.gpii-token-test.accessToken"]
            }, {
                func: "jqUnit.assertValue",
                args: ["The expiresIn is received and saved", "{settingsDataSource}.gpii-token-test.timestampExpires"]
            }]
        }]
    }]
});
...
fluid.defaults("gpii.tests.settingsDataSource.validSavedAccessToken", {
    ...
    members: {
        "gpii-token-test": {
            ...
            timestampExpires: new Date(new Date().getTime() + 10 * 1000).toISOString()  // expires in 10 mins
        }
    }
});
    ...
    modules: [{
        ...
        tests: [{
            ...
            sequence: [{
                ...
                args: ["The saved access token is same as the initial saved value", "{that}.initialSavedAccessToken", "{settingsDataSource}.gpii-token-test.accessToken"]
            }, {
                func: "jqUnit.assertEquals",
                args: ["The saved timestampExpires is same as the initial saved value", "{that}.initialSavedTimestampExpires", "{settingsDataSource}.gpii-token-test.timestampExpires"]
            }]
        }]
    }]
});
...
fluid.defaults("gpii.tests.settingsDataSource.expiredSavedAccessToken", {
    ...
    members: {
        "gpii-token-test": {
            accessToken: "a-valid-initial-access-token",
            timestampExpires: new Date(new Date().getTime() - 10 * 1000).toISOString()  // already expired for 10 mins
        }
    }
});
    ...
    modules: [{
        ...
        tests: [{
            ...
            sequence: [{
                ...
                args: ["The saved access token is different from the initial saved value", "{that}.initialSavedAccessToken", "{settingsDataSource}.gpii-token-test.accessToken"]
            }, {
                func: "jqUnit.assertNotEquals",
                args: ["The saved timestampExpires is different from the initial saved value", "{that}.initialSavedTimestampExpires", "{settingsDataSource}.gpii-token-test.timestampExpires"]
            }, {
                // Make sure the nock is torn down only once after all nock required tests have completed
                func: "gpii.tests.settingsDataSource.teardownNock"
            }]
        }]
    }]
});
...
gpii.tests.settingsDataSource.assertInitialAccessToken = function (that, settingsDataSource, isExpired) {
    var initialAccessToken = fluid.get(settingsDataSource, [gpii.tests.settingsDataSource.userToken, "accessToken"]);
    var initialTimestampExpires = fluid.get(settingsDataSource, [gpii.tests.settingsDataSource.userToken, "timestampExpires"]);

    jqUnit.assertValue("A saved access token is in place", initialAccessToken);
    jqUnit[isExpired ? "assertTrue" : "assertFalse"]("A saved access token is not expired", gpii.oauth2.getExpiresIn(initialTimestampExpires) === 0);
