...
require("../src/GpiiRevisionRequester.js");
...
gpii.tests.revisionRequester.setUpNock = function (config) {
    var cloudMock = nock(gpii.tests.revisionRequester.hostname);
    cloudMock.log(console.log);

    // mock GET request
    cloudMock.get(gpii.tests.revisionRequester.path)
        .reply(config.status, config.response);
};

// Revision requester customized for testing
fluid.defaults("gpii.tests.revisionRequester", {
    gradeNames: ["gpii.revisionRequester"],
    cloudUrl: gpii.tests.revisionRequester.hostname,
    url: gpii.tests.revisionRequester.hostname + gpii.tests.revisionRequester.path
});
