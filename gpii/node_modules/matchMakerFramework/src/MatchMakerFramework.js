...
(function () {
        ...
        invokers: {
            preProcess: {
                ...
                args: [ "{that}", "{arguments}.0", "{arguments}.1", "{arguments}.2", "{arguments}.3", "{arguments}.4"]
                                     // token, prefs, device, solutionsRegistry, event
            }
        },
        events: {
            onSolutions: null
    ...
    gpii.matchMakerFramework.matchMakerDispatcher = function (that, matchMakerPayload, event) {
        var selectedMatchMaker = gpii.matchMakerFramework.selectMatchMaker(matchMakerPayload);
     ...
     */
    gpii.matchMakerFramework.selectMatchMaker = function (matchMakerPayload) {
	...
	gpii.matchMakerFramework.GetPreferredMatchMaker = function (matchMakerPayload) {
		// Constants
		var USER_DEFINED_MATCHMAKER_PREF0 = "http://registry.gpii.org/common/matchMakerType";
		var USER_DEFINED_MATCHMAKER_PREF1 = "http://registry.gpii.net/common/matchMakerType";
		// Rejection tests
		...
		fluid.each(activeContexts, function (activeContext) {
			if ((activeContext in contexts) && ("preferences" in contexts[activeContext])) {
				...
				if ((USER_DEFINED_MATCHMAKER_PREF0 in prefs) && (curPriority > foundPriority)) {
					foundPriority = curPriority;
					foundMatchMaker = prefs[USER_DEFINED_MATCHMAKER_PREF0];
				} else if ((USER_DEFINED_MATCHMAKER_PREF1 in prefs) && (curPriority > foundPriority)) {
					foundPriority = curPriority;
					foundMatchMaker = prefs[USER_DEFINED_MATCHMAKER_PREF1];
				}
			}
		});
	...
	gpii.matchMakerFramework.GetCapableMatchMaker = function (matchMakerPayload) {
		// Constants
		var EXCLUDED_MATCHMAKER_PREF0 = "http://registry.gpii.org/common/excludedMatchMakers";
		var EXCLUDED_MATCHMAKER_PREF1 = "http://registry.gpii.net/common/excludedMatchMakers";
		// Contexts
		var contexts = matchMakerPayload.preferences.contexts;
		...
		fluid.each(activeContexts, function (activeContext) {
			if ((activeContext in contexts) && ("preferences" in contexts[activeContext])) {
				var prefs = contexts[activeContext].preferences;
				if (EXCLUDED_MATCHMAKER_PREF0 in prefs) {
					excludedMatchMakers = prefs[EXCLUDED_MATCHMAKER_PREF0];
				} else if (EXCLUDED_MATCHMAKER_PREF1 in prefs) {
					excludedMatchMakers = prefs[EXCLUDED_MATCHMAKER_PREF1];
				}
			}
		});
		// Try again block
		if (("tryAgain" in matchMakerPayload) && ("active" in matchMakerPayload.tryAgain) && (matchMakerPayload.tryAgain.active)) {
			if (excludedMatchMakers in matchMakerPayload.tryAgain) {
				excludedMatchMakers.concat(matchMakerPayload.tryAgain.matchMakerPayload.tryAgain);
			}
		}
		...
		fluid.each(contexts, function (context) {
			if (("conditions" in context) && (context.conditions.length > 0)) {
		...
		fluid.each(gpii.matchMakerFramework.matchmakerCapabilities, function (capability) {
			if (excludedMatchMakers.indexOf(capability.id) < 0) {
				// Do we support conditions?
				var supportsConditions = true;
				if (("conditions" in capability) && ("supported" in capability.conditions)) {
					supportsConditions = capability.conditions.supported;
				}
				// Which preferences do we support?
				var supportedPreferences = [];
				if (("preferences" in capability) && ("supported" in capability.preferences)) {
				...
				if (supportsConditions || !payloadHasConditions) {
					...
					if ("metrics" in capability) {
						if ("baseValue" in capability.metrics) baseValue = capability.metrics.baseValue;
						if ("forEachPreference" in capability.metrics) forEachPreference = capability.metrics.forEachPreference;
						if ("forEachUnsupportedPreference" in capability.metrics) forEachUnsupportedPreference = capability.metrics.forEachUnsupportedPreference;
					}
					confidence[capability.id] = baseValue
     ...
     */
    gpii.matchMakerFramework.preProcess = function (that, token, preferences, deviceContext, solutionsRegistry, event) {
        var togo = {
            token: token,
