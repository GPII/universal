...
(function () {
    ...
    gpii.matchMakerFramework.preProcess = function (that, lifecycleManager, initialPayload) {
        ...
        // add the web solutions to device context
        var webSolutions = fluid.transform(Object.keys(initialPayload.webSolutionsRegistryEntries), function (id) {
            return { id: id };
        });
        initialPayloadCopy.deviceContext.webSolutions = webSolutions;
    ...
    gpii.matchMakerFramework.matchMakerDispatcher = function (that, matchMakerInput) {
        ...
        promise.then(function (matchMakerOutput) {
            ...
            matchMakerResponse.selectedMatchMaker = selectedMatchMaker;
    ...
    gpii.matchMakerFramework.selectMatchMaker = function (that, matchMakerPayload) {
        ...
        if (Array.isArray(matchMakerPayload.preferences)) {
            // multi-NP set request which is only supported by the RB MM
            return "RuleBased";
        }
        var result = gpii.matchMakerFramework.GetPreferredMatchMaker(matchMakerPayload);
        return (result) ? result : gpii.matchMakerFramework.GetCapableMatchMaker(that, matchMakerPayload);
    };
     ...
     */
    gpii.matchMakerFramework.GetCapableMatchMaker = function (that, matchMakerPayload) {
        // Contexts
        var contexts = matchMakerPayload.preferences.contexts;
        ...
        if (matchMakerPayload.tryAgain && matchMakerPayload.tryAgain.active) {
            if (matchMakerPayload.tryAgain.excludedMatchMakers) {
                excludedMatchMakers = excludedMatchMakers.concat(matchMakerPayload.tryAgain.excludedMatchMakers);
            }
        }
        ...
        // Filter amtchmaker capabilities list based on available matchmakers:
        var filteredCapabilities = fluid.copy(gpii.matchMakerFramework.matchmakerCapabilities);
        fluid.remove_if(filteredCapabilities, function (val) {
            return !(val.matchMakerId in that.options.matchMakers);
        });
        // Compute capabilities
        var foundMatchmaker = false;
        var confidence = {};
        fluid.each(filteredCapabilities, function (capability) {
    ...
    fluid.defaults("kettle.requests.request.handler.tryDifferent", {
        gradeNames: ["fluid.littleComponent", "gpii.request.flowManager.matchMakingRequest", "autoInit"],
        invokers: {
            handle: {
                funcName: "gpii.matchMakerFramework.handleTryDifferent",
                args: ["{that}", "{flowManager}.lifecycleManager", "{flowManager}.matchMakerFramework", "{flowManager}", "{request}.req.body", "{request}.events"],
                dynamic: true
            },
            tryDifferentComplete: {
                funcName: "gpii.matchMakerFramework.tryDifferentComplete",
                args: ["{request}", "{arguments}.0"]
            },
            startLifecycle: {
                funcName: "gpii.matchMakerFramework.startLifecycle",
                args: [ "{that}", "{flowManager}.lifecycleManager", "{arguments}.0", "{that}.events.onLoginComplete" ]
                                // final payload from matchmaking process
            },
        },
        listeners: {
            onMatchDone: "{that}.startLifecycle",
            onLoginComplete: "{that}.tryDifferentComplete"
        }
    });

    gpii.matchMakerFramework.startLifecycle = function (that, lifecycleManager, lifecyclePayload, event) {
    ...
    gpii.matchMakerFramework.tryDifferentComplete = function (request, arg) {
        request.events.onSuccess.fire("New configuration applied to the system");
    }
    gpii.matchMakerFramework.handleTryDifferent = function (that, lifecycleManager, matchMakerFramework, flowManager, body, events) {        console.log("handleTryDifferent");
        // if no user is logged in, ignore
        if (lifecycleManager.getUserTokens().length === 0) {
            events.onSuccess.fire("Try different was called, but no user logged in.. Ignoring");
            return;
        }

        // if user is logged in:
        var session = fluid.copy(lifecycleManager.session);

        // fix up tryAgain directive:
        if (!session.tryAgain) {
            session.tryAgain = {
                active: true,
                excludedMatchMakers: [ session.selectedMatchMaker ]
            };
        } else {
            session.tryAgain.excludedMatchMakers.push(session.selectedMatchMaker);
        }

        // log user out
        lifecycleManager.stop({ userToken: session.userToken }, function (response) {
            fluid.log("Lifecycle manager returned: ", response);

            // recreate MM event input incl. tryAgain
            var args = {
                userToken: session.userToken,
                preferences: session.preferences,
                deviceContext: session.deviceContext,
                solutionsRegistryEntries: session.solutionsRegistryEntries,
                webSolutionsRegistryEntries: session.webSolutionsRegistryEntries,
                tryAgain: session.tryAgain
            }

            // Trigger matchmaking sequence
            that.events.onReadyToMatch.fire(args);
        });
    };
})();
