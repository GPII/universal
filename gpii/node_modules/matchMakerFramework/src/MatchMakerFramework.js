...
(function () {
    ...
    fluid.defaults("gpii.matchMakerFramework", {
        ...
        invokers: {
            preProcess: {
                ...
                args: ["{that}", "{flowManager}", "{arguments}.0"]
            }, // initialPayload
     ...
     */
    gpii.matchMakerFramework.preProcess = function (that, flowManager, initialPayload) {
        var matchMakerInput = fluid.extend({
            activeContexts: [ // TODO calculate properly
                "gpii-default"
            ],
            environmentReporter: {}, // TODO,
            inferredCommonTerms: gpii.matchMakerFramework.inferCommonTerms(initialPayload.preferences, flowManager.options.fullSolutionsRegistry)
        }, initialPayload);
     ...
     *      soluion
     */
    gpii.matchMakerFramework.inferCommonTerms = function (preferences, fullSolutionsRegistry) {
        ...
        fluid.each(preferences.contexts, function (context, contextId) {
            ...
            fluid.each(context.preferences, function (prefsBlock, prefsId) {
                if (prefsId.indexOf("http://registry.gpii.net/applications/") === 0) {
                    var id = prefsId.substring("http://registry.gpii.net/applications/".length);
                    if (id in flattenedSolutionsRegistry) {
                        var entry = flattenedSolutionsRegistry[id];
                        fluid.each(entry.settingsHandlers, function (handler) {
                            if (handler.inverseCapabilitiesTransformations) {
                                var transformed = fluid.model.transformWithRules(prefsBlock, handler.inverseCapabilitiesTransformations);
                                var out = {};
                                fluid.set(out, [contextId, id], transformed);
