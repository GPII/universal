...
(function () {
    ...
    */
    gpii.matchMakerFramework.utils.computeCapabilitiesFromSolution = function (solutionId, solution, transformSpec) {
        var settingsHandlerBlocks = [];
        // TODO: This should only be done for the regular settings transformations - not when dealing with aptology?!
        // Add entry for the application (similar to the old application.id entries, to ensure
        // we match application specific settings)
        var solutionEl = "http://registry.gpii.net/applications/" + solutionId.replace(/\./g, "\\.");
        var solutionIdModel = [];
        solutionIdModel[solutionEl] = true;
        var solutionIdBlock = fluid.model.transformWithRules(solutionIdModel, transformSpec);
        var solutionIdArr = gpii.matchMakerFramework.utils.computeLeaves(solutionIdBlock);
        fluid.each(solution.settingsHandlers, function (handlerBlock) {
            var capabilities = gpii.matchMakerFramework.utils.computeCapabilitiesFromSettingsHandler(handlerBlock, transformSpec);
            settingsHandlerBlocks = settingsHandlerBlocks.concat(capabilities);
        });
        return [].concat(settingsHandlerBlocks, solutionIdArr);
    };
    ...
    gpii.matchMakerFramework.utils.expandSolutions = function (solutions, transformSpec) {
        return fluid.transform(solutions, function (solution, solutionId) {
            var paths = gpii.matchMakerFramework.utils.computeCapabilitiesFromSolution(solutionId, solution, transformSpec);
    ...
    gpii.matchMakerFramework.utils.disposeSolutions = function (data, strategy, transformSpec) {
        ...
        fluid.each(data.hierarchicalPrefs.contexts, function (context, contextId) {
            ...
            // put solution on the 'active' list (ie. launch on startup)

            // remove solution type from preferences list
            // remove prefs covered by solution from users NP set
    ...
    // check if arrays contains the same values (order doesnt matter)
    gpii.matchMakerFramework.utils.compareArrays = function (arr1, arr2) {
        for (var val1 in arr1) {
            if ($.inArray(arr1[val1], arr2) === -1) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Tests todo:
     * if (type sola == type solb) { remove priorities from sola & solb }
     */
    gpii.matchMakerFramework.utils.disposeFromPriority = function (solrecs, data) {
        // sort into prioritized array: 1024: [sola, solb], 200 [solc]
        var prioritiesMap = {};
        ...
        for (var i = 0; i < priorities.length; i++) {
            ...
            for (var solutionId in sols) {
                ...
                // if isset (sol.disposition) ignore
                if (solrecs[solutionId].disposition) {
                    return;
                }
                ...
                for (var otherId in sols) {
                    ...
                    if (gpii.matchMakerFramework.utils.compareArrays(solutionType, data.solutionTypes[otherId])) {
                ...
                if (tied.length <= 1) {
                    ...
                    // if there are ties, we "unreject" all of them, removing their disposition complete
                    // so they can be tiebraked by other disposal means (such as via the canopy)
     ...
     * used when needing to merge inferred common terms from the MM input payload with the already existing
     * common terms of the preferences set. This happens for all the context blocks (for which inferred
     ...
     */
    gpii.matchMakerFramework.utils.addInferredCommon = function (preferences, inferred) {
    ...
    /** TODO KASPER: Refactor with gpii.matchMakerFramework.utils.computeCapabilitiesFromSettingsHandler
    * and gpii.matchMakerFramework.utils.computeCapabilitiesFromSolution */

    gpii.matchMakerFramework.utils.computeSolutionTypeFromSettingsHandler = function (settingsHandler, transformSpec) {
        var leaves = [];
        var inputPaths = fluid.model.transform.collectInputPaths(settingsHandler.capabilitiesTransformations || {});
        inputPaths = inputPaths.concat(fluid.makeArray(settingsHandler.capabilities));

        // Input paths are flat array from, so need to be turned into an object to model transform
        var skel = gpii.matchMakerFramework.utils.pathsToSkeleton(inputPaths, true);
        var hskeleton = fluid.model.transformWithRules(skel, transformSpec);

        // return array with the found solution types
        return leaves.concat(gpii.matchMakerFramework.utils.computeLeaves(hskeleton));
    };


    gpii.matchMakerFramework.utils.computeSolutionType = function (solution, transformSpec) {
        var solutionTypes = [];
        fluid.each(solution.settingsHandlers, function (handlerBlock) {
            var newTypes = gpii.matchMakerFramework.utils.computeSolutionTypeFromSettingsHandler(handlerBlock, transformSpec);
            solutionTypes = solutionTypes.concat(newTypes);
        });
        return solutionTypes;
    };


    /** TODO KASPER tests for this stuff and perhaps some refactoring?
    ...
    gpii.matchMakerFramework.utils.addSolutionTypeInformation = function (payload, transformSpec) {
        fluid.each(payload.solutionsRegistryEntries, function (entry, entryId) {
            ...
            var solutionTypes = gpii.matchMakerFramework.utils.computeSolutionType(entry, transformSpec);
