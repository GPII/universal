...
(function () {
     ...
     *    that application. * MIN_USER_PRIORITY: If an explicit priority is set by the user for an application, this is *    the minimum value it should have, to avoid conflicting with system generated priorities.
     */
    ...
    /**
    // TODO KASPER UPDATE DOCS
     * Helper function for computeAndOntologizeCapabilitiesFromSolution. Takes an array of paths as input
     * and returns them in the ontology provided via the transform spec
    */
    ...
    */
    ...
    /* TODO KASPER: Doc and tests */
    gpii.matchMakerFramework.utils.getActiveFromEnabledTerms = function (specialPrefs, specialCapabilities, solution, solutionId) {
        var active;
        ...
        fluid.each(specialCapabilities, function (term) {
            ...
            if (term.endsWith("/enabled") && solution.capabilities && solution.capabilities.includes(term)) {
                // set the active flag based on the users' preference value
                active = fluid.get(specialPrefs, [ solutionId, term ]) || specialPrefs[term];
            }
        });
        ...
        for (var i = 0; i < priorities.length; i++) {
            ...
            for (var solutionId in sols) {
                ...
                if (tied.length === 0) {
                    ...
                    var prefActive = gpii.matchMakerFramework.utils.getActiveFromEnabledTerms(data.specialPrefs[contextId] || {},
    ...
    /**
     * Adds information about the supported capabilities for each solution (in the flat format).
     * The result will be a list of inputPaths from the capabilitiesTransformations of the settingshandlers
     * as well as the content of the solutions "capabiliites" entry.
     *
     ...
     *
     * @param payload {Object} An object with the format of a standard matchmaker input payload. At a very
     *     minimum this payload should have a `solutionRegistryEntries` key with the solution entries as well
    ...
    gpii.matchMakerFramework.utils.addSolutionTypeInformation = function (payload, transformSpec) {
        fluid.each(payload.solutionCapabilities, function (entry, entryId) {
            // first get an array of what solution types the current entry is:
            var solutionTypes = gpii.matchMakerFramework.utils.transformCapabilitiesToOntologizedLeaves(entry, transformSpec);
            // // Make sure entries are unique // TODO KASPER
            // var uniqueTypes = {};
            // fluid.each(solutionTypes, function (type) {
            //     uniqueTypes[type] = true;
            // });
            // solutionTypes = Object.keys(uniqueTypes);
