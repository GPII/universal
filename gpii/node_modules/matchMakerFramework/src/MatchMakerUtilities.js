...
(function () {
    ...
    */
     ...
     */
    var computeLeavesImpl = function (model, path, paths, ignoreArrayParsing) {
        fluid.each(model, function (value, key) {
            var newPath = fluid.pathUtil.composePath(path, key.toString());
            if (fluid.isPrimitive(value) || ignoreArrayParsing && fluid.isArrayable(value)) {
                paths.push(newPath);
            }
            else {
                computeLeavesImpl(value, newPath, paths, ignoreArrayParsing);
            }
        });
    };
     ...
     * @param {Boolean} ignoreArrayParsing - When a leaf value is an array, indicate whether ignore the further
     * computation to extract array elements. If true, the further computation will be ignored. Otherwise, array
     * elements will be returned as leaves. The default value is false.
     * @return {Array} An array of el-paths, one entry for each leaf found in the object.
     */
    gpii.matchMakerFramework.utils.computeLeaves = function (model, ignoreArrayParsing) {
        ignoreArrayParsing = ignoreArrayParsing || false;
        var paths = [];
        computeLeavesImpl(model, "", paths, ignoreArrayParsing);
        return paths;
    };
    ...
    gpii.matchMakerFramework.utils.addInferredCommonTermsToPreferences = function (preferences, inferred) {
        ...
        fluid.each(fluid.copy(inferred), function (context, contextId) {
