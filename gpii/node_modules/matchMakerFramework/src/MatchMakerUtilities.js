...
(function () {
    ...
    */
    gpii.matchMakerFramework.utils.transformCapabilitiesToLeaves = function (paths, transformSpec) {
    ...
    */
    gpii.matchMakerFramework.utils.computeAndOntologizeCapabilitiesFromSolution = function (solution, transformSpec) {
        ...
        fluid.each(solution.settingsHandlers, function (handlerBlock) {
            var inputPaths = gpii.matchMakerFramework.utils.extractCapabilities(handlerBlock);
            var capabilities = gpii.matchMakerFramework.utils.transformCapabilitiesToLeaves(inputPaths, transformSpec);
            paths = paths.concat(capabilities);
        });
        // grab the capabilities block from the solution if present:
        if (solution.capabilities) {
            var capabilities = gpii.matchMakerFramework.utils.transformCapabilitiesToLeaves(solution.capabilities, transformSpec);
            paths = paths.concat(capabilities);
        }
        return paths;
    };
        ...
        for (var i = 0; i < priorities.length; i++) {
            ...
            for (var solutionId in sols) {
                ...
                if (tied.length === 0) {
                    ...
                    solrecs[solutionId].active = true;
