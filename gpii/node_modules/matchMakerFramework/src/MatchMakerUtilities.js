...
(function () {
    ...
    /**
     * Helper function for computeOntologizedCapabilitiesFromSettingsHandler. Grabs the capabilities (incl. those from the
     * capabilities transformations) for a single settings handler. Returns them in the ontology
     * provided via the transform spec
    */
    gpii.matchMakerFramework.utils.computeOntologizedCapabilitiesFromSettingsHandler = function (settingsHandler, transformSpec) {
    ...
    */
    gpii.matchMakerFramework.utils.computeOntologizedCapabilitiesFromSolution = function (solution, transformSpec) {
        var settingsHandlerBlocks = [];
        fluid.each(solution.settingsHandlers, function (handlerBlock) {
            var capabilities = gpii.matchMakerFramework.utils.computeOntologizedCapabilitiesFromSettingsHandler(handlerBlock, transformSpec);
    ...
    gpii.matchMakerFramework.utils.expandSolutions = function (solutions, transformSpec) {
        return fluid.transform(solutions, function (solution, solutionId) {
            var paths = gpii.matchMakerFramework.utils.computeOntologizedCapabilitiesFromSolution(solution, transformSpec);
    ...
    gpii.matchMakerFramework.utils.addSolutionTypeInformation = function (payload, transformSpec) {
        fluid.each(payload.solutionsRegistryEntries, function (entry, entryId) {
            ...
            var solutionTypes = gpii.matchMakerFramework.utils.computeOntologizedCapabilitiesFromSolution(entry, transformSpec);
