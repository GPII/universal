...
(function () {
    ...
    */
    ...
    /*
     * Merges all the settings handler blocks from [ "configure", "start", "stop", "notifyUpdate" ]
     * sections of solutionsRegistry entry given as parameter. It will return an array which is
     * the concatenation of all the settingsHandler blocks.
     */
    gpii.matchMakerFramework.utils.mergeSettingsHandlerBlocks = function (solution) {
        var entries = [ "configure", "start", "stop", "notifyUpdate" ];
        var settingsHandlerBlocks = [];
        fluid.each(entries, function (key) {
            // concat settingsHandlers block for that entry if it exist, else concat empty array
            settingsHandlerBlocks = settingsHandlerBlocks.concat(fluid.makeArray(fluid.get(solution, [key, "settingsHandlers"])));
        });
    ...
    */
    gpii.matchMakerFramework.utils.computeCapabilitiesFromSolution = function (solution, transformSpec) {
        var leaves = [];
        // merge all settingsHandler blocks
        var settingsHandlers = gpii.matchMakerFramework.utils.mergeSettingsHandlerBlocks(solution);
        leaves = leaves.concat.apply(leaves, fluid.transform(settingsHandlers,
