...
(function () {
    ...
    gpii.matchMakerFramework.utils.preProcess = function (initialPayload) {
        var matchMakerInput = fluid.extend({
            activePrefsSets: [], // set later in the process
    ...
    */
     ...
     *
     * @param {Object} specialPreferences - The specialPreferences value of the relevant context (from the value of the same name in the MatchMaker input payload).
    ...
    */
    ...
    gpii.matchMakerFramework.utils.updateSingleInferredConfiguration = function (inferredConfiguration, context, pref, value, solutionId, upperLevelApplicationTerm) {
        if (solutionId) {
            var path = gpii.matchMakerFramework.utils.isCommonTerm(pref) ?
                [ context, "applications", solutionId, "settings", pref ] :
                [ context, "applications", solutionId, "settings", upperLevelApplicationTerm, pref ];
            ...
            fluid.each(contextBlock.preferences, function (val, pref) {
                if (gpii.matchMakerFramework.utils.isCommonTerm(pref)) {
                    ...
                    fluid.each(val, function (solPrefVal, solPref) {
                        gpii.matchMakerFramework.utils.updateSingleInferredConfiguration(inferred, contextName, solPref, solPrefVal, solutionId, pref);
                    });
                }
            });
