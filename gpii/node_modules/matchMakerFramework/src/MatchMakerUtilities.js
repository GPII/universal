...
(function () {
    ...
    gpii.matchMakerFramework.utils.extractCapabilities = function (settingsHandler) {
        var capabilities = fluid.model.transform.collectInputPaths(settingsHandler.capabilitiesTransformations || {});
        return capabilities.concat(fluid.makeArray(settingsHandler.capabilities));
    };
    ...
    */
    gpii.matchMakerFramework.utils.computeAndOntologizeCapabilitiesFromSettingsHandler = function (settingsHandler, transformSpec) {
        var inputPaths = gpii.matchMakerFramework.utils.extractCapabilities(settingsHandler);
    ...
    */
    ...
    gpii.matchMakerFramework.utils.filterPreferencesForSolution = function (solutionId, allPreferences, solutionEntry) {
        ...
        // filter any common terms *inside the application block* to ensure only valid common terms are there:
        if (allPreferences[applicationString]) { // application block for this solution
            fluid.remove_if(filtered[applicationString], function (value, preference) {
                // Remove if it is not a supported term:
                return (preference.startsWith("http://registry.gpii.net/common/") && !supportedCommon[preference]);
            });
        }
    ...
    gpii.matchMakerFramework.utils.updateInferredConfiguration = function (preferences, inferredConfiguration) {
        ...
        fluid.each(preferences.contexts, function (contextBlock, contextName) {
            fluid.each(contextBlock.preferences, function (val, pref) {
                ...
                } else if (pref.startsWith("http://registry.gpii.net/applications/")) {
    ...
    /**
     * Given a term and one or more solutions registry entries, this function will walk each
     * solution (and its settingshandlers) and find the lowest liveness for any solution that
     * supports the term via its capabilities and/or capabilitiesTransformations. The lowest or least
     * liveness is dictated by gpii.matchMakerFramework.utils.livenessOrdering, where a lower
     * index means less/lower liveness.
     *
     * @param term {String}: the term to find the liveness for
     ...
     */
    gpii.matchMakerFramework.utils.getLeastLiveness = function (term, solutions) {
        var leastLiveness;
