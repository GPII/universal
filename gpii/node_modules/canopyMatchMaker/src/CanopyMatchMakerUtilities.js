...
gpii.canopyMatchMaker.utils.disposeFromCanopy = function (leaves, ranked, solrecs, data, contextId, ontologicalMetricFunction) {
    ...
    for (var i = 0; i < ranked.length; ++i) {
        ...
        if (sol.disposition === "reject" || sol.active === false) {
            continue;
        }
        ...
        if (inCanopy) {
            var prefActive = gpii.matchMakerFramework.utils.getActiveFromEnabledTerms(data.specialPrefs[contextId] || {}, data.specialCapabilities[sol.index], data.solutionsRegistryEntries[sol.index], sol.index);
