...
gpii.canopyMatchMaker.utils.computeFitness = function (leaves, solution, ontologicalMatchingFunction) {
    var vector = [];
    for (var i = 0; i < leaves.length; ++i) {
        var leaf = leaves[i];
        vector[i] = ontologicalMatchingFunction(leaf, solution);
    }
 ...
 * @solrecs {object} map of solutions keyed by solution id.
 */
gpii.canopyMatchMaker.utils.rankSolutions = function (leaves, solrecs, ontologicalMatchingFunction) {
    fluid.each(solrecs, function (solrec) {
        solrec.fitness = gpii.canopyMatchMaker.utils.computeFitness(leaves, solrec.skeleton, ontologicalMatchingFunction);
    });
...
gpii.canopyMatchMaker.utils.disposeStrategy = function (leaves, solrecs, data, ontologyMetadata) {
    var ontologicalMatchingName = fluid.get(ontologyMetadata, "ontologicalMatchingFunction") || "gpii.canopyMatchMaker.utils.prefixLength";
    var ontologicalMatchingFunction = fluid.getGlobalValue(ontologicalMatchingName);
    ...
    var ranked = gpii.canopyMatchMaker.utils.rankSolutions(leaves, solrecs, ontologicalMatchingFunction);
    // dispose based on canopy
    var disposed = gpii.canopyMatchMaker.utils.disposeFromCanopy(leaves, ranked, solrecs, data, ontologicalMatchingFunction);
...
gpii.canopyMatchMaker.utils.disposeFromCanopy = function (leaves, ranked, solrecs, data, ontologicalMatchingFunction) {
    ...
    for (var i = 0; i < ranked.length; ++i) {
        ...
        for (var j = 0; j < leaves.length; ++j) {
            var depth = ontologicalMatchingFunction(leaves[j], sol.skeleton);
            if (depth > canopy[j]) {
                inCanopy = true;
                canopy[j] = depth;
            }
...
gpii.canopyMatchMaker.utils.ISO24751.prefixLength = function (path, model) {
    var segs = fluid.pathUtil.parseEL(path);
