...
(function () {
    ...
    gpii.matchMaker.canopy.computeFitness = function (leaves, solution) {
        ...
        for (var i = 0; i < leaves.length; ++ i) {
            var leaf = leaves[i];
            vector[i] = gpii.matchMaker.canopy.prefixLength(leaf, solution);
        }
        vector = vector.sort(gpii.matchMaker.canopy.sortDescending);
        return vector;
    };
    ...
    gpii.matchMaker.canopy.disposeSolutionsImpl = function (leaves, solrecs) {
        ...
        for (var i = 0; i < solrecs.length; ++ i) {
            ...
            for (var j = 0; j < leaves.length; ++ j) {
                var depth = gpii.matchMaker.canopy.prefixLength(leaves[j], sol.skeleton);
                if (depth > canopy[j]) {
                    inCanopy = true;
                    canopy[j] = depth;
                }
    ...
    gpii.matchMaker.canopy.prefixLength = function (path, model) {
    ...
    gpii.matchMaker.canopy.extractDispositions = function (solrecs) {
        var togo = [];
    ...
    // only used by canopyMatchMaker - moved from MatchMakerFramework
    gpii.matchMaker.canopy.sortDescending = function (numA, numB) {
        return numB - numA;
    };

    // TODO: UNUSED FUNCTION
    gpii.matchMaker.canopy.makeComparator = function (leaves) {
        return function (solnA, solnB) {
            var domA = false, domB = false;
            for (var i = 0; i < leaves.length; ++ i) {
                var leaf = leaves[i];
                var goodA = gpii.matchMaker.canopy.prefixLength(leaf, solnA.solution);
                var goodB = gpii.matchMaker.canopy.prefixLength(leaf, solnB.solution);
                if (goodA > goodB) {
                    domA = true;
                }
                else if (goodB > goodA) {
    ...
    gpii.matchMaker.canopy.bestCoverage = function (leaves, soln) {
        ...
        for (var i = 0; i < leaves.length; ++ i) {
            var leaf = leaves[i];
            var good = gpii.matchMaker.canopy.prefixLength(leaf, soln.solution);
