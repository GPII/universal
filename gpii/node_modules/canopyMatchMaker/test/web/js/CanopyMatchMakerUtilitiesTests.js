...
gpii.tests.canopyMatchMaker.runBasicTests = function () {
    ...
    jqUnit.test("Rank and dispose solutions - non-conflicting solutions matching canopy equally well", function () {
        // Using a preferences set that matches the two solutions equally well the one with the highest priority will be chosen:
        var solutions = {
            "lesserMag": lesserMagnifier,
            "mag": magnifier
        };
        var solrecs = gpii.matchMakerFramework.utils.expandSolutions(solutions, { "*": "*" });
        solrecs.mag.priority = 1024;
        var ranked = gpii.canopyMatchMaker.utils.rankSolutions(["display.screenEnhancement.magnification"], solrecs, gpii.canopyMatchMaker.utils.ISO24751.ontologicalMetric);
        var expectedRanking = ["mag", "lessermag"]
        jqUnit.assertDeepEq("Ranked solutions - tie break equal canopy match using priorities", expectedRanking, ranked);
        var minimalData = {
            solutionTypes: {},
            solutionTypeMapping: {}
        };

        var disposed = gpii.canopyMatchMaker.utils.disposeFromCanopy(["display.screenEnhancement.magnification"], ranked, solrecs, minimalData, gpii.canopyMatchMaker.utils.ISO24751.ontologicalMetric);
        var dispositions = gpii.canopyMatchMaker.utils.extractDispositions(disposed);
        expected = {
            "mag": "accept",
            "lesserMag": "reject"
        };
        jqUnit.assertDeepEq("Disposed solutions", expected, dispositions);
    });
};
