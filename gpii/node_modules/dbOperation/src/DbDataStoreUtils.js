...
gpii.dbOperation.dbDataStore.findRecord = function (dataSource, directModel, valueNotEmpty, dataProcessFunc) {
    ...
    if (emptyFields.length > 0) {
        var error = gpii.oauth2.composeError(gpii.dbOperation.errors.missingInput, {fieldName: emptyFields.join(" & ")});
...
gpii.dbOperation.dbDataStore.addRecord = function (dataSource, docType, idName, data) {
    ...
    if (data !== undefined) {
        ...
        var error = gpii.oauth2.composeError(gpii.dbOperation.errors.missingDoc, {docName: docType});
...
gpii.dbOperation.dbDataStore.addAuthorization = function (saveDataSource, authorizationType, authorizationData) {
    ...
    if (authorizationType !== gpii.dbOperation.docTypes.gpiiAppInstallationAuthorization) {
        error = gpii.oauth2.composeError(gpii.dbOperation.errors.unauthorized, {docName: authorizationType});
        promiseTogo.reject(error);
        return promiseTogo;
    }
    ...
    if (!authorizationData) {
        error = gpii.oauth2.composeError(gpii.dbOperation.errors.missingDoc, {docName: authorizationType});
        promiseTogo.reject(error);
        return promiseTogo;
    }
