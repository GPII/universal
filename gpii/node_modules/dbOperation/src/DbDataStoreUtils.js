...
*/
gpii.dbOperation.dbDataStore.addRecord = function (dataSource, docType, idName, data) {
 ...
 * @param error {Object} An object that contains an element keyed by "message".
 * @param termMap {Object} An object that contains the mapping between terms used within error.message and their actual values.
...
gpii.dbOperation.verifyAddedData = function (typeValue, type, data) {
    // Verify data type
    if (typeValue !== fluid.get(gpii.dbOperation.docTypes, [type])) {
        return gpii.dbOperation.composeError(gpii.dbOperation.errors.unauthorized, {docName: type});
    }

    // Verify the existence of the data
    if (!data) {
        return gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: type});
    }
...
gpii.dbOperation.dbDataStore.findPrefsSafeByGpiiKeyPostProcess = function (data) {
    ...
    if (data && data.doc && data.value) {
        ...
        fluid.set(result, "prefsSafe", gpii.dbOperation.dbDataStore.cleanUpDoc(data.doc));
    }
 ...
 * @param prefsSafeType {String} The prefs safe type
 ...
 */
gpii.dbOperation.dbDataStore.addPrefsSafe = function (saveDataSource, prefsSafeType, prefsSafeData) {
    var promiseTogo = fluid.promise();
    var data;
    var error = gpii.dbOperation.verifyAddedData(prefsSafeType, "prefsSafe", prefsSafeData);
    ...
    promiseTogo = gpii.dbOperation.dbDataStore.addRecord(saveDataSource, prefsSafeType, "id", data);
    return promiseTogo;
};
 ...
 * authorization record, or an error if `gpiiAppInstallationAuthorizationData` parameter is not provided.
 */
gpii.dbOperation.dbDataStore.addAuthorization = function (saveDataSource, authorizationType, authorizationData) {
    var promiseTogo = fluid.promise();
    var data;
    var error = gpii.dbOperation.verifyAddedData(authorizationType, "gpiiAppInstallationAuthorization", authorizationData);
    ...
    promiseTogo = gpii.dbOperation.dbDataStore.addRecord(saveDataSource, authorizationType, "id", data);
    return promiseTogo;
};
