/*!
GPII CouchDB/PouchDB Data Store Utilities
 ...
 * @param dataSource {Component} An instance of gpii.dbOperation.dbDataSource
...
gpii.dbOperation.dbDataStore.findRecord = function (dataSource, directModel, valueNotEmpty, dataProcessFunc) {
    ...
    if (emptyFields.length > 0) {
        ...
        promise.then(function (data) {
            ...
            fluid.log("dbOperation, findRecord(), error occurs: ", error);
            promiseTogo.reject(error);
        });
    }
 ...
 * @param dataSource {Component} An instance of gpii.dbOperation.dbDataSource that handles the record creation
 * @param docType {String} The document type. See gpii.dbOperation.docTypes defined in
 * %gpii-universal/gpii/node_modules/dbOperation/src/DbConst.js
 * @param idName {String} The name for the unique id field. Usually "id".
 * @param data {Object} The data to be saved in the new record
 * @param idValue {String} Optional. The id value. If not provided, an UUID will be generated.
...
gpii.dbOperation.dbDataStore.addRecord = function (dataSource, docType, idName, data, idValue) {
    ...
    if (data !== undefined) {
        ...
        fluid.log("dbOperation, addRecord(), error occurs: ", error);
        var error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: docType});
 ...
 * @param dataSource {Component} An instance of gpii.dbOperation.dbDataSource that handles the record update
 * @param docType {String} The expected document type in the updated record. See gpii.dbOperation.docTypes defined in
 * %gpii-universal/gpii/node_modules/dbOperation/src/DbConst.js
...
gpii.dbOperation.dbDataStore.updateRecord = function (dataSource, docType, docId, data) {
    ...
    if (!data) {
        error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: docType});
        promise.reject(error);
    } else if (data.type !== docType) {
        error = gpii.dbOperation.composeError(gpii.dbOperation.errors.mismatchedDocType, {docType: docType});
        promise.reject(error);
    } else {
        var directModel = {};
        fluid.set(directModel, "id", docId);
        var finalDirectModel = fluid.extend(true, {}, dataSource.options.directModel, directModel);
        promise = dataSource.set(finalDirectModel, data);
    }
    return promise;
...
/**
 * Add a GPII key
 * @param saveDataSource {Component} The saveDataSource component provided by gpii.dbOperation.dbDataStore
 * @param gpiiKeyData {Object} The GPII key data. An example of the data:
 *  {
 ...
 * GPII key record, or an error if `gpiiKeyData` parameter is not provided.
 */
gpii.dbOperation.dbDataStore.addGpiiKey = function (saveDataSource, gpiiKeyData) {
    ...
    data = {
        ...
        prefsSetName: gpiiKeyData.prefsSetName || null,
        revoked: false,
        revokedReason: null,
 ...
 * @param saveDataSource {Component} The saveDataSource component provided by gpii.dbOperation.dbDataStore
 * @param gpiiKey {String} The GPII key.
 * @param gpiiKeyData {Object} The GPII key data. An example of the data:
 *  {
 *      type: {String},
...
gpii.dbOperation.dbDataStore.updateGpiiKey = function (saveDataSource, gpiiKey, gpiiKeyData) {
    ...
    if (!gpiiKey) {
        ...
        error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: gpii.dbOperation.docTypes.gpiiKey});
        promiseTogo.reject(error);
    } else {
        data = {
            ...
            prefsSetName: gpiiKeyData.prefsSetName,
...
/**
 * Find a preferences by a GPII key
...
gpii.dbOperation.dbDataStore.findPrefsSafeByGpiiKeyPostProcess = function (data) {
    ...
    if (data && data.doc && data.value) {
        result = {};
        fluid.set(result, "gpiiKey", data.key);
        fluid.set(result, "gpiiKeyDetails", gpii.dbOperation.dbDataStore.cleanUpDoc(data.value.gpiiKey));
        fluid.set(result, "prefsSafe", data.doc.type === gpii.dbOperation.docTypes.prefsSafe ? gpii.dbOperation.dbDataStore.cleanUpDoc(data.doc) : null);
    }
 ...
 * Add a prefs safe
 * @param saveDataSource {Component} The saveDataSource component provided by gpii.dbOperation.dbDataStore
 * @param prefsSafeData {Object} The prefs safe data. An example of the data:
 *  {
...
gpii.dbOperation.dbDataStore.addPrefsSafe = function (saveDataSource, prefsSafeData) {
    ...
    if (!prefsSafeData) {
        var error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: gpii.dbOperation.docTypes.prefsSafe});
        promiseTogo.reject(error);
    } else {
        data = {
            schemaVersion: gpii.dbOperation.schemaVersion,
 ...
 * Update a prefs safe
 * @param saveDataSource {Component} The saveDataSource component provided by gpii.dbOperation.dbDataStore
 * @param prefsSafeId {String} The prefs safe id.
 * @param prefsSafeData {Object} The prefs safe data. An example of the data:
 *  {
 *      type: {String},
 *      schemaVersion: {String},
...
gpii.dbOperation.dbDataStore.updatePrefsSafe = function (saveDataSource, prefsSafeId, prefsSafeData) {
    ...
    if (!prefsSafeId) {
        ...
        error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: gpii.dbOperation.docTypes.prefsSafe});
        promiseTogo.reject(error);
    } else {
        data = {
 ...
 *     }
 * }
 * @return {Promise} An object that carries the client and authorization information associated with the access token.
 */
gpii.dbOperation.dbDataStore.findClientByOauth2ClientIdPostProcess = function (data) {
    ...
    if (data && data.doc && data.value) {
        ...
        fluid.set(result, "client", gpii.dbOperation.dbDataStore.cleanUpDoc(data.doc));
        fluid.set(result, "clientCredential", gpii.dbOperation.dbDataStore.cleanUpDoc(data.value.clientCredential));
    }
...
gpii.dbOperation.dbDataStore.findAuthorizationByAccessTokenPostProcess = function (data) {
    ...
    if (data && data.doc && data.value) {
        ...
        fluid.set(result, "client", gpii.dbOperation.dbDataStore.cleanUpDoc(data.doc));
        fluid.set(result, "authorization", gpii.dbOperation.dbDataStore.cleanUpDoc(data.value.authorization));
    }
 ...
 * @param saveDataSource {Component} The saveDataSource component provided by gpii.dbOperation.dbDataStore
 * @param authorizationData {Object} The authorization data. An example of gpiiAppInstallationAuthorization data:
 *
 ...
 *
 * @return {Promise} A promise object that carries either a response returned from CouchDB/PouchDB for adding the
 * authorization record, or an error if `gpiiAppInstallationAuthorizationData` parameter is not provided.
 */
gpii.dbOperation.dbDataStore.addAuthorization = function (saveDataSource, authorizationData) {
    ...
    if (!authorizationData) {
        var error = gpii.dbOperation.composeError(gpii.dbOperation.errors.missingDoc, {docName: gpii.dbOperation.docTypes.gpiiAppInstallationAuthorization});
        promiseTogo.reject(error);
    } else {
        data = {
            schemaVersion: gpii.dbOperation.schemaVersion,
