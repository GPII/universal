...
fluid.registerNamespace("gpii.userListeners.pcsc.pcsc");
    ...
    reader.on("status", function (status) {
        ...
        if (action === "presented") {
            ...
            gpii.userListeners.pcsc.readCard(that, reader, status.atr).then(function (tag) {
                that.events.onTokenArrive.fire(that, tag);
            }, function () {
...
gpii.userListeners.pcsc.detectCard = function (atr) {
    ...
    if (atr instanceof Buffer) {
        ...
        card = fluid.find(knownCards, function (card) {
    ...
    if (cardType) {
        gpii.userListeners.pcsc.connectToCard(reader, cardType).then(function (card) {
            gpii.userListeners.pcsc.readNDEF(card).then(function (tagBuffer) {
                var text = gpii.userListeners.pcsc.decodeNDEF(tagBuffer);
                if (text) {
...
gpii.userListeners.pcsc.connectToCard = function (reader, cardType) {
    ...
    reader.connect({share_mode: reader.SCARD_SHARE_SHARED}, function (err, proto) {
        if (err) {
            promise.reject("reader.connect", err);
        } else {
            var card = Object.assign({
                transmit: function (data, expectedLength, callback) {
                    return reader.transmit(data, expectedLength, proto, callback);
                }
            }, cardType);
...
/**
 * Decodes an NDEF message, returning the value of the first text record.
 * @param tagBuffer
 * @return {*}
 */
gpii.userListeners.pcsc.decodeNDEF = function (tagBuffer) {
    var text = null;
    if (tagBuffer) {
