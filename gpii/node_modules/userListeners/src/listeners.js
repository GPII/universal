...
fluid.defaults("gpii.userListener", {
    ...
    members: {
        ...
        proximity: false,
        listenerName: "@expand:fluid.identity({that}.typeName)"
    },
    listeners: {
...
gpii.userListeners.startListener = function (that) {
    that.startListener().then(function () {
        fluid.log(fluid.logLevel.WARN, that.listenerName + " user listener started");
    }, function (err) {
        fluid.log(fluid.logLevel.WARN, that.listenerName + " user listener not started: ", err && err.message || err);
    });
};
