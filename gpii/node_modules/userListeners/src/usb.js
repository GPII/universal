 ...
 *
 * If there's already a device with a token, then this one is ignored.
 *
 * @param that {Component} The gpii.userListeners.usb instance.
 * @param usbPath {String} The root of the USB device.
 * @returns {Promise} resolves when the token is read, rejects on error (or there's already a device)
 */
gpii.userListeners.usb.readUSBToken = function (that, usbPath) {
    var promise = fluid.promise();

    if (that.currentDevice.path) {
        fluid.log("Ignoring USB device (" + usbPath + "): There's already a device with a token, at " +
            that.currentDevice.path);
        promise.reject({
            isError: true,
            message: "There's already a device with a token"
        });
    } else {
        ...
        fs.readFile(tokenFile, "utf8", function (err, data) {
            if (err) {
                ...
                var token = data.trim();
                fluid.log("Got token from USB device (" + usbPath + "):", token);
