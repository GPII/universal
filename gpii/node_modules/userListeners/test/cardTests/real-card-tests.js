...
fluid.defaults("gpii.pcscUserListener.test", {
    invokers: {
        getPCSC: "gpii.tests.card.getPCSC",
        startListener: "fluid.identity"
    }
});

gpii.tests.card.getPCSC = function () {
    return new Pcsclite();
};
...
/**
 * Get an instance of the gpii.pcscUserListener component.
 */
gpii.tests.card.getListenerComponent = function () {
    var userListeners = gpii.userListeners({
        distributeOptions: {
            record: "gpii.pcscUserListener.test",
            target: "{that pcscUserListener}.options.gradeNames"
        }
    });
...
jqUnit.asyncTest("Card connect and read", function () {
    ...
    jqUnit.expect(2);
    var pcsc = gpii.tests.card.getPCSC();
    ...
    gpii.tests.card.connectToCard(pcsc).then(function (result) {
        ...
        console.log("Card type:", cardType.name);
        ...
        gpii.userListeners.pcsc.readNDEF(card).then(function (tag) {
            console.log("TAG", tag);
            jqUnit.start();
        }, function (err) {
            jqUnit.fail(err);
...
jqUnit.asyncTest("Card user listener", function () {
    jqUnit.expect(3);
    var listener = gpii.tests.card.getListenerComponent();
