...
fluid.defaults("gpii.test.userListeners.userListener", {
    invokers: {
...
gpii.tests.userListener.getPcscListener = function () {
    var userListeners = gpii.userListeners({
        ...
        distributeOptions: {
            record: "gpii.test.userListeners.userListener",
 ...
 * @param options.onDisconnect {Function} Callback when
...
jqUnit.asyncTest("PC/SC listener - mock card", function () {
    ...
    jqUnit.expect(gpii.tests.userListener.cardTests.length * 4);
    var index = 0;

    var listener = gpii.tests.userListener.getPcscListener();
    var reader;
    ...
    reader = gpii.tests.userListener.mockReader({
        onDisconnect: function () {
            jqUnit.assert("should disconnect after every read");
        }
    });
