    ...
    if (emptyFields.length > 0) {
        ...
        promise.then(function (data) {
            ...
            if (result !== undefined && result.isError) {
                result.dbUrl = gpii.dbOperation.dbDataStore.sanitizeUrl(dataSource);
                promiseTogo.reject(result);
            } else {
                promiseTogo.resolve(result);
            }
        }, function (error) {
            error.dbUrl = gpii.dbOperation.dbDataStore.sanitizeUrl(dataSource);
...
/**
 * Return a version of the database URL without showing sensitive information,
 * such as user name and/or password.
 * @param {Component} dataSource - An instance of gpii.dbOperation.dbDataSource.
 * @return {String} A sanitized version of the url.
 */
gpii.dbOperation.dbDataStore.sanitizeUrl = function (dataSource) {
    var fullUrl = new urlModule.URL(
        dataSource.resolveUrl(dataSource.options.url, dataSource.options.termMap, dataSource.options.directModel)
    );
    return urlModule.format(fullUrl, {auth: false});
};
